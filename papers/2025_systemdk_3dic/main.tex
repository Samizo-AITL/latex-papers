...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[font=\sffamily, node distance=32mm, >=Latex,
    box/.style={draw, rounded corners, fill=blue!10, minimum width=36mm, minimum height=12mm, very thick, align=center}]
    \node[box] (eval) {\bfseries Evaluation};
    \node[below=2.0mm of eval] {\footnotesize Simulation \& Measurements};

    \node[box, right=of eval] (sysdk) {\bfseries SystemDK};
    \node[below=2.0mm of sysdk] {\footnotesize Constraint Translation};

    \node[box, right=of sysdk, minimum width=48mm] (eda) {\bfseries EDA Flow};
    \node[below=2.0mm of eda] {\footnotesize STA / P\&R / CTS / Floorplan};

    \draw[->, very thick] (eval) -- (sysdk);
    \draw[->, very thick] (sysdk) -- (eda);
  \end{tikzpicture}}
