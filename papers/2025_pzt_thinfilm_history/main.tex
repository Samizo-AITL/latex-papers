...
\tikzset{
  ...
  alox/.style={blue!60,line width=0.9pt}
}
\def\W{80}
\def\X{0}
\def\gap{22}

% ===== (a) 共通BE（TE=COM, BE=VBS）— PZT縁露出 → AlOxあり =====
\begin{scope}[shift={(0,0)}]
  ...
  % ラベル
  \node[anchor=west] at (\X,12.8){\textbf{(a)} 共通BE（TE=COM，BE=VBS）— PZT縁露出のため AlO$_x$ 保護あり};
\end{scope}

% ===== (b) 共通TE（TE=VBS, BE=COM）— PZT/TE一体 → AlOxなし =====
\begin{scope}[shift={(0,-\gap)}]
  ...
  % PZT島（上面でTEと接続：側壁保護不要）
  \path[pzt] (\X+11,5) -- ++(13,0) -- ++(-2,5) -- ++(-9,0) -- cycle;
  \path[pzt] (\X+46,5) -- ++(13,0) -- ++(-2,5) -- ++(-9,0) -- cycle;

  % 共通TE (VBS) — 連続バーでPZT上面に接続
  \fill[te] (\X,10) rectangle (\X+\W,11);
  % ※ AlOx は描かない（PZT/TEが繋がっており側壁露出領域がないため）
  \node[anchor=west] at (\X+\W+2,10.5){\scriptsize TE: VBS};

  % ラベル
  \node[anchor=west] at (\X,13.2){\textbf{(b)} 共通TE（TE=VBS，BE=COM）— PZT/TE 接続のため AlO$_x$ なし};
\end{scope}
...
\caption{%
$\mu$TFP の電極駆動構造。%
(a) 共通BE（TE=COM，BE=VBS）：PZT縁が露出するため AlO$_x$ 側壁保護が必要。%
(b) 共通TE（TE=VBS，BE=COM）：PZT と TE が上面で接続され側壁露出が無く、AlO$_x$ は不要。}
