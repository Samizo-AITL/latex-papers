...
\usepackage{pgfplots}
\pgfplotsset{compat=1.18} % ← 共通の目盛り上書きはしない＝標準のまま
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{cite}
...
% --- 参考文献の行間（少しゆったり） ---
\makeatletter
\setlength{\IEEEbibitemsep}{1.0ex plus 0.2ex}
\makeatother
...
% ===== References =====
% \balance  % ← 外す：右カラムに回り込まないようにする
% 参考：もし左カラムに収まらない場合は、次の数字を1つずつ増やして
% 最終ページの段落改め位置を前に寄せてください。
% 例：\IEEEtriggercmd{\newpage}\IEEEtriggeratref{12}
\IEEEtriggercmd{\newpage}%
\IEEEtriggeratref{12}% ← 目安。左カラムに文献が全て入る値に調整
