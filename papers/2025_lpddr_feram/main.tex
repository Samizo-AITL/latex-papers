...
\usepackage{pgfplots}
\pgfplotsset{compat=1.18} % ← 標準

\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{cite}
\usepackage{balance}
\usepackage[hidelinks]{hyperref} % ← できるだけ最後

% ====== lengths that must be set after \begin{document} ======
% 参考文献の行間（安全のため文書開始時に設定）
\AtBeginDocument{\setlength{\IEEEbibitemsep}{1.0ex plus 0.2ex}}
...
% ===== References =====
\vspace{1.0\baselineskip}   % 本文とREFERENCESタイトルの間を広げる
\bibliographystyle{IEEEtran}
\bibliography{references}

% ===== Author Biography =====
\vspace{-0.75\baselineskip} % ReferencesとBiographyの間を詰める
\begingroup\small
