...
% ===== フェイルビットマップ（1カラム幅・堅牢版） =====
\begin{figure}[!t]
\centering
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}
  % パラメータ（単位はここで定義）
  \def\WaferR{3cm}    % ウエハ半径
  \def\DotR{0.5pt}    % フェイル点半径（見やすく 0.5pt 推奨）

  % ウエハ外形
  \draw[thick] (0,0) circle (\WaferR);

  % 再現性が必要ならシード固定（不要なら削除）
  \pgfmathsetseed{2025}

  % 極座標でランダム配置：角度=0..360°, 半径=WaferR*sqrt(rand)
  \foreach \i in {1,...,120} {%
    \pgfmathsetmacro{\ang}{rand*360}%
    \pgfmathsetmacro{\rho}{sqrt(rand)}% 0..1（均一分布→面積一様化のためsqrt）
    \pgfmathsetlengthmacro{\rlen}{\rho*\WaferR}% ← 長さに変換（単位付き）
    \fill ([shift={(\ang:\rlen)}]0,0) circle[radius=\DotR];
  }
\end{tikzpicture}%
} % \resizebox を閉じる
