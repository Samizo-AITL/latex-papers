...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % Halo B（楕円の頂点が Active 上面に一致）
  \fill[orange!45,opacity=.45] (1.25,\HaloCy) ellipse (0.38 and \HaloRy);
  \draw[orange!70!black,->] (1.25,\ActiveTop+0.04) -- (1.25,\HaloCy+\HaloRy*0.6);
  \node[orange!70!black,anchor=west] at (1.33,\ActiveTop+0.06) {\scriptsize Halo B};

  % 接触部に高抵抗スポット表示（Halo と TiSi2 が触れる付近）
  \fill[orange!80!black] (1.52,\ActiveTop) circle (0.03); % 接触点
  \draw[orange!80!black,->] (1.8,1.00) -- (1.54,\ActiveTop+0.02);
  \node[orange!80!black] at (2.25,1.02) {\scriptsize 高抵抗スポット};
\end{tikzpicture}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % Side Wall（対策後：基部を広げて“幅広”を明瞭化）
  %  ベース幅を 1.3 -> 1.80 まで後退させた台形（上が細・下が広）
  \fill[gray!55] (1.3,\ActiveTop) -- (1.80,\ActiveTop) -- (1.45,2.1) -- (1.3,2.1) -- cycle;
  \node[anchor=west] at (1.50,1.55) {\scriptsize Side Wall (Under)};
  ...
  % Halo B（(a) と同じ y で頂点＝Active 上面）
  \fill[orange!35,opacity=.35] (1.25,\HaloCy) ellipse (0.38 and \HaloRy);

  % 距離の可視化
  \draw[orange!70!black,<->] (1.80,0.55) -- (1.95,0.55);
  \node[orange!70!black] at (1.88,0.70) {\scriptsize 距離確保};
\end{tikzpicture}

\footnotesize (b) 対策後（Side Wall Etch Under）：サイドウォールを幅広に後退させ，Halo B と TiSi$_2$ 前線の
距離を確保して干渉を低減。
\normalsize
