...
\begin{tabular}{c}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      xlabel={VDD [V]}, ylabel={Pass/Fail},
      ymin=0, ymax=1.2, xmin=2.5, xmax=3.6,
      ytick={0,1}, yticklabels={Fail,Pass},
      title={Untreated}
    ]
      \addplot+[mark=*] coordinates {(2.7,0) (3.0,0) (3.3,0) (3.6,0)};
    \end{axis}
  \end{tikzpicture} \\[1ex]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      ymin=0, ymax=1.2, xmin=2.5, xmax=3.6,
      ytick={0,1}, yticklabels={Fail,Pass},
      title={Side Wall Etch Under}
    ]
      \addplot+[mark=*] coordinates {(2.7,0) (3.0,0) (3.3,1) (3.6,1)};
    \end{axis}
  \end{tikzpicture} \\[1ex]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot+[mark=*] coordinates {(2.7,1) (3.0,1) (3.3,1) (3.6,1)};
    \end{axis}
  \end{tikzpicture}
\end{tabular}
\caption{Shmoo plots of random single-bit failures.
Untreated fails at both 25\,$^\circ$C and 85\,$^\circ$C.
Side Wall Etch Under passes at 25\,$^\circ$C but fails at 85\,$^\circ$C
(recovered at higher VDD).
RTA Optimized process shows pass at both temperatures.
（日英併記）未対策では25℃/85℃ともに不良。
Side Wall Etch Underでは25℃で合格するが85℃で不良（電圧上昇で回復）。
RTA最適化では85℃でも合格。}
\label{fig:shmoo}
\end{figure}
