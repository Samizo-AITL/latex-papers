...
\begin{tikzpicture}
  % ---- params (units only here)
  \def\WaferR{3cm}   % wafer radius
  \def\DotR{0.5pt}   % dot radius
  ...
  % uniform-in-area placement
  \foreach \i in {1,...,120} {%
    \pgfmathsetmacro{\ang}{rand*360}%
    \pgfmathsetmacro{\rho}{sqrt(rand)}% unitless (0..1)
    \pgfmathsetlengthmacro{\rlen}{\rho*\WaferR}% length now
