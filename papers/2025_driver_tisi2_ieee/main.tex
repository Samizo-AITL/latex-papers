...
\paragraph*{ゲート酸化膜（重ね酸化＋酸化膜除去）}
多電圧素子を同一ウェーハ上で形成するため、%
\textbf{重ね酸化とウエットエッチによる酸化膜除去を交互に繰り返す方式}を採用した。%
すなわち、G1→G2→G3の各酸化工程の間に、%
フォトマスクで不要領域を開口し、%
その領域の酸化膜をウエットエッチで除去してから次の酸化を行う。%
これにより、領域ごとに異なる膜厚を選択的に残すことができる。

\begin{table}[H]
\centering
\caption{電圧と酸化スタックの対応（重ね酸化＋除去法）}
...
多電源ゲート絶縁膜構造を一連の酸化・除去工程で形成した。%

本手法は、LTPS パネル駆動用の高電圧素子とロジック素子を同一基板上で実現するものであり、%
当時のエプソンにおける多電源対応CMOS技術の一般的設計手法に基づく。%
本プロセス自体は開発段階で終了したが、%
その構成原理は後続世代のHV CMOS系プロセスにも共通する技術思想である。
