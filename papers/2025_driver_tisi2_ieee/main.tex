...
\begin{tikzpicture}[x=1cm,y=1cm]
  % Active (p-sub) --- 左辺を x=0 に
  \fill[gray!20] (0,0) rectangle (6,0.7);
  \node[anchor=west] at (0.10,0.08) {\scriptsize Active (p-sub)};

  % Poly（左辺を Active と同じ x=0 に合わせる）
  \fill[blue!22] (0,0.7) rectangle (1.3,2.1);
  \node at (0.65,1.58) {\scriptsize Poly};

  % Side Wall（対策前：拡散側まで張り出し）
  \fill[gray!55] (1.3,0.7) -- (1.55,0.7) -- (1.35,2.1) -- (1.3,2.1) -- cycle;
  ...
  % TiSi2（拡散側：近接）
  \fill[red!28] (1.55,0.7) rectangle (5.8,0.92);
  \node[red!60!black,anchor=west] at (4.6,1.05) {\scriptsize TiSi$_2$};

  % Halo B（近接して干渉）
  \fill[orange!45,opacity=.45] (1.25,0.42) ellipse (0.38 and 0.18);
  \draw[orange!70!black,->] (1.25,0.74) -- (1.25,0.56);
  \node[orange!70!black,anchor=west] at (1.33,0.77) {\scriptsize Halo B};
  \node[orange!70!black,anchor=west] at (1.33,0.57) {\scriptsize 高抵抗スポット化};
\end{tikzpicture}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % Poly（長方形・左辺 x=0 で (a) と統一）
  \fill[blue!22] (0,0.7) rectangle (1.3,2.1);
  \node at (0.65,1.58) {\scriptsize Poly};

  % Side Wall（エッチアンダーで後退）
  \fill[gray!55] (1.3,0.7) -- (1.45,0.7) -- (1.25,2.1) -- (1.3,2.1) -- cycle;
  \node[anchor=west] at (1.33,1.55) {\scriptsize Side Wall (Under)};
  ...
  % TiSi2（拡散側：離隔を確保）
  \fill[red!28] (1.80,0.7) rectangle (5.8,0.92);

  % Halo B（離隔を強調）
  \fill[orange!35,opacity=.35] (1.25,0.38) ellipse (0.38 and 0.18);
  \draw[orange!70!black,<->] (1.45,0.55) -- (1.80,0.55);
  \node[orange!70!black] at (1.63,0.70) {\scriptsize 距離確保};

  % ラベル
  \node[red!60!black,anchor=west] at (4.6,1.05) {\scriptsize TiSi$_2$};
\end{tikzpicture}

\footnotesize (b) 対策後（Side Wall Etch Under）：サイドウォール後退により Halo B と TiSi$_2$ 前線の
距離が確保され，局所高抵抗スポットの発生が抑制される。
\normalsize

\caption{サイドウォール・エッチアンダーの効果模式図。対策前は Halo B が TiSi$_2$ 前線に近接し，
相転移が進みにくく局所高抵抗スポットを誘発。対策後は距離確保により干渉が低減し，読み出しマージンが改善する。}
