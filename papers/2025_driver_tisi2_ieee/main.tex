...
% --- 共通パラメータ ---
\newcommand{\ActiveTop}{0.7}             % Active (p-sub) 上面 y
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % Halo B（楕円の頂点は Active 上面に一致）＋ ラベルは右側
  \fill[orange!45,opacity=.45] (1.25,\HaloCy) ellipse (0.38 and \HaloRy);
  \node[orange!70!black,anchor=west] at (1.65,\ActiveTop+0.02) {\scriptsize Halo B};

  % 接触部：高抵抗スポット（点＋テキストのみ）
  \fill[orange!80!black] (1.55,\ActiveTop) circle (0.035);
  \node[orange!80!black,anchor=west] at (1.62,\ActiveTop+0.02) {\scriptsize 高抵抗スポット};
\end{tikzpicture}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % Side Wall（幅広：基部を後退させて強調）
  \fill[gray!55] (1.3,\ActiveTop) -- (1.95,\ActiveTop) -- (1.55,2.1) -- (1.3,2.1) -- cycle;
  \node[anchor=west] at (1.60,1.55) {\scriptsize Side Wall (Under)};
  ...
  % TiSi2（拡散側：離隔を確保）
  \fill[red!28] (2.05,\ActiveTop) rectangle (5.8,0.92);
  \node[red!60!black,anchor=west] at (4.6,1.05) {\scriptsize TiSi$_2$};

  % Halo B（(a) と同じ高さ）＋ ラベルは右側（矢印なし）
  \fill[orange!35,opacity=.35] (1.25,\HaloCy) ellipse (0.38 and \HaloRy);
  \node[orange!70!black,anchor=west] at (1.65,\ActiveTop+0.02) {\scriptsize Halo B};
\end{tikzpicture}

\footnotesize (b) 対策後（Side Wall Etch Under）：サイドウォールを幅広に後退し，Halo B と TiSi$_2$ 前線の距離を確保。
\normalsize

\caption{サイドウォール・エッチアンダーの効果模式図。両図とも Halo 楕円の頂点は Active 上面に一致。
(a) では Halo と TiSi$_2$ の接触近傍に高抵抗スポットが形成。(b) では離隔により発生が抑制される。}
