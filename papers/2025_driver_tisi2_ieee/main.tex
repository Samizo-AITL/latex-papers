...
% ===== フェイルビットマップ（1カラム幅・エラー無し） =====
\begin{figure}[!t]
\centering
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}
  % パラメータ
  \def\WaferR{3}            % 座標系の半径（cm単位で解釈される）
  \def\DotR{0.4pt}          % フェイル点の半径（単位必須）

  % ウエハ円（半径3cm）
  \draw[thick] (0,0) circle (\WaferR cm);

  % ランダムにフェイル点を配置（角度:度, 半径:0〜\WaferR）
  \pgfmathsetseed{2025}     % 繰り返しビルドで図形を固定したい場合
  \foreach \i in {1,...,120} {%
    \pgfmathsetmacro{\ang}{rand*360}%
    \pgfmathsetmacro{\rad}{\WaferR*sqrt(rand)}%
    \fill (\rad*cos(\ang), \rad*sin(\ang)) circle[radius=\DotR];
  }
\end{tikzpicture}%
} % \resizebox の閉じ
