...
\usepackage{array}         % 表列幅指定
\usepackage{caption}
\usepackage{subcaption}    % 複数図の並列表記
...
\subsection{不良現象の観測}
量産試験において，VDD=\SI{3.3}{V}での機能試験ならびに\SI{25}{^\circ C}および\SI{85}{^\circ C}でのSRAMマクロBISTを実施した結果，不良はアドレス空間にランダムに散在して発生した。  
行列欠陥に特徴的な直線的クラスタは観測されず，典型的な\emph{単一セル不良}であることが確認された。  
またデータパターン依存性は弱く，March C-/LAテストで再現性をもって観測された。

\subsection{電気的シグネチャ}
Shmoo解析では，電源電圧マージンの低下時に不良出現確率が上昇し，高温条件でさらに顕著となった。  
保持特性への依存は小さく，\emph{読出し動作時の不安定化}が主要因であることを示唆した。  
また，ビット線RCの大幅な変動は認められず，セル内部素子の局所抵抗上昇が原因と推定された。

\subsection{切り分け}
レイアウトとの相関解析では，端列やコーナ部，デュアルポート近傍など特定パターンとの関連は見られなかった。  
プロセス条件との相関では，Ti膜厚，RTAランプ条件，およびhalo Bのドーズ量・オフセットに依存傾向が認められた。  
特にサイドウォールエッチアンダーを深めた実験ロットでは不良発生率が減少し，後述の対策方針と整合した。

\subsection{物理解析}
代表的な不良セルに対し，OBIRCH/LIVAによるホットスポット観測を行い，FIB断面TEM/EDS解析を実施した。  
不良は\emph{ポリゲート端部〜サイドウォール近傍のTiSi$_2$領域}に局在していた。  
高分解能TEMではC49相残留を示唆する結晶コントラストが観測され，EDS分析ではTiリッチ領域に微量のBが検出された。  
さらにナノプロービングによる局所抵抗測定で，良品対比数倍のシート抵抗上昇が確認された。
...
\subsection{再現実験}
RTAランプ条件を変更して相転移の進行度を意図的に制御した実験では，ソーク時間短縮により不良率が増大し，逆にサイドウォールエッチアンダーを深めた試作ロットでは改善が確認された。  
これらは，相転移の完全化とhalo領域からの距離確保の双方が歩留り改善に寄与することを裏付けている。
...
\subsection{暫定対策}
量産ラインの即応性を重視し，プロセスフローの大規模改変を伴わない方策として，サイドウォールエッチ工程における\emph{エッチアンダー量を増加}させた。  
これにより，halo拡散によるB濃度ピークとTiSi$_2$形成前線との距離を確保し，C49相残留を抑制することを狙った。  
実験ロットにおいては不良発生率の低下が確認され，\emph{歩留り改善に一定の効果}が得られた。  
一方で，トランジスタ寸法のばらつき増大など二次的影響も懸念されるため，恒久的な解決策とは位置づけられなかった。
