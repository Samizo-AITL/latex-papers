...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[box, above=14mm of cof] (bp) {Reservoir \& Back Pressure\\($\approx -2.0$\,kPa) + PI damper};
  ...
  \caption{System architecture of the proposed Bio-Inkjet (Bio-IJ). A bulk KNN actuator, COF high-voltage driver, silicon cavity/nozzles, and fluidics（back-pressure + PI damper）を統合。}
  \label{fig:block}
  % \vspace{-1mm} % 必要なら有効化（詰めたい場合のみ）
\end{figure*}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    % 説明
    \node[align=left, left=0mm of act, xshift=-3mm] (lefttxt) {Back pressure $\approx -2.0$\,kPa\\PI membrane damper\\Supply};
