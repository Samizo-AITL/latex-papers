...
% --- Fig.2: Cross section (single-column width, no overlap) ---
\begin{figure}[!t]
  \centering
  \begin{tikzpicture}[
    ...
    % ── 上部説明（左右に固定幅／中央で重ならない） ──
    \node[align=left, anchor=north west, text width=.44\linewidth, inner sep=0pt]
      at ([yshift=2mm]hs.north west)
      {Back pressure $\approx -2.0$\,kPa;\\ PI membrane damper; Supply};

    \node[align=right, anchor=north east, text width=.44\linewidth, inner sep=0pt]
      at ([yshift=2mm]hs.north east)
