...
\begin{figure}[!t]
  \centering
  \resizebox{\columnwidth}{!}{%
    \begin{tikzpicture}[node distance=10mm, every node/.style={draw,rounded corners,minimum width=20mm,minimum height=6mm,align=center},>=stealth,transform shape]
      \node (sram) {SRAM};
      \node (ctrl) [right=of sram] {Backup Ctrl};
      \node (fefet) [right=of ctrl] {FeFET NVM};
      \draw[->] (sram) -- node[above]{Backup} (ctrl);
      \draw[->] (ctrl) -- node[above]{Restore} (fefet);
    \end{tikzpicture}}
