...
% ========= 体裁・間隔（少しだけ追加で詰める） =========
\setlength{\textfloatsep}{8pt plus 1pt minus 2pt}  % 図と本文の間
\setlength{\floatsep}{6pt plus 1pt minus 2pt}
\setlength{\intextsep}{5pt plus 1pt minus 2pt}
\setlength{\abovecaptionskip}{2pt}                 % キャプション上
\setlength{\belowcaptionskip}{0pt}                 % キャプション下を詰め

% タイトル直後の余白を詰める（IEEEtran推奨の軽微調整）
\IEEEaftertitletext{\vspace{-1.2\baselineskip}}

% 数式上下もほんの少しだけ詰める（読みやすさを損ねない範囲）
...
\begin{figure}[!t]
  ...
  \resizebox{0.88\linewidth}{!}{%
    \begin{tikzpicture}[node distance=8mm,
      ...
      \node (pump)  [right=28mm of fefet] {Charge Pump $\pm$2.5V};
      \node (io)    [above=of pump] {I/O \& AMS (3.3V)};
      \node (stat)  [below=of pump] {Status/Ctrl};
      \draw[->] (logic) -- (sram);
      \draw[->] (sram) -- node[left, xshift=-1pt]{backup} (fefet);
      \draw[->] (fefet) -- (pump);
      \draw[->] (pump)  -- (io);
      \draw[->] (pump)  -- (stat);
    \end{tikzpicture}}
...
\begin{figure}[!b]
  ...
  \resizebox{0.82\linewidth}{!}{%
    \begin{tikzpicture}[
      node distance=9mm,
      box/.style={draw,rounded corners,minimum width=19mm,minimum height=6mm,align=center}]
      \node[box] (sram) {SRAM};
      \node[box, right=of sram] (ctrl) {Backup Ctrl};
      \node[box, right=of ctrl] (nvm)  {FeFET NVM};
