...
\usepackage{tikz}
\usetikzlibrary{positioning,arrows.meta} % ← 矢印エラー対策で追加
\usepackage{booktabs}
\usepackage{cite}

% TikZ 矢印 tip 既定（Latex 矢印を安定使用）
\tikzset{>={Latex[length=2mm,width=2mm]}}
...
\begin{figure}[!t]
  ...
  \begin{tikzpicture}[node distance=6mm,
      every node/.style={draw,rectangle,minimum width=25mm,minimum height=6mm},>=stealth]
    \node (a) {FEOL (Co RTA)};
...
% ==== Fig.5: Backup/restore flow（単カラム／はみ出し防止）====
\begin{figure}[!t]
  \centering
  ...
  \begin{tikzpicture}[
    ...
    \draw[->] (sram) -- node[above,yshift=0.8mm]{Backup} (ctrl);
    \draw[->] (ctrl) -- node[above,yshift=0.8mm]{Restore} (nvm);
  \end{tikzpicture}}
