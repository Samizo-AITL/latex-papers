...
Write/erase pulses ($\pm$2.3--2.7~V, 1--50~\si{\micro\second}) are generated by an on-chip charge pump.
...
\begin{figure}[!t]
  ...
  \resizebox{0.95\columnwidth}{!}{%
    \begin{tikzpicture}[node distance=8mm, every node/.style={draw,rounded corners,minimum width=20mm,minimum height=6mm,align=center},>=Stealth,transform shape]
      ...
      \draw[->] (logic) -- node[right]{bus} (sram);
      \draw[->] (sram) -- node[right]{backup} (fefet);
      \draw[->] (fefet) -- node[above]{$\pm$V\textsubscript{P/E}} (pump);
      \draw[->] (pump) -- node[right]{status/ctrl} (io);
    \end{tikzpicture}}
