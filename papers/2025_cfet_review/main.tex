...
% nMOS block
\draw[fill=blue!20] (0,0) rectangle (3,0.6);
\node at (1.5,0.3) {\scriptsize nFET tier};

% pMOS block
\draw[fill=red!20] (0,2.0) rectangle (3,2.6);
\node at (1.5,2.3) {\scriptsize pFET tier};

% Coupling capacitance
\draw[<->,thick] (1.5,0.6) -- (1.5,2.0);
\node[anchor=west] at (1.6,1.3) {\scriptsize $C_{np}$};

% Thermal resistance
\draw[->,thick,orange] (3.2,0.6) -- (3.2,2.0);
\node[anchor=west,orange] at (3.3,1.3) {\scriptsize $R_{th}$};

% Stress arrow
\draw[<->,thick,dashed,gray] (-0.3,0.6) -- (-0.3,2.0);
\node[anchor=east,gray] at (-0.4,1.3) {\scriptsize Stress $\sigma$};

% Labels for terminals
\node[anchor=east] at (-0.2,2.3) {\scriptsize VDD};
\node[anchor=east] at (-0.2,0.3) {\scriptsize GND};

\end{tikzpicture}}
\caption{Conceptual BSIM-CFET compact model extension:
stacked nFET/pFET with inter-tier coupling capacitance ($C_{np}$),
thermal resistance ($R_{th}$), and stress-aware mobility correction ($\sigma$).}
\label{fig:bsim_cfet}
\end{figure}
