...
\begin{figure}[t]
  ...
  \node[flowstep, below=6mm of spec] (ctrl) {② 制御系設計\\MATLAB/Simulink（PID, FSM）\\LLM整合 $\to$ C $\to$ Verilog};
  \node[flowstep, below=6mm of ctrl] (fpga) {③ 回路設計（FPGA）\\RTL/SDC・固定小数点・I/O定義};
  \node[flowstep, below=6mm of fpga] (asic) {④ 回路設計（ASIC）\\PDK適用／合成・P\&R・CTS};
  ...
  \node[flowstep, below=6mm of fem] (fpga_test) {⑦ FPGA検証\\実時間応答・安定性評価};
