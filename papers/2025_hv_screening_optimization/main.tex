...
% ==== XeLaTeX 必須（Actionsでは latexmk_use_xelatex:true に）====
\usepackage{iftex}
\ifXeTeX\else
...
% GitHub Actions で apt: fonts-noto-cjk を入れる前提
\setCJKmainfont{Noto Serif CJK JP}
\setCJKsansfont{Noto Sans CJK JP}
\setCJKmonofont{Noto Sans Mono CJK JP}

% ---- 図・数式・表 ----
\usepackage{graphicx}                % dvipdfmx オプションは付けない（XeLaTeX）
\graphicspath{{./}{figures/}}        % 図の既定パス
\DeclareGraphicsExtensions{.pdf,.png,.jpg}
\usepackage{amsmath,amssymb,bm}
\usepackage{booktabs}
\usepackage{multirow}

% ---- URL/ハイパーリンク（日本語・長URLの改行に対応）----
\usepackage{url}
