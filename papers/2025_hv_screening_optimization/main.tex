...
\begin{figure}[t]
  \centering
  \begin{tikzpicture}[x=1mm,y=1.5mm]
    % --- Poly ゲート電極 ---
    \draw[thick] (0,24) -- (70,24)
      node[right,font=\scriptsize]{Poly ゲート電極};

    % --- HV ゲート酸化膜 ---
    \draw[thick] (0,18) -- (70,18)
      node[right,font=\scriptsize]{HV ゲート酸化膜};

    % --- Si 基板 ---
    \draw[thick] (0,10) -- (70,10)
      node[right,font=\scriptsize]{Si 基板};

    % --- COP：菱形（内部ボイド） ---
    \draw[thick] (30,10) -- (31,12) -- (32,10) -- (31,8) -- cycle;

    % --- ラベル（矢印は最小限） ---
    \draw[->,thin] (46,16) -- (31,11.9);
    \node[right,font=\scriptsize] at (46,16) {COP（析出＋ボイド）};

    \draw[->,thin] (24,15) -- (31,12);
    \node[left,font=\scriptsize] at (24,15) {界面接触部};
  \end{tikzpicture}
  \caption{MOSキャパ構造におけるHVゲート酸化膜とCOP欠陥（模式図）。COPはSi結晶中の酸素析出起点に形成される菱形ボイドであり、界面に接触すると局所電界集中を生じる。}
  \label{fig:moscap_cop_diamond}
\end{figure}

\subsection*{発生要因（ウエハ供給切替の経緯）}

本品質問題の直接要因は、ウエハ供給元の一時的切替に起因する。
従来、uTEPヘッド用ドライバICの量産では
\textbf{SUMCO製CZウエハ}を継続使用しており、長期実績を有していた。
しかし、当該期間中にSUMCO側の在庫が一時的に枯渇し、
代替として\textbf{信越化学工業製CZウエハ}を使用した。

業界的には信越製ウエハはSUMCOと同等、あるいはそれ以上の信頼性を有するとされ、
工程担当者は材料置換による品質リスクは低いと判断した。
しかし解析の結果、信越製ウエハでは酸素濃度分布がわずかに高く、
局所的な酸素析出およびCOP密度上昇が確認された。
これがHVゲート酸化膜下部での電界集中を誘発し、
完全な故障には至らないものの、
\textbf{通電応力下で不完全な異常を内在したデバイス}が量産工程に混入した。

本来、\textbf{使用実績のないウエハを採用する場合は}、
ウエハ購入仕様書に基づく実績ウエハとの差異評価および適用可否判定を
品質保証ガイドラインに従って実施すべきであった。
しかし緊急の供給対応により、この手順が簡略化され、
結果として材料起因の信頼性リスクを見逃したことが判明した。

この事例は、名目上の仕様値が一致していても、
結晶成長履歴や酸素アニール条件の差が
デバイス信頼性に直接影響することを示す重要な教訓である。

\subsection*{欠陥の性質と発現形態}

本事象で観測された異常は、従来のゲート酸化膜リークのように
明確な短絡や初期破壊として顕在化するものではなく、
\textbf{通電応力下で局所的に電界分布が不均一化した「不完全な異常」を内在するタイプ}であった。
すなわち、製品検査時点では動作・リークともに規格内であるが、
高電圧印加時に一部トランジスタで
電流-電圧特性の非線形挙動や閾値変動が再現性をもって観測された。

解析の結果、この挙動はHVゲート酸化膜下部に残存する
微小なCOP由来空隙（ボイド）が局所的な電界集中を生じ、
トンネル経路の形成を部分的に助長することで発生することが分かった。
このため、初期には“完全なリーク不良”ではなく、
\textbf{部分的な劣化点を潜在的に含んだデバイス}として量産工程を通過していた。

従来のスクリーニング条件では、こうした中間的劣化状態の検出感度が不足しており、
一部デバイスが組立後やプリンタ最終検査で異常波形を呈することで初めて顕在化した。
本研究で導入した複数回印加方式は、
この潜在欠陥を早期に顕在化させることを目的として設計されたものである。
