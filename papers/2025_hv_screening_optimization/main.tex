...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=1.0]
    ...
    % --- 層構造 ---
    \draw[thick] (-2.5, 0) -- (2.5, 0) node[right, font=\scriptsize]{Polyゲート電極};
    \draw[thick] (-2.5, -0.6) -- (2.5, -0.6) node[right, font=\scriptsize]{HVゲート酸化膜};
    \draw[thick] (-2.5, -1.2) -- (2.5, -1.2) node[right, font=\scriptsize]{Si基板};

    % --- COP欠陥（くぼみ形状） ---
    \draw[thick] (-0.4, -1.2) .. controls (-0.2, -0.9) and (0.2, -0.9) .. (0.4, -1.2);
    \node[right, font=\scriptsize] at (0.45, -1.0) {COP（析出＋ボイド）};

    % --- 電界線（模式） ---
    \draw[->, very thin] (-0.2, -0.6) -- (-0.2, -1.0);
    \draw[->, very thin] (0.0, -0.6) -- (0.0, -1.05);
    \draw[->, very thin] (0.2, -0.6) -- (0.2, -1.1);
    \node[left, font=\scriptsize] at (-0.8, -0.9) {局所電界集中};

  \end{tikzpicture}
  \caption{MOSキャパ構造におけるHVゲート酸化膜とCOP欠陥（模式図）。
COPが酸化膜界面に接すると、局所的な電界集中が生じる。}
  \label{fig_cop_crosssection}
\end{figure}
