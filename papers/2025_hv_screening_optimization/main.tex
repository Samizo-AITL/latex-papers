...
\usepgfplotslibrary{groupplots} % ← 必須：\begin{groupplot} 用
...
% '_' を本文・キャプションで安全化（必ず hyperref より前）
...
\begin{figure}[t]
  \centering
  \begin{tikzpicture}
    % 2枚並列（左右）のグループプロット。列幅に収まるよう各軸幅を調整
    \begin{groupplot}[
      group style={group size=2 by 1, horizontal sep=10mm},
      width=0.45\columnwidth, height=0.45\columnwidth, % ← 列幅ベースで安定
      ymin=0,
      ymajorgrids,
      xmajorgrids,
      tick align=outside,
      ...
      enlarge x limits=0.05,   % 端の棒が切れないように少し余白
    ]
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      width=\columnwidth,            % ← 列幅にフィット
      height=0.58\columnwidth,       % 比率で高さ調整
      xmin=1, xmax=8.02,
      ymin=0, ymax=500,
      ...
      line width=0.7pt,
      mark size=2.2pt,
      enlargelimits=false,            % 余白の自動拡張を抑止
      clip=false,                     % マーカー端の切れを防止
      legend style={font=\scriptsize,draw=none,fill=white,fill opacity=0.8},
      legend cell align=left,
      tick label style={font=\scriptsize},
      label style={font=\scriptsize},
