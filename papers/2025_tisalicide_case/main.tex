...
\begin{figure*}[!t]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
    ...
    \begin{scope}[shift={(0,0)}]
      ...
      % halo: SW直下、大きめ。右端がわずかにTiSiへ接触
      \draw[halo] (0.95,-0.22) ellipse (0.70 and 0.22);
      \node[label,blue!60] at (0.95,-0.50) {halo};
      % 残渣：接触点付近
      \draw[residual] (1.62,0.02) circle (0.10);
      \node[label,red!70] at (2.10,0.20) {C49+B residual};
    \end{scope}

    % ===================== (b) AFTER (非接触) =====================
    \begin{scope}[shift={(8,0)}]
      \node[font=\small] at (3,2.0) {(b) After: SW under-etch, no contact};
      \path[si] (0,-1.0) rectangle (6,0); \draw[gray!60,thick] (0,0)--(6,0);
      \path[fill=gray!50] (0,0) rectangle (1.2,1.6); \node[label] at (0.6,1.8) {SW};
      \path[silicide] (1.2,-0.05) rectangle (6,0.10); \node[label] at (5.0,0.25) {TiSi};
      % halo: SW直下（非接触になるよう左寄せ＆少し小さめ）
      % 中心 x=0.75, 半径 rx=0.45 -> 右端 x=1.20（TiSi開始ライン）より 0.00〜0.05 左でクリア
      \draw[halo] (0.75,-0.20) ellipse (0.45 and 0.20);
      \node[label,blue!60] at (0.75,-0.48) {halo};
      % 非接触を明示（必要なら小さな隙間矢印を追加可：ここでは省略）
      \node[label,green!50!black] at (2.2,-0.30) {no C49+B residual (no contact)};
    \end{scope}
  \end{tikzpicture}}
  \caption{Cross-sections near a sidewall (SW). (a) Oblique post-poly halo lies under SW and its right edge barely meets the TiSi front, where a C49+B residual can form. (b) With SW under-etch geometry, the halo remains under SW but \emph{does not contact} the TiSi front, eliminating the residual.}
