...
% ===== Improved Fig.2: Cross-sections before/after sidewall under-etch =====
\begin{figure}[!t]
  \centering
  \begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
    ...
    % ---------- (a) BEFORE: no under-etch, halo touches Ti during silicidation ----------
    \begin{scope}[shift={(0,0)}]
      % Title
      \node[font=\small] at (2.4,3.1) {(a) Before: halo–Ti contact (C49 residuals)};

      % STI oxide trench at left
      \path[oxide] (0,0) rectangle (1.2,2.4);  % STI
      \node[label,rotate=90] at (0.15,1.2) {STI Oxide};

      % Silicon substrate + active
      \path[si] (0,-1.6) rectangle (5,0);      % bulk Si
      \node[label] at (3.8,-1.3) {Si Substrate};

      % Active region (right of trench) top surface at y=0
      % Halo implant under sidewall (touching top surface region)
      \draw[halo] (1.15,-0.05) ellipse (0.6 and 0.28); % shallow part
      \draw[halo] (1.35,-0.35) ellipse (0.9 and 0.42); % deeper spread
      \node[label,blue!60] at (1.55,0.35) {Halo B};

      % Ti blanket (pre-silicidation) / silicide over active
      \path[metal] (0,2.4) rectangle (5,2.7); \node[label] at (4.4,2.95) {Ti layer};
      % Silicide on Si (active) forms but impeded near sidewall
      \path[silicide] (1.2,-0.02) rectangle (5,0.10);
      \node[label] at (3.6,0.28) {TiSi$_2$ (target C54)};

      % C49 residual grains near contact region
      \foreach \x/\y in {1.35/0.05,1.55/0.02,1.75/0.00}
        \draw[residual] (\x,\y) circle (0.045);
      \node[label,red!70] at (2.15,0.35) {C49 residual (high-$\rho$)};

      % Callouts
      \draw[->,label] (1.00,1.9) -- (1.18,0.05) node[pos=0.0,above] {};
      \node[label] at (0.95,2.05) {Sidewall corner};
      \draw[->,label] (2.0,2.0) -- (1.4,-0.35);
      \node[label] at (2.25,2.15) {Halo diffusion};

      % Ground line
      \draw[gray!60] (0,-1.6) -- (5,-1.6);
    \end{scope}

    % ---------- (b) AFTER: with sidewall under-etch (recess), separation increased ----------
    \begin{scope}[shift={(6.4,0)}]
      % Title
      \node[font=\small] at (2.4,3.1) {(b) After: sidewall under-etch (separation)};

      % STI oxide with intentional under-etch notch at the corner
      \path[oxide] (0,0) -- (1.2,0) -- (1.2,2.4) -- (0,2.4) -- cycle;
      % create recess notch (undercut) at top inner corner
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Cross-sections near the STI sidewall. (a) Without countermeasure: halo diffusion reaches Ti during silicidation, leaving C49 residual grains. (b) With sidewall under-etch: a recess increases the spacing between halo implant and silicide front, suppressing C49 residuals and stabilizing yield.}
  \label{fig:crosssection_improved}
\end{figure}
