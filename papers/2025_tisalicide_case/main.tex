...
% ===== Fig.2: poly + SW cross-sections (final tuned) =====
\begin{figure*}[!t]
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
    ...
    \begin{scope}[shift={(8,0)}]
      ...
      % SW (slightly wider; under-etch profile at bottom by raising foot line ONLY — no white patch)
      \path[fill=gray!55] (0.60,0.20) -- (1.10,0.20) -- (1.10,1.20) -- (0.60,1.20) -- cycle;
      \node[label,anchor=south] at (0.85,1.20) {SW};

      % TiSi on poly & diffusion (start x = 1.10 here)
      \path[silicide] (0,1.20) rectangle (0.60,1.35);
      \path[silicide] (1.10,-0.05) rectangle (6,0.10);
      \node[label] at (5.0,0.25) {TiSi};

      % Halo under SW, NON-CONTACT: right edge < 1.10
      % center=(0.70,-0.25), rx=0.50 -> right edge = 1.20? adjust to 0.48 so 0.70+0.48=1.18 < 1.10? we want non-contact:
      % Use rx=0.38 -> right edge=1.08 < 1.10
      \draw[halo] (0.70,-0.25) ellipse (0.38 and 0.25);
      \node[label,blue!70] at (0.70,-0.56) {halo};

      \node[label,green!50!black] at (2.20,-0.30) {no C49+B residual (no contact)};
    \end{scope}
  \end{tikzpicture}}
  \caption{Cross-sections near poly and sidewall (SW). (a) Post-poly oblique halo lies under SW and its right edge barely meets the diffusion TiSi, creating a C49+B high-R spot. (b) With SW under-etch geometry (foot raised), halo stays under SW but does \emph{not} contact TiSi, eliminating the defect.}
  \label{fig:fig2_poly}
\end{figure*}
