...
\begin{figure*}[!t]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
    ...
    \begin{scope}[shift={(0,0)}]
      ...
      % sidewall (SW, aligned to substrate left edge)
      \path[fill=gray!50] (0,0) rectangle (0.5,1.6);
      \node[label] at (0.25,1.8) {SW};

      % TiSi silicide
      \path[silicide] (0.5,-0.05) rectangle (6,0.10);
      \node[label] at (5.0,0.25) {TiSi};

      % halo (to the right of SW)
      \draw[halo] (0.9,-0.05) ellipse (0.55 and 0.25);
      \node[label,blue!60] at (1.7,0.3) {halo};

      % C49 + B residual (at contact point)
      \draw[residual] (0.5,0.0) circle (0.10);
      \node[label,red!70] at (1.5,-0.3) {C49+B residual};
    \end{scope}
    ...
    \begin{scope}[shift={(8,0)}]
      ...
      % sidewall (SW, aligned same as (a))
      \path[fill=gray!50] (0,0) rectangle (0.5,1.6);
      \node[label] at (0.25,1.8) {SW};

      % foot recess (etch-back at bottom corner)
      \fill[white] (0,0) -- (0.2,0) -- (0.2,0.15) -- (0,0.15) -- cycle;

      % TiSi silicide
      \path[silicide] (0.5,-0.05) rectangle (6,0.10);
      \node[label] at (5.0,0.25) {TiSi};

      % halo (shifted left, away from TiSi)
      \draw[halo] (0.2,-0.05) ellipse (0.55 and 0.25);
      \node[label,blue!60] at (1.1,0.3) {halo};

      % distance arrow
      \draw[<->,thick,gray!70] (0.5,0.15) -- (0.75,0.15);
      \node[label] at (0.65,0.35) {separation};

      % no residual
      \node[label,green!50!black] at (2.0,-0.3) {no C49+B residual};
    \end{scope}
  \end{tikzpicture}}
