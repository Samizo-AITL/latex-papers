...
\begin{figure*}[!t]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
    ...
    \tikzstyle{silicide}=[fill=orange!35,draw=orange!80!black]
    ...
    \begin{scope}[shift={(0,0)}]
      \node[font=\small] at (3,2.0) {(a) Before: C49+B residual at SW corner};

      % Si substrate and surface
      \path[si] (0,-1.0) rectangle (6,0);
      \draw[gray!60,thick] (0,0) -- (6,0);

      % SW (left edge aligned to substrate left), widened to right
      \path[fill=gray!50] (0,0) rectangle (1.2,1.6);
      \node[label] at (0.6,1.8) {SW};

      % TiSi (silicide) on surface
      \path[silicide] (1.2,-0.05) rectangle (6,0.10);
      \node[label] at (5.0,0.25) {TiSi};

      % halo entirely inside Si-sub (top just touches y=0)
      \draw[halo] (1.60,-0.20) ellipse (0.55 and 0.20);
      \node[label,blue!60] at (2.40,0.30) {halo};

      % C49+B residual at corner
      \draw[residual] (1.20,0.00) circle (0.10);
      \node[label,red!70] at (2.20,-0.30) {C49+B residual};
    \end{scope}

    % ===================== (b) AFTER =====================
    \begin{scope}[shift={(8,0)}]
      \node[font=\small] at (3,2.0) {(b) After: SW etch-back, separation secured};

      % Si substrate and surface
      \path[si] (0,-1.0) rectangle (6,0);
      \draw[gray!60,thick] (0,0) -- (6,0);

      % SW (same width as a), with foot recess
      \path[fill=gray!50] (0,0) rectangle (1.2,1.6);
      \node[label] at (0.6,1.8) {SW};
      \fill[white] (0,0) -- (0.40,0) -- (0.40,0.20) -- (0,0.20) -- cycle;

      % TiSi (silicide)
      \path[silicide] (1.2,-0.05) rectangle (6,0.10);
      \node[label] at (5.0,0.25) {TiSi};

      % halo entirely under SW (inside Si-sub)
      \draw[halo] (0.60,-0.20) ellipse (0.55 and 0.20);
      \node[label,blue!60] at (1.40,0.30) {halo};

      % separation arrow (kept below surface)
      \draw[<->,thick,gray!70] (1.20,0.12) -- (1.60,0.12);
      \node[label] at (1.40,0.32) {separation};

      % no residual
      \node[label,green!50!black] at (2.20,-0.30) {no C49+B residual};
    \end{scope}
  \end{tikzpicture}}
  \caption{Cross-sections near a sidewall (SW) based on the sketch.
  (a) Halo resides entirely within the Si substrate and touches the surface at the SW corner, forming a C49+B residual.
  (b) After sidewall deposition and etch-back (foot recess), the halo remains inside Si and under the SW, securing distance to the TiSi front and eliminating the C49+B residual.}
