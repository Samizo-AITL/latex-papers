...
\begin{figure*}[!t]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
    ...
    \begin{scope}[shift={(0,0)}]
      ...
      % Si substrate (left edge at x=0) + surface
      \path[si] (0,-1.0) rectangle (6,0); \draw[gray!60,thick] (0,0)--(6,0);

      % Poly gate (LEFT EDGE ALIGNED TO SUBSTRATE LEFT)
      \path[poly] (0,0) rectangle (0.6,1.2); \node[label] at (0.30,1.35) {poly};
      % Sidewall (to the right of poly)
      \path[fill=gray!55] (0.6,0) rectangle (1.0,1.2); \node[label] at (0.80,1.35) {SW};

      % Silicide on poly top and on diffusion surface
      \path[silicide] (0,1.2) rectangle (0.6,1.35);
      \path[silicide] (1.0,-0.05) rectangle (6,0.10); \node[label] at (5.0,0.25) {TiSi};

      % Halo: under poly/SW, 右端がTiSiにわずか接触
      \draw[halo] (0.75,-0.25) ellipse (0.65 and 0.25);
      \node[label,blue!70] at (0.75,-0.55) {halo};

      % 高抵抗スポット（接触点付近）
      \draw[residual] (1.00,0.02) circle (0.10);
      \node[label,red!70] at (1.55,0.22) {high-R spot (C49+B)};
    \end{scope}
    ...
    \begin{scope}[shift={(8,0)}]
      ...
      % Si substrate + surface（同じ左基準）
      \path[si] (0,-1.0) rectangle (6,0); \draw[gray!60,thick] (0,0)--(6,0);

      % Poly gate（左端合わせ）
      \path[poly] (0,0) rectangle (0.6,1.2); \node[label] at (0.30,1.35) {poly};
      % SW：幅を若干広げ（右へ）、下の白四角は無し
      \path[fill=gray!55] (0.6,0.20) -- (1.10,0.20) -- (1.10,1.2) -- (0.6,1.2) -- cycle;
      \node[label] at (0.85,1.35) {SW};

      % Silicide
      \path[silicide] (0,1.2) rectangle (0.6,1.35);
      \path[silicide] (1.10,-0.05) rectangle (6,0.10); \node[label] at (5.0,0.25) {TiSi};

      % Halo：SW直下・非接触（右端 < TiSi開始）
      \draw[halo] (0.70,-0.25) ellipse (0.50 and 0.25);
      \node[label,blue!70] at (0.70,-0.55) {halo};
