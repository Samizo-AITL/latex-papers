...
% ===== Fig.2 (two-column): Cross-sections before / after sidewall dep.+etch-back =====
\begin{figure*}[!t]
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
    ...
    % ===========================================================
    % (a) BEFORE : no countermeasure, halo reaches surface -> C49 residuals
    % ===========================================================
    \begin{scope}[shift={(0,0)}]
      \node[font=\small] at (2.6,3.0) {(a) Before: halo--surface proximity (C49 residuals)};

      % STI oxide trench (left)
      \path[oxide] (0,0) rectangle (1.2,2.4);

      % Silicon substrate
      \path[si] (0,-1.6) rectangle (5,0);
      \node[label] at (3.6,-1.25) {Si Substrate};

      % Silicide (target C54) thin layer on active
      \path[silicide] (1.2,-0.02) rectangle (5,0.10);
      \node[label] at (3.6,0.28) {TiSi$_2$ (target C54)};

      % Halo diffusion near sidewall (contacts silicidation region)
      \draw[halo] (1.15,-0.05) ellipse (0.60 and 0.28);
      \draw[halo] (1.35,-0.35) ellipse (0.95 and 0.42);
      \node[label,blue!60] at (1.55,0.40) {Halo B};

      % Residual C49 grains near corner
      \foreach \x/\y in {1.35/0.05,1.55/0.02,1.74/0.00}
        \draw[residual] (\x,\y) circle (0.045);
      \node[label,red!70] at (2.10,0.36) {C49 residual (high-$\rho$)};

      % Callouts
      \draw[->,label] (2.15,2.0) -- (1.40,-0.34);
      \node[label] at (2.48,2.15) {halo diffusion};
      \draw[->,label] (0.90,2.0) -- (1.18,0.07);
      \node[label] at (0.90,2.15) {sidewall corner};

      \draw[gray!60] (0,-1.6) -- (5,-1.6);
    \end{scope}

    % ===========================================================
    % (b) AFTER : sidewall deposition + anisotropic etch-back (foot recess)
    % ===========================================================
    \begin{scope}[shift={(6.6,0)}]
      \node[font=\small] at (2.6,3.0) {(b) After: sidewall deposition + etch-back (foot recess)};

      % STI oxide trench (left)
      \path[oxide] (0,0) rectangle (1.2,2.4);

      % Sidewall liner (conformal dep.) and foot recess (after etch-back)
      \path[spacer] (1.10,0) -- (1.10,2.4) -- (1.00,2.4) -- (1.00,0) -- cycle; % liner
      \fill[spacer] (1.00,0) -- (1.00,0.18)
        .. controls (0.94,0.14) and (0.90,0.10) .. (0.86,0.08) -- (0.86,0) -- cycle; % foot recess
      \node[label,green!50!black] at (1.02,0.36) {liner + foot recess};

      % Silicon substrate
      \path[si] (0,-1.6) rectangle (5,0);
      \node[label] at (3.6,-1.25) {Si Substrate};

      % Silicide (C54 completed)
      \path[silicide] (1.2,-0.02) rectangle (5,0.10);
      \node[label] at (3.6,0.28) {TiSi$_2$ (C54 completed)};

      % Halo now farther from edge
      \draw[halo] (1.55,-0.18) ellipse (0.65 and 0.30);
      \draw[halo] (1.80,-0.46) ellipse (1.00 and 0.46);
      \node[label,blue!60] at (2.05,0.40) {Halo B};

      % No residuals; show increased lateral spacing
      \node[label,green!60!black] at (1.58,0.28) {no C49 residual};
      \draw[<->,thick,gray!70] (1.02,0.10) -- (1.55,0.10);
      \node[label] at (1.285,0.32) {increased lateral separation};

      \draw[gray!60] (0,-1.6) -- (5,-1.6);
    \end{scope}
  \end{tikzpicture}}
  \caption{Two-column schematic cross-sections near an STI sidewall. 
  (a) Without countermeasure, halo diffusion approaches the silicidation region and leaves C49 residual grains (high resistivity). 
  (b) Sidewall \emph{deposition + anisotropic etch-back} forms a liner and a slight foot recess, increasing lateral spacing between the halo implant extension and the silicide front; C49 residuals are suppressed and yield is stabilized.}
