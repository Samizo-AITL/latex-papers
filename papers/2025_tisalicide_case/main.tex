...
% ===== Fig.2 (two-column): Cross-sections before vs. sidewall deposition + etch-back =====
\begin{figure*}[!t]
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
    ...
    \begin{scope}[shift={(0,0)}]
      \node[font=\small] at (2.6,3.2) {(a) Before: halo--Ti contact (C49 residuals)};
      % STI oxide trench (left)
      \path[oxide] (0,0) rectangle (1.2,2.6);
      \node[label,rotate=90] at (0.15,1.3) {STI Oxide};

      % Si substrate
      \path[si] (0,-1.6) rectangle (5,0);
      \node[label] at (3.7,-1.3) {Si Substrate};
      ...
      % Ti blanket + silicide (target C54)
      \path[metal] (0,2.6) rectangle (5,2.9); \node[label] at (4.4,3.05) {Ti layer};
      \path[silicide] (1.2,-0.02) rectangle (5,0.10);
      \node[label] at (3.5,0.28) {TiSi$_2$ (target C54)};

      % Residual C49 grains
      \foreach \x/\y in {1.35/0.05,1.55/0.02,1.72/0.00}
        \draw[residual] (\x,\y) circle (0.045);
      \node[label,red!70] at (2.15,0.38) {C49 residual (high-$\rho$)};

      % Callouts
      \draw[->,label] (2.1,2.1) -- (1.4,-0.35);
      \node[label] at (2.45,2.25) {Halo diffusion};
      \draw[->,label] (0.9,2.1) -- (1.18,0.06);
      \node[label] at (0.9,2.25) {Sidewall corner};
    ...
    \begin{scope}[shift={(6.6,0)}]
      \node[font=\small] at (2.6,3.2) {(b) After: sidewall deposition + etch-back (foot recess)};

      % STI oxide trench
      \path[oxide] (0,0) rectangle (1.2,2.6);
      \node[label,rotate=90] at (0.15,1.3) {STI Oxide};

      % Conformal spacer (deposited) + anisotropic etch-back leaving foot recess
      % Sidewall liner
      \path[spacer] (1.10,0) -- (1.10,2.6) -- (1.00,2.6) -- (1.00,0) -- cycle;
      % Foot recess wedge at bottom after etch-back (slight under at foot)
      \fill[spacer] (1.00,0) -- (1.00,0.18) .. controls (0.94,0.14) and (0.90,0.10) .. (0.86,0.08)
                    -- (0.86,0) -- cycle;
      \node[label,green!50!black] at (0.96,0.36) {liner \& foot recess};

      % Si substrate
      \path[si] (0,-1.6) rectangle (5,0);
      \node[label] at (3.7,-1.3) {Si Substrate};

      % Halo now further from active edge due to spacer geometry
      \draw[halo] (1.55,-0.18) ellipse (0.65 and 0.30);
      \draw[halo] (1.80,-0.46) ellipse (1.00 and 0.46);
      \node[label,blue!60] at (2.05,0.40) {Halo B};

      % Ti blanket + silicide (C54 completed)
      \path[metal] (0,2.6) rectangle (5,2.9); \node[label] at (4.4,3.05) {Ti layer};
      \path[silicide] (1.2,-0.02) rectangle (5,0.10);
    ...
    \end{scope}
  \end{tikzpicture}}
  \caption{Two-column cross-sections near the STI sidewall. (a) Before: halo diffusion reaches the silicidation region, leaving C49 residual grains. (b) After: \emph{sidewall deposition + anisotropic etch-back} forms a liner and a slight foot recess, increasing the lateral spacing between the halo implant and the silicide front; C49 residuals are suppressed and yield is stabilized.}
