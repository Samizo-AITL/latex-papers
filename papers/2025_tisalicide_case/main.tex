...
In addition to isolation maturity, another key reason for stable yield
at the 0.18~µm STI node was the industry-wide transition from TiSi\textsubscript{2}
to CoSi\textsubscript{2} salicide.
Unlike TiSi\textsubscript{2}, which required a C49$\rightarrow$C54 phase transformation,
CoSi\textsubscript{2} formed directly in the stable low-resistivity phase
($\sim$15--20~µ$\Omega\cdot$cm).
This avoided random resistive spots caused by incomplete transformation,
providing improved process window and better compatibility with scaled junctions.
Therefore, while HV device risk limited adoption of 0.18~µm STI for driver ICs,
the baseline logic process itself exhibited higher yield stability.
