...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[x=1mm,y=1mm]
    % 色定義
    \definecolor{cofAu}{RGB}{244,170,88}   % COF: Au（橙）
    \definecolor{cofCu}{RGB}{170,120,80}   % COF: Cu（茶系）
    \colorlet{auMetal}{gray!60}
    \colorlet{teLayer}{gray!10}
    \colorlet{pztLayer}{gray!15}
    \colorlet{beLayer}{gray!30}

    % 実厚比（PZT 1.2µm ≒ Au 1.3µm がほぼ同じ高さに見えるよう調整）
    \def\scale{8.0} % 1 µm = 8 mm
    \pgfmathsetmacro{\auT}{1.3*\scale}
    \pgfmathsetmacro{\pztT}{1.2*\scale}

    % 共通パラメータ
    \def\gap{0.6}     % タイトル直下の余白
    \def\cofAuT{1.0}  % COF Au 図示厚
    \def\cofCuT{4.0}  % COF Cu 図示厚
    \def\teT{2.0}     % TE 図示厚
    \def\beT{2.0}     % BE 図示厚
    \def\W{62}        % 図幅
    \def\bx{10}       % 左島のX
    ...
    \begin{scope}[shift={(0,47)}]
      ...
      % --- 追加：Auマイグレーション（TE表面上の矢印） ---
      % TEの上面より少し上の高さに、左島→右島へ矢印を描く
      ...
      \node[font=\scriptsize,anchor=south,
            text=orange!80!black] at (0.5*(\bx+\bw+\bx+\pitch),\ymig+1.6)
            {Auマイグレーション（TE表面）};
    ...
    \begin{scope}[shift={(0,0)}]
      ...
      \fill[white] (\sxL,\yPZTbot) rectangle (\sxR,\yAUbot); % 中央スリット
  ...
  上：対策前（TE 連続・配線間にAuマイグレーションを矢印で表示）／
