...
本研究の合理化検討はCOF配線上Au薄化に主眼を置いたが、並行してアクチュエータチップ側の配線（PZT/TE上Au/NiCr配線）においても、
高湿・高電界条件で\textbf{マイグレーション短絡}が確認された。
根本要因は、（i）狭小配線間隔に起因する\textbf{電界集中}、（ii）Au/NiCr界面近傍での\textbf{空孔・金属イオン移動}の促進、（iii）局所的な\textbf{湿度・汚染物}トラップである。
そこで、\textbf{PZT基板に微細スリットを導入}し、隣接配線間の有効距離と漏れ経路長を増加、さらに表面の毛細水膜連結を分断した。

\medskip
\noindent
\textbf{評価手順（Evaluation flow）：}
(i) 従来配線で85/85および通電バイアスHASTにより短絡再現性を確認、
(ii) スリット付新版で同一マージンの再試験（$n\ge 30$）、
(iii) FIB-SEM／EDSで改修前後の劣化部位を比較し、起点の抑止を確認。

\medskip
\noindent
\textbf{結果（Result）：}
スリット導入後は\textit{従来条件で発生した短絡が再現せず}、抵抗ドリフトも$\le 1\%$に抑制された。
COF側Au薄化（下限 $\SI{0.30}{\micro\meter}$）と両立し、量産規格へ反映済みである。
本合理化評価を契機に、\textbf{製品全体の信頼性を底上げ}できた。

\begin{figure}[htbp]
  \centering
  % ===== TikZ schematic (Before / After) =====
  % 依存: \usepackage{tikz}, \usetikzlibrary{arrows.meta,patterns}
  \tikzset{
  ...
  \begin{tikzpicture}[x=1mm,y=1mm]
    ...
    \begin{scope}
      ...
      \node[lbl,anchor=west] at (2,26) {\textbf{(a) 従来構造 / Before}};
      \node[lbl,anchor=west] at (2,22) {PZT substrate (TE on PZT)};
      % Two electrodes (Au/NiCr) on top
      \draw[metal] (8,18) rectangle (32,22);   % Left electrode
      \draw[metal] (38,18) rectangle (62,22);  % Right electrode
      \node[lbl] at (20,15.5) {Au/NiCr};
      \node[lbl] at (50,15.5) {Au/NiCr};
      % Gap label
      \draw[<->,line width=0.4pt] (32,20) -- (38,20);
      \node[lbl] at (35,22.5) {狭小ギャップ / narrow gap};
      % Field concentration arrows
      \draw[arrow] (33.0,18) -- +(0,-8);
      \draw[arrow] (37.0,18) -- +(0,-8);
      \node[lbl] at (35,7) {電界集中 / field crowding};
      % Possible migration path (dashed)
      \draw[dashed,red!70] (32.5,18) .. controls (35,16) .. (37.5,18);
      \node[lbl,red!70] at (35,12.5) {migration / short risk};

      % Legend (no \square)
      \fill[metal] (2,2) rectangle +(2,2);      % small swatch
      \node[lbl,anchor=west] at (4.5,3) {Au/NiCr};
      \fill[pzt]   (18,2) rectangle +(2,2);     % small swatch
      \node[lbl,anchor=west] at (20.5,3) {PZT (schematic)};
    \end{scope}
    ...
    \begin{scope}[xshift=\W+12mm]
      % Substrate (PZT)
      \draw[pzt] (0,0) rectangle (\W,\H);
      \node[lbl,anchor=west] at (2,26) {\textbf{(b) スリット導入 / After (with slit)}};
      \node[lbl,anchor=west] at (2,22) {PZT with isolation slit};
      % Two electrodes (Au/NiCr)
      \draw[metal] (8,18) rectangle (32,22);   % Left electrode
      \draw[metal] (38,18) rectangle (62,22);  % Right electrode
      \node[lbl] at (20,15.5) {Au/NiCr};
      \node[lbl] at (50,15.5) {Au/NiCr};

      % Isolation slit in substrate
      \draw[slit] (34,0) rectangle (36,18);
      \node[lbl,rotate=90] at (35,9) {絶縁スリット / slit};

      % Gap arrow (same physical gap) but broken leakage path
      \draw[<->,line width=0.4pt] (32,20) -- (38,20);
      \node[lbl] at (35,22.5) {幾何学ギャップは同等};

      % Suppressed migration (no dashed arc)
      \node[lbl,green!50!black] at (35,12.5) {漏れ経路分断 / path broken};

      % Field arrows reduced
      \draw[arrow,green!50!black] (33.0,18) -- +(0,-6);
      \draw[arrow,green!50!black] (37.0,18) -- +(0,-6);
      \node[lbl,green!50!black] at (35,6) {局所電界低減};
    \end{scope}
  \end{tikzpicture}

  \caption{アクチュエータ配線改修（スリット導入）により、配線間の有効漏れ経路を分断し電界集中を緩和。%
  \emph{Actuator-side metallization improvement with substrate slit to break leakage paths and relax field crowding.}}
  \label{fig:appendix-slit}
\end{figure}

\medskip
\noindent
\textbf{設計ガイド（Design guide）：}
(1) 有効クリープ距離の確保（スリット・段差・疎水化の併用）、(2) 表面粗さ・残渣の管理、(3) Au/NiCr厚み比の見直し（NiCrを拡散・腐食バリアとして適正化）、(4) バイアス・湿度複合試験での定期モニタを推奨\cite{JIEP,JEITA}。
