...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[node distance=8mm and 10mm, >=Latex]
    ...
    \node[box, below=of cost] (impact) {事業効果（見込み）\\年間十数億円規模};
  ...
  \caption{Au厚仕様決定ロジック。
  現行仕様（0.5 µm ±0.20）から量産実績・工程能力・信頼性評価・
  コスト感応度を経て、新仕様（0.425 ±0.125 µm）に到達。
  コスト低減効果を市場出荷見込みに外挿し，事業効果を推定した。}
  \label{fig:au-logic}
\end{figure}
...
本稿の主な貢献は以下の3点である。
\begin{enumerate}
  \item COF外部端子におけるAuメッキ薄化を対象とし，
        量産実績を踏まえた合理的な新仕様
        （0.425$\pm$0.125\,µm）の設定プロセスを提示した。
  \item 下限0.30\,µmを維持したうえで，
        NPC接合信頼性，イオンマイグレーション，
        環境加速試験により実使用条件で十分な信頼性マージンを確認した。
  \item コスト感応度解析を行い，
        チップ当たり数円の合理化効果を基に，
        市場拡大を考慮した**年間十数億円規模の潜在的削減効果**を
        定量的に示し，設計合理化が事業価値創出に直結することを明確にした。
\end{enumerate}
...
%================ Figure: COF Au/Cu & Au migration before/after (TikZ) =========
\begin{figure}[t]
  \centering
  \resizebox{0.95\columnwidth}{!}{%
  \begin{tikzpicture}[x=1mm,y=1mm]
    % 色定義
    \definecolor{cofAu}{RGB}{244,170,88}
    \definecolor{cofCu}{RGB}{170,120,80}
    \colorlet{auMetal}{gray!60}
    \colorlet{teLayer}{gray!10}
    \colorlet{pztLayer}{gray!15}
    \colorlet{beLayer}{gray!30}

    % 実厚比（PZT 1.2µm ≒ Au 1.3µm）
    \def\scale{8.0}
    \pgfmathsetmacro{\auT}{1.3*\scale}
    \pgfmathsetmacro{\pztT}{1.2*\scale}
    ...
    \begin{scope}[shift={(0,47)}]
      \node[anchor=south west,font=\small] at (0,0)
        {対策前：TE 表面で Au マイグレーション発生リスク};

      % y位置
      \pgfmathsetmacro{\yTop}{-\gap}
      \pgfmathsetmacro{\yCOFbot}{\yTop-(\cofAuT+\cofCuT)}
      \pgfmathsetmacro{\yAUbot}{\yCOFbot-\auT}
      \pgfmathsetmacro{\yTEbot}{\yAUbot-\teT}
      \pgfmathsetmacro{\yPZTbot}{\yTEbot-\pztT}
      \pgfmathsetmacro{\yBEbot}{\yPZTbot-\beT}

      % 層
      \fill[beLayer]  (0,\yBEbot) rectangle (\W,\yPZTbot);
      \fill[pztLayer] (0,\yPZTbot) rectangle (\W,\yTEbot);
      \fill[teLayer]  (0,\yTEbot) rectangle (\W,\yAUbot); % TE 連続

      % Au島
      \fill[auMetal] (\bx,\yAUbot) rectangle (\bx+\bw,\yCOFbot);
      \fill[auMetal] (\bx+\pitch,\yAUbot) rectangle (\bx+\pitch+\bw,\yCOFbot);

      % COF（下：Au，上：Cu）
      \fill[cofAu] (\bx,\yCOFbot) rectangle (\bx+\bw,\yCOFbot+\cofAuT);
      \fill[cofAu] (\bx+\pitch,\yCOFbot) rectangle (\bx+\pitch+\bw,\yCOFbot+\cofAuT);
      \fill[cofCu] (\bx,\yCOFbot+\cofAuT) rectangle (\bx+\bw,\yTop);
      \fill[cofCu] (\bx+\pitch,\yCOFbot+\cofAuT) rectangle (\bx+\pitch+\bw,\yTop);

      % --- Auマイグレーション矢印（TE表面） ---
      \pgfmathsetmacro{\ymig}{\yAUbot+0.25*\cofAuT}
      \draw[very thick, -{Stealth[length=2.2mm]}, color=orange!80!black]
        (\bx+\bw+0.8,\ymig) .. controls (\bx+\bw+5,\ymig+1.2)
        and (\bx+\pitch-5,\ymig+1.2) .. (\bx+\pitch-0.8,\ymig);
      % ★ 事前に x 中点を計算してから使う（エラー回避）
      \pgfmathsetmacro{\xmigmid}{0.5*(\bx+\bw+\bx+\pitch)}
      \node[font=\scriptsize,anchor=south,text=orange!80!black]
        at (\xmigmid,\ymig+1.6) {Auマイグレーション（TE表面）};

      % ラベル
      \node[anchor=west,font=\scriptsize] at (\W+2,\yTop) {COF（Cu 8 µm／Au 0.425 µm）};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yAUbot+0.4*\auT) {Au（アクチュエータ）1.3 µm};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yTEbot+0.5*\teT) {TE 20 nm};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yPZTbot+0.5*\pztT) {PZT 1.2 µm};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yBEbot+0.5*\beT) {BE 100 nm};
    \end{scope}

    %-------------------------------- 下段：対策後 ------------------------------
    \begin{scope}[shift={(0,0)}]
      \node[anchor=south west,font=\small] at (0,0)
        {対策後：中央スリットで TE と PZT を切り、Au マイグレーションを抑制};
      ...
      \fill[beLayer] (0,\yBEbot) rectangle (\W,\yPZTbot);
      \fill[pztLayer] (0,\yPZTbot) rectangle (\W,\yTEbot);
      \fill[teLayer]  (0,\yTEbot) rectangle (\W,\yAUbot);
      \pgfmathsetmacro{\sxL}{0.5*\W-0.5*\slitW}
      \pgfmathsetmacro{\sxR}{0.5*\W+0.5*\slitW}
      \fill[white] (\sxL,\yPZTbot) rectangle (\sxR,\yAUbot); % スリット
      ...
      \node[anchor=west,font=\scriptsize] at (\W+2,\yTop) {COF（Cu 8 µm／Au 0.425 µm）};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yAUbot+0.4*\auT) {Au（アクチュエータ）1.3 µm};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yTEbot+0.5*\teT) {TE 20 nm（中央スリット）};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yPZTbot+0.5*\pztT) {PZT 1.2 µm（中央スリット）};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yBEbot+0.5*\beT) {BE 100 nm};
    \end{scope}
  \end{tikzpicture}%
  }
  \vspace{-1mm}
  \caption{COF 構造を Au/Cu の 2 層として示した Au マイグレーション模式図。
  上：対策前（TE 連続・配線間Auマイグレーションを矢印で表示）／
  下：対策後（TE と PZT に中央スリット導入で拡散経路を遮断）。}
