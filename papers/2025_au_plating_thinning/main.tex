...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[x=1mm,y=1mm,>=Latex,inner sep=1pt]
    ...
    %---------------- 共通パラメタ ----------------
    % 右側ラベルのX位置（図とかぶらないよう外側へ）
    \def\XR{75} % 右ラベルx（anchor=west）

    % ===== 上段：対策前 =====
    % タイトル（中央揃え）—— ここが基準
    \node[font=\small, align=center] (ttlBefore) at (40,94) {対策前：TE表面でAuマイグレーション発生リスク};

    % Cu上辺をタイトル直下（約1mm下）に合わせる
    \def\yCuTopA{93} % タイトル 94 → Cu上辺 93（=1mm下）
    % スタック寸法（上から下へ）
    \def\hCu{6.0} \def\hAu{2.7} \def\hAct{13.0} \def\hTE{2.0} \def\hPZT{12.0} \def\hBE{6.0}
    ...
    % COF端子（Au→Cu）
    \fill[orange!35] (20,\yAuTopA) rectangle (32,{\yAuTopA+\hAu});     % Au on top of Act
    \fill[brown!30]  (20,\yCuTopA) rectangle (32,{\yCuTopA+\hCu});     % Cu
    \fill[orange!35] (48,\yAuTopA) rectangle (60,{\yAuTopA+\hAu});
    \fill[brown!30]  (48,\yCuTopA) rectangle (60,{\yCuTopA+\hCu});

    % Au migration（右側に退避）
    \draw[->] (60,{\yTETopA+0.1}) -- (66,{\yTETopA+0.1});
    \node[font=\scriptsize, anchor=west] at (67,{\yTETopA+1.2}) {Au migration on TE};

    % 右側ラベル（層名・膜厚）— 図と重ならない右外
    \node[font=\scriptsize, anchor=west] at (\XR,{\yBETopA+1.5}) {BE 100\,nm};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yPZTTopA+6})  {PZT 1.2\,\textmu m};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yTETopA+1.2}) {TE 20\,nm};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yActTopA+11.8}) {Au 1.3\,\textmu m (Act)};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yCuTopA+3.2})  {Cu 8\,\textmu m};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yAuTopA+1.0})  {COF Au 0.425\,\textmu m};

    % ===== 下段：対策後（PZTは“抜かず”連続、スリット無し） =====
    % タイトル
    \node[font=\small, align=center] (ttlAfter) at (40,48) {対策後：沿面改質・電界緩和によるマイグレーション抑制};
    ...
    \pgfmathsetmacro{\yPZTTopB}{\yTETopB - \hPZT}
    \pgfmathsetmacro{\yBETopB}{\yPZTTopB - \hBE}

    % 下地（PZTは連続：スリット無し）
    \fill[gray!30] (8,\yBETopB) rectangle (72,{\yBETopB+\hBE});     % BE
    \fill[gray!15] (8,\yPZTTopB) rectangle (72,{\yPZTTopB+\hPZT});   % PZT (continuous)
    ...
    % 右側ラベル
    \node[font=\scriptsize, anchor=west] at (\XR,{\yBETopB+1.5}) {BE 100\,nm};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yPZTTopB+6})  {PZT 1.2\,\textmu m};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yTETopB+1.2}) {TE 20\,nm};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yActTopB+11.8}) {Au 1.3\,\textmu m (Act)};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yCuTopB+3.2})  {Cu 8\,\textmu m};
    \node[font=\scriptsize, anchor=west] at (\XR,{\yAuTopB+1.0})  {COF Au 0.425\,\textmu m};
  \end{tikzpicture}}
  \caption{COF端子（Au 0.425\,\textmu m／Cu 8\,\textmu m）とアクチュエータAu電極（1.3\,\textmu m）の界面模式図。上：対策前はTE表面でAuマイグレーションが起点となる。下：対策後は沿面電界緩和・界面改質により抑制。下地：NiCr 50\,nm／TE 20\,nm／PZT 1.2\,\textmu m／BE 100\,nm。}
