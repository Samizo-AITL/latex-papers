...
\begin{figure}[!t]
  ...
  \begin{tikzpicture}[x=1mm,y=1mm,>=Latex,inner sep=1pt]
    % 図全体の外形を固定（はみ出し防止）
    \path[use as bounding box] (0,0) rectangle (140,48);

    % ---------------- 左：対策前 ----------------
    \begin{scope}
      % 下地層
      \fill[gray!30] (0,0) rectangle (62,6);    % BE
      \fill[gray!15] (0,6) rectangle (62,18);   % PZT
      \fill[gray!10] (0,18) rectangle (62,20);  % TE
      \node[font=\scriptsize] at (31,1.6) {BE (100 nm)};
      \node[font=\scriptsize] at (31,9.4) {PZT (1.2 µm)};
      \node[font=\scriptsize] at (31,19.2) {TE (20 nm)};

      % アクチュエータAu電極（1.3 µm）
      \fill[gray!60] (12,20) rectangle (24,33);
      \fill[gray!60] (38,20) rectangle (50,33);
      \node[font=\scriptsize] at (18,31.4) {Au 1.3 µm (Act)};
      \node[font=\scriptsize] at (44,31.4) {Au 1.3 µm (Act)};

      % 任意のNiCrライニング（50 nm）例示
      \fill[gray!50] (50,20) rectangle (51,33);
      \node[font=\scriptsize,anchor=west] at (51.5,27) {NiCr 50 nm};

      % COF端子（Au 0.425 / Cu 8）
      \fill[orange!35] (12,33) rectangle (24,35.7); % Au
      \fill[brown!30]  (12,35.7) rectangle (24,41.7); % Cu
      ...
      % タイトル
      \node[font=\small] at (31,46.0) {対策前（Auマイグレーション発生リスク）};
    \end{scope}

    % 仕切り線
    \draw[dashed] (70,0) -- (70,48);

    % ---------------- 右：対策後（PZTスリット） ----------------
    \begin{scope}[xshift=78mm]
      % 下地層（PZTスリット導入）
      \fill[gray!30] (0,0) rectangle (62,6);              % BE
      \fill[gray!15] (0,6) rectangle (27,18);             % PZT 左
      \fill[gray!15] (35,6) rectangle (62,18);            % PZT 右
      \draw[line width=0.4pt] (31,6) -- (31,0);           % スリット溝
      \fill[gray!10] (0,18) rectangle (27,20);            % TE 左
      \fill[gray!10] (35,18) rectangle (62,20);           % TE 右
      \node[font=\scriptsize] at (31,1.6) {BE (100 nm)};
      \node[font=\scriptsize] at (31,9.4) {PZT (1.2 µm)};
      \node[font=\scriptsize] at (31,19.2) {TE (20 nm)};
      \node[font=\scriptsize,rotate=90] at (33.2,9.0) {PZT スリット};
      ...
      % タイトル
      \node[font=\small] at (31,46.0) {対策（PZTスリット導入：実効沿面距離の延長）};
    \end{scope}
  \end{tikzpicture}}
  \vspace{1mm}
  \caption{COF端子（Au 0.425 µm／Cu 8 µm）をアクチュエータAu電極（1.3 µm）上に重ねた断面模式図。
  左：対策前はTE表面でAuマイグレーションが起点となり得る。
  右：PZTスリット導入により実効沿面距離を延長し抑制。
