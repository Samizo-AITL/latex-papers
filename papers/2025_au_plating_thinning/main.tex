...
%=====================================================
% Fig: 対策前／対策後（縦並び・タイトル上・右側ラベルパネル）
% 1カラム幅で本文と重ならない版
%=====================================================
\begin{figure}[t]
  \centering
  ...
  \begin{tikzpicture}[x=0.9mm,y=0.9mm,>=Latex,inner sep=1pt]
    % 全体のバウンディングボックス（右側ラベルぶんも含める）
    \path[use as bounding box] (0,0) rectangle (92,104);

    %-----------------------------------------------
    % 上段：対策前
    %-----------------------------------------------
    \node[font=\small] at (38,99) {対策前：TE表面でAuマイグレーション発生リスク};

    % キャンバス座標
    \def\xL{8}  \def\xR{68}
    \def\yBE{80} \def\yPZT{92} \def\yTE{94}

    % 層（内部ラベルは描かない）
    \fill[gray!30] (\xL,\yBE-6) rectangle (\xR,\yBE);      % BE 100 nm
    \fill[gray!15] (\xL,\yBE)   rectangle (\xR,\yPZT);      % PZT 1.2 μm
    \fill[gray!10] (\xL,\yPZT)  rectangle (\xR,\yTE);       % TE 20 nm

    % アクチュエータAu
    \fill[gray!60] (22,\yTE) rectangle (34,\yTE+13);
    \fill[gray!60] (42,\yTE) rectangle (54,\yTE+13);
    % COF端子（Au/Cu）
    \fill[orange!35] (22,\yTE+13) rectangle (34,\yTE+15.7);
    \fill[brown!30]  (22,\yTE+15.7) rectangle (34,\yTE+21.7);
    \fill[orange!35] (42,\yTE+13) rectangle (54,\yTE+15.7);
    \fill[brown!30]  (42,\yTE+15.7) rectangle (54,\yTE+21.7);

    % Au移動の矢印
    \draw[->] (36,\yPZT-0.2) -- (40,\yPZT-0.2);

    % 右側ラベル・上段（縦に整列）
    \matrix[anchor=west,row sep=1.2mm] at (72,\yTE+20) {
      \node[font=\scriptsize] {Cu 8\,µm（COF）};\\
      \node[font=\scriptsize] {Au 0.425\,µm（COF）};\\
      \node[font=\scriptsize] {Au 1.3\,µm（Act）};\\
      \node[font=\scriptsize] {TE 20\,nm};\\
      \node[font=\scriptsize] {PZT 1.2\,µm};\\
      \node[font=\scriptsize] {BE 100\,nm};\\
      \node[font=\scriptsize] {Au migration on TE};\\
    };

    %-----------------------------------------------
    % 下段：対策後（PZTスリットを中央抜きで表現）
    %-----------------------------------------------
    \node[font=\small] at (38,56) {対策後：PZTスリット導入による実効沿面距離延長};

    \def\yBEb{38} \def\yPZTb{50} \def\yTEb{52}
    \def\gap{2.2} % スリット幅

    % BE
    \fill[gray!30] (\xL,\yBEb-6) rectangle (\xR,\yBEb);
    % PZT（両側）＋中央スリット
    \fill[gray!15] (\xL,\yBEb) rectangle ({38-\gap/2},\yPZTb);
    \fill[gray!15] ({38+\gap/2},\yBEb) rectangle (\xR,\yPZTb);
    % TE（両側）＋中央スリット
    \fill[gray!10] (\xL,\yPZTb) rectangle ({38-\gap/2},\yTEb);
    \fill[gray!10] ({38+\gap/2},\yPZTb) rectangle (\xR,\yTEb);
    ...
    % 右側ラベル・下段
    \matrix[anchor=west,row sep=1.2mm] at (72,\yTEb+20) {
      \node[font=\scriptsize] {Cu 8\,µm（COF）};\\
      \node[font=\scriptsize] {Au 0.425\,µm（COF）};\\
      \node[font=\scriptsize] {Au 1.3\,µm（Act）};\\
      \node[font=\scriptsize] {TE 20\,nm};\\
      \node[font=\scriptsize] {PZT 1.2\,µm（中央にスリット）};\\
      \node[font=\scriptsize] {PZT slit（抜き溝）};\\
      \node[font=\scriptsize] {BE 100\,nm};\\
    };
  \end{tikzpicture}}
  \vspace{2pt}
  \caption{COF端子（Au 0.425\,µm／Cu 8\,µm）とアクチュエータAu電極（1.3\,µm）の界面模式図。上：対策前はTE表面でAuマイグレーションが起点となる。下：対策後はPZTスリットで実効沿面距離が伸び、マイグレーションが抑制される。下地：NiCr 50\,nm／TE 20\,nm／PZT 1.2\,µm／BE 100\,nm。}
