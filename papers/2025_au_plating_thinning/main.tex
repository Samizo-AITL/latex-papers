...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[x=0.9mm,y=0.9mm,>=Latex,inner sep=1pt]
    %---------------- パラメータ ----------------
    \def\gap{1.4} % タイトルとCu上辺のすき間(mm)

    % 行送り用の外枠（上側を少し拡張）
    \path[use as bounding box] (0,0) rectangle (78,102);
    ...
    \fill[orange!35] (20,89) rectangle (32,91.7);      % Au
    \fill[brown!30]  (20,91.7) rectangle (32,97.7);    % Cu
    \fill[orange!35] (46,89) rectangle (58,91.7);
    \fill[brown!30]  (46,91.7) rectangle (58,97.7);

    % ←ここがポイント：Cu上辺(=97.7)に対して gap だけ上にタイトル
    \node[font=\small,anchor=south] at (0,97.7+\gap) {対策前：TE 表面で Au マイグレーション発生リスク};
    ...
    \draw[line width=0.5pt,gray!60] (39,20) -- (39,34); % PZTスリット
    ...
    \fill[orange!35] (20,47) rectangle (32,49.7);      % Au
    \fill[brown!30]  (20,49.7) rectangle (32,55.7);    % Cu
    \fill[orange!35] (46,47) rectangle (58,49.7);
    \fill[brown!30]  (46,49.7) rectangle (58,55.7);

    % ←ここがポイント：下図のCu上辺(=55.7)＋gap にタイトル
    \node[font=\small,anchor=south] at (0,55.7+\gap) {対策後：PZT スリット導入による実効沿面距離延長};

  \end{tikzpicture}

  \caption{上：対策前は TE 表面で Au マイグレーションが起点となる。下：対策後は PZT スリット導入により実効沿面距離が延長され抑制。下地：NiCr 50 nm／TE 20 nm／PZT 1.2 µm／BE 100 nm。}
