...
Auマイグレーションを抑制する構造へ改修した（Fig.~\ref{fig:act-mig-cross}）。

% ===== Fig: Cross-section (COF ↔ Actuator) =====
\begin{figure}[t]
  \centering
  \resizebox{\columnwidth}{!}{%
  \begin{tikzpicture}[x=1mm,y=1mm,>=Latex]
    % ----- drawing helpers (exaggerated scale) -----
    % thickness (exaggerated for visibility)
    \def\hCu{3.5}      % ~8 µm (exaggerated)
    \def\hAuCOF{1.8}   % ~0.425 µm (exaggerated)
    \def\hAuAct{2.8}   % ~1.3 µm (exaggerated)
    \def\hTE{0.5}      % 20 nm (exaggerated)
    \def\hPZT{10}      % 1.2 µm (exaggerated)
    \def\hBE{0.8}      % 100 nm (exaggerated)
    \def\W{70}         % total width
    \def\padW{14}      % pad width (each)
    \def\gap{10}       % slit width in PZT/TE (vertical trench)
    \def\xL{15}        % left pad x-start
    \def\xR{ \xL + \padW + \gap } % right pad x-start

    % ----- substrate baseline (for reference) -----
    \draw[white] (0,0) -- (\W,0);

    % ----- Bottom Electrode (BE) -----
    \fill[gray!50] (0,0) rectangle (\W,\hBE);
    \node[font=\scriptsize,anchor=south west] at (1,\hBE) {BE \,(100\,nm)};

    % ----- PZT bulk with slit -----
    \fill[gray!15] (0,\hBE) rectangle (\W,\hBE+\hPZT);
    % vertical slit (etched trench) down to BE
    \fill[white] (\xL+\padW,\hBE) rectangle (\xL+\padW+\gap,\hBE+\hPZT);
    \draw[thick] (\xL+\padW,\hBE) -- (\xL+\padW,\hBE+\hPZT);
    \draw[thick] (\xL+\padW+\gap,\hBE) -- (\xL+\padW+\gap,\hBE+\hPZT);
    \node[font=\scriptsize,rotate=90] at (\xL+\padW+0.5*\gap,\hBE+0.55*\hPZT) {PZTスリット};
    \node[font=\scriptsize,anchor=south west] at (1,\hBE+\hPZT) {PZT \,(1.2\,\si{\micro\metre})};

    % ----- Top Electrode (TE), interrupted at slit -----
    \fill[black!30] (0,\hBE+\hPZT) rectangle (\W,\hBE+\hPZT+\hTE);
    \fill[white] (\xL+\padW,\hBE+\hPZT) rectangle (\xL+\padW+\gap,\hBE+\hPZT+\hTE);
    \node[font=\scriptsize,anchor=south west] at (1,\hBE+\hPZT+\hTE) {TE \,(20\,nm)};

    % ----- Actuator Au pads on TE -----
    \fill[yellow!65!orange] (\xL,\hBE+\hPZT+\hTE) rectangle (\xL+\padW,\hBE+\hPZT+\hTE+\hAuAct);
    \fill[yellow!65!orange] (\xR,\hBE+\hPZT+\hTE) rectangle (\xR+\padW,\hBE+\hPZT+\hTE+\hAuAct);
    \draw (\xL,\hBE+\hPZT+\hTE) rectangle (\xL+\padW,\hBE+\hPZT+\hTE+\hAuAct);
    \draw (\xR,\hBE+\hPZT+\hTE) rectangle (\xR+\padW,\hBE+\hPZT+\hTE+\hAuAct);
    \node[font=\scriptsize] at (\xL+0.5*\padW,\hBE+\hPZT+\hTE+0.5*\hAuAct) {Au 1.3\,\si{\micro\metre}};
    \node[font=\scriptsize] at (\xR+0.5*\padW,\hBE+\hPZT+\hTE+0.5*\hAuAct) {Au 1.3\,\si{\micro\metre}};

    % ----- COF terminal (Cu + Au cap) overlapping pads -----
    % Cu (thick)
    \fill[brown!45!black] (\xL-1,\hBE+\hPZT+\hTE+\hAuAct) rectangle (\xL+\padW+1,\hBE+\hPZT+\hTE+\hAuAct+\hCu);
    \fill[brown!45!black] (\xR-1,\hBE+\hPZT+\hTE+\hAuAct) rectangle (\xR+\padW+1,\hBE+\hPZT+\hTE+\hAuAct+\hCu);
    % Au cap on Cu
    \fill[yellow!80!black] (\xL-1,\hBE+\hPZT+\hTE+\hAuAct+\hCu) rectangle (\xL+\padW+1,\hBE+\hPZT+\hTE+\hAuAct+\hCu+\hAuCOF);
    \fill[yellow!80!black] (\xR-1,\hBE+\hPZT+\hTE+\hAuAct+\hCu) rectangle (\xR+\padW+1,\hBE+\hPZT+\hTE+\hAuAct+\hCu+\hAuCOF);
    \draw (\xL-1,\hBE+\hPZT+\hTE+\hAuAct) rectangle (\xL+\padW+1,\hBE+\hPZT+\hTE+\hAuAct+\hCu+\hAuCOF);
    \draw (\xR-1,\hBE+\hPZT+\hTE+\hAuAct) rectangle (\xR+\padW+1,\hBE+\hPZT+\hTE+\hAuAct+\hCu+\hAuCOF);
    \node[font=\scriptsize,anchor=west] at (\xR+\padW+2,\hBE+\hPZT+\hTE+\hAuAct+0.5*\hCu) {COF Cu \,(8\,\si{\micro\metre})};
    \node[font=\scriptsize,anchor=west] at (\xR+\padW+2,\hBE+\hPZT+\hTE+\hAuAct+\hCu+0.5*\hAuCOF) {COF Au \,(0.425\,\si{\micro\metre})};

    % ----- Au–Au joint note -----
    \draw[<->,thin] (\xL+\padW+1.5,\hBE+\hPZT+\hTE+\hAuAct+0.2) -- ++(0, \hCu+\hAuCOF-0.4);
    \node[font=\scriptsize,anchor=west] at (\xL+\padW+2,\hBE+\hPZT+\hTE+\hAuAct+0.5*\hCu) {Au–Au接合（NPC充填）};

    % ----- labels left side -----
    \node[font=\scriptsize,anchor=east] at ( \xL-2,\hBE+0.5*\hPZT ) {PZT};
    \node[font=\scriptsize,anchor=east] at ( \xL-2,0.5*\hBE ) {BE};
  \end{tikzpicture}%
  }
  \caption{COF端子（Au 0.425\,µm / Cu 8\,µm）とアクチュエータ側（Au 1.3\,µm／TE 20\,nm／PZT 1.2\,µm／BE 100\,nm）の断面概念図。厚み比は視認性のため誇張。中央はPZTスリット改修。}
  \label{fig:cof-act-cross}
\end{figure}
