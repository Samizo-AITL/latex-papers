...
\begin{figure}[t]
  \centering
  \resizebox{\columnwidth}{!}{%
  \begin{tikzpicture}[x=0.90mm,y=0.90mm,>=Latex,inner sep=0.6pt]
    %--------------------------------------------------
    % 全体バウンディングボックス
    \path[use as bounding box] (0,0) rectangle (78,140);

    %================ パラメータ ========================
    \def\YtTop{126.0}   % 上図タイトル
    \def\YtBot{62.0}    % 下図タイトル
    \def\gapTitleCu{3.0}% ← タイトルとCu上辺の間隔を3mmに拡大
    \def\thBE{6}  \def\thPZT{12} \def\thTE{2}
    \def\thAuA{13}\def\thAuC{2.7}\def\thCuC{6.0}
    \def\Xl{8} \def\Xr{70}
    \def\XaL{20} \def\XaR{32}
    \def\XbL{46} \def\XbR{58}

    %================ ブロック描画関数 ====================
    \newcommand{\Block}[2]{%
      \pgfmathsetmacro{\Ytitle}{#1}
      \pgfmathsetmacro{\YcuTop}{\Ytitle - \gapTitleCu}
      \pgfmathsetmacro{\YauCtop}{\YcuTop}
      \pgfmathsetmacro{\YauCbot}{\YauCtop - \thAuC}
      \pgfmathsetmacro{\YcuCtop}{\YauCbot}
      \pgfmathsetmacro{\YcuCbot}{\YcuCtop - \thCuC}
      \pgfmathsetmacro{\YauAtop}{\YcuCbot}
      \pgfmathsetmacro{\YauAbot}{\YauAtop - \thAuA}
      \pgfmathsetmacro{\YTEtop}{\YauAbot}
      \pgfmathsetmacro{\YTEbot}{\YTEtop - \thTE}
      \pgfmathsetmacro{\YPZTop}{\YTEbot}
      \pgfmathsetmacro{\YPZBot}{\YPZTop - \thPZT}
      \pgfmathsetmacro{\YBEtop}{\YPZBot}
      \pgfmathsetmacro{\YBEbot}{\YBEtop - \thBE}

      %----- タイトル（中央） -----
      \node[font=\small,anchor=south] at (0.5*(\Xl+\Xr),\Ytitle)
        {\ifnum#2=0 対策前：TE表面でAuマイグレーション発生リスク
         \else 対策後：PZT抜き＋TE分離による沿面電界緩和\fi};

      %----- COF端子（Au/Cu） -----
      \fill[orange!35] (\XaL,\YauCtop) rectangle (\XaR,\YauCbot);
      \fill[brown!30]  (\XaL,\YcuCtop) rectangle (\XaR,\YcuCbot);
      \fill[orange!35] (\XbL,\YauCtop) rectangle (\XbR,\YauCbot);
      \fill[brown!30]  (\XbL,\YcuCtop) rectangle (\XbR,\YcuCbot);

      %----- アクチュエータAu -----
      \fill[gray!60] (\XaL,\YauAtop) rectangle (\XaR,\YauAbot);
      \fill[gray!60] (\XbL,\YauAtop) rectangle (\XbR,\YauAbot);

      %----- TE 層 -----
      \ifnum#2=0
        \fill[gray!10] (\Xl,\YTEtop) rectangle (\Xr,\YTEbot); % 対策前：連続
      \else
        \fill[gray!10] (\Xl,\YTEtop) rectangle (0.5*(\XaR+\XbL),\YTEbot);
        \fill[gray!10] (0.5*(\XaR+\XbL),\YTEtop) rectangle (\Xr,\YTEbot);
        \draw[line width=0.4pt] (0.5*(\XaR+\XbL),\YTEtop) -- (0.5*(\XaR+\XbL),\YBEbot);
      \fi

      %----- PZT 層 -----
      \ifnum#2=0
        \fill[gray!15] (\Xl,\YPZTop) rectangle (\Xr,\YPZBot); % 連続
      \else
        \fill[gray!15] (\Xl,\YPZTop) rectangle (0.5*(\XaR+\XbL),\YPZBot);
        \fill[gray!15] (0.5*(\XaR+\XbL),\YPZTop) rectangle (\Xr,\YPZBot);
      \fi

      %----- BE 層 -----
      \fill[gray!30] (\Xl,\YBEtop) rectangle (\Xr,\YBEbot);

      %----- Au移動矢印（対策前のみ） -----
      \ifnum#2=0
        \draw[->] (\XaR+2,\YTEtop-0.2) -- (\XbL-2,\YTEtop-0.2);
      \fi

      %----- 右側ラベル -----
      \pgfmathsetmacro{\Xlab}{74}
      \ifnum#2=0
        \node[anchor=west,font=\scriptsize] at (\Xlab,\YBEbot+2.0) {BE 100 nm};
        \node[anchor=west,font=\scriptsize] at (\Xlab,\YPZBot+2.0) {PZT 1.2 µm（連続）};
        \node[anchor=west,font=\scriptsize] at (\Xlab,\YTEbot+2.0) {TE 20 nm（連続）};
      \else
        \node[anchor=west,font=\scriptsize] at (\Xlab,\YBEbot+2.0) {BE 100 nm};
        \node[anchor=west,font=\scriptsize] at (\Xlab,\YPZBot+2.0) {PZT 1.2 µm（左右分離）};
        \node[anchor=west,font=\scriptsize] at (\Xlab,\YTEbot+2.0) {TE 20 nm（左右分離）};
      \fi
      \node[anchor=west,font=\scriptsize] at (\Xlab,\YauAbot+1.5) {Au 1.3 µm（Act）};
      \node[anchor=west,font=\scriptsize] at (\Xlab,\YcuCtop+0.8) {COF Au 0.425 µm / Cu 8 µm};
    }
    ...
    \Block{\YtBot}{1} % 下：対策後
  \end{tikzpicture}}
  \caption{COF端子とアクチュエータ電極の界面模式図（上：対策前，下：対策後）。
  各層名・膜厚は右側に集約。対策後はPZT抜き（中央スリット）とTE分離により沿面電界を緩和。}
