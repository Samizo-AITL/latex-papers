...
Auマイグレーションを抑制する構造へ改修した（Fig.~\ref{fig:cof-act-cross}）。
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[x=1mm,y=1mm,line cap=round,line join=round]
    %----- 手書き風スタイル -----
    \tikzset{sketch/.style={line width=0.5pt,decorate,decoration={random steps,segment length=2mm,amplitude=0.35mm}}}
    \tikzset{metal/.style={sketch,fill=gray!20}}
    \tikzset{au/.style={sketch,fill=yellow!60!brown!20}}
    \tikzset{cu/.style={sketch,fill=brown!40!orange!20}}
    % パネル幅
    \def\W{72} \def\H{42}
    ...
    \begin{scope}
      % 見出し
      \node[anchor=west,font=\scriptsize] at (0,\H+2) {対策前};
      % 層（下から BE / PZT / TE）
      \draw[metal,fill=gray!30] (0,0) rectangle (\W,2);      % BE 100 nm（模式）
      \node[font=\scriptsize,anchor=east] at (-1,1){BE};
      \draw[metal,fill=gray!20] (0,2) rectangle (\W,16);     % PZT 1.2 µm（模式）
      \node[font=\scriptsize,anchor=east] at (-1,9){PZT};
      \draw[metal,fill=gray!15] (0,16) rectangle (\W,17);    % TE 20 nm（模式）
      \node[font=\scriptsize,anchor=east] at (-1,16.5){TE};

      % アクチュエータAu電極(1.3 µm) 上に COF端子 Au(0.425)/Cu(8) を実装
      % 左パッド
      \draw[au] (15,17) rectangle (27,30);                   % Act Au 1.3 µm（模式）
      \draw[au] (15,30) rectangle (27,30.4);                 % COF Au 0.425 µm（模式厚）
      \draw[cu] (15,30.4) rectangle (27,38.4);               % COF Cu 8 µm（模式厚）
      % 右パッド
      \draw[au] (45,17) rectangle (57,30);
      \draw[au] (45,30) rectangle (57,30.4);
      \draw[cu] (45,30.4) rectangle (57,38.4);

      % NiCr（側壁薄膜の注記のみ）
      \node[font=\scriptsize] at (58.5,23){NiCr 50 nm};

      % Auマイグレーション（表面ににじむ手書き風）
      \draw[sketch] (27.2,17.2) .. controls (33,18) and (39,18) .. (44.8,17.2);
      \node[font=\scriptsize,anchor=south] at (36,18.4){Auマイグレーション};

      % 寸法ラベル（代表値）
      \node[font=\scriptsize,anchor=north] at (21,16.8){Au 1.3\,µm（Act）};
      \node[font=\scriptsize,anchor=south] at (21,30.2){Au 0.425\,µm（COF）};
      \node[font=\scriptsize,anchor=south] at (21,38.4){Cu 8\,µm};
    \end{scope}

    %================ 右：対策（PZTスリット導入） =================
    \begin{scope}[xshift=88mm]
      % 見出し
      \node[anchor=west,font=\scriptsize] at (0,\H+2) {対策（PZT スリット導入）};
      % 層
      \draw[metal,fill=gray!30] (0,0) rectangle (\W,2);      % BE
      \node[font=\scriptsize,anchor=east] at (-1,1){BE};
      \draw[metal,fill=gray!20] (0,2) rectangle (\W,16);     % PZT
      \node[font=\scriptsize,anchor=east] at (-1,9){PZT};
      \draw[metal,fill=gray!15] (0,16) rectangle (\W,17);    % TE
      \node[font=\scriptsize,anchor=east] at (-1,16.5){TE};

      % 中央スリット（TE～PZT～BEを貫通）
      \draw[white,fill=white] (35,0) -- (35,17) -- (37,17) -- (37,0) -- cycle;
      \draw[sketch] (35,0) -- (35,17) (37,0) -- (37,17);
      \node[font=\scriptsize,anchor=south] at (36,17){PZTスリット};

      % パッド（左右）
      \draw[au] (15,17) rectangle (27,30);
      \draw[au] (15,30) rectangle (27,30.4);
      \draw[cu] (15,30.4) rectangle (27,38.4);
      ...
      % 注記
      \node[font=\scriptsize,align=left,anchor=west] at (0,-4)
        {NiCr 50\,nm / TE 20\,nm / PZT 1.2\,µm / BE 100\,nm（模式厚）};
    \end{scope}
  \end{tikzpicture}
  \caption{断面スケッチ風模式図：左＝対策前（Auマイグレーション発生），右＝対策（PZTスリットで端子間距離を実効的に延長）}
  \label{fig:act-xsec-sketch}
\end{figure}
