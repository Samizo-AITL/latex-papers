...
%================ Appendix: アクチュエータ側配線のAuマイグレーション対策（断面模式図） ================
\begin{figure*}[t]
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[x=0.8mm,y=0.8mm,line cap=round,line join=round]
    % panel sizes
    \def\W{92}  % one panel width
    \def\H{46}  % panel height
    \def\G{16}  % gap between panels

    % ---------- common layer drawer ----------
    \newcommand{\layers}[2]{% origin=(#1,#2)
      % BE
      \draw[fill=gray!30,draw=black] (#1,#2) rectangle ++(\W,4);
      \node[anchor=east,font=\scriptsize] at (#1-2,#2+2){BE (100 nm)};
      % PZT
      \draw[fill=gray!20,draw=black] (#1,#2+4) rectangle ++(\W,28);
      \node[anchor=east,font=\scriptsize] at (#1-2,#2+18){PZT (1.2 µm)};
      % TE
      \draw[fill=gray!10,draw=black] (#1,#2+32) rectangle ++(\W,2);
      \node[anchor=east,font=\scriptsize] at (#1-2,#2+33){TE (20 nm)};
    }

    % ---------- left panel : before ----------
    \layers{0}{0}
    \node[anchor=west,font=\small] at (0,\H+2){対策前（Au マイグレーション発生）};

    % actuator Au pads (1.3 µm) on TE
    \draw[fill=yellow!60!brown!25] (16,34) rectangle (34,44); % left Act Au
    \draw[fill=yellow!60!brown!25] (58,34) rectangle (76,44); % right Act Au
    \node[font=\scriptsize] at (25,43.5){Au 1.3 µm (Act)};
    \node[font=\scriptsize] at (67,43.5){Au 1.3 µm (Act)};

    % COF Au(0.425) / Cu(8) stacked on actuator Au
    \draw[fill=yellow!70!brown!35] (16,44) rectangle (34,45); % COF Au
    \draw[fill=brown!40!orange!30] (16,45) rectangle (34,53); % COF Cu
    \draw[fill=yellow!70!brown!35] (58,44) rectangle (76,45);
    \draw[fill=brown!40!orange!30] (58,45) rectangle (76,53);
    \node[font=\scriptsize] at (25,45.8){COF Au 0.425 µm};
    \node[font=\scriptsize] at (67,52.8){Cu 8 µm};
    \node[font=\scriptsize] at (67,45.8){COF Au 0.425 µm};

    % side note for NiCr (薄膜抵抗層の注記のみ)
    \node[font=\scriptsize,align=left] at (81,39){NiCr 50 nm};

    % migration indication along TE surface
    \draw[-{Latex[length=2mm]},thick] (34.5,34.8) .. controls (45,36) and (55,36) .. (57.5,34.8);
    \node[font=\scriptsize,anchor=south] at (46,36.2){Au migration on TE};

    % ---------- right panel : countermeasure ----------
    \begin{scope}[xshift=\W+\G]
      \layers{0}{0}
      \node[anchor=west,font=\small] at (0,\H+2){対策（PZT スリット導入：実効沿面距離の延長）};

      % central slit through TE/PZT to BE
      \draw[fill=white,draw=black,thick] (45,0) -- (45,34) -- (47,34) -- (47,0) -- cycle;
      \node[font=\scriptsize] at (46,35.8){PZT スリット};

      % pads
      \draw[fill=yellow!60!brown!25] (16,34) rectangle (34,44);
      \draw[fill=yellow!70!brown!35] (16,44) rectangle (34,45);
      \draw[fill=brown!40!orange!30] (16,45) rectangle (34,53);

      \draw[fill=yellow!60!brown!25] (58,34) rectangle (76,44);
      \draw[fill=yellow!70!brown!35] (58,44) rectangle (76,45);
      \draw[fill=brown!40!orange!30] (58,45) rectangle (76,53);
      ...
      \node[font=\scriptsize] at (25,45.8){COF Au 0.425 µm};
      \node[font=\scriptsize] at (67,45.8){COF Au 0.425 µm};
      \node[font=\scriptsize] at (25,52.8){Cu 8 µm};
      \node[font=\scriptsize] at (67,52.8){Cu 8 µm};
    \end{scope}
  \end{tikzpicture}}%
  \caption{COF 端子（Au 0.425 µm／Cu 8 µm）をアクチュエータ Au 電極（1.3 µm）の上に実装した断面模式図。
  左：対策前は TE 表面で Au マイグレーションが起点となり得る。
  右：PZT スリットを導入し端子間の実効沿面距離を延長、マイグレーションを抑制。%
  （下地構成：NiCr 50 nm／TE 20 nm／PZT 1.2 µm／BE 100 nm）}
  \label{fig:act-xsec}
\end{figure*}
