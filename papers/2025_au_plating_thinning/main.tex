...
% ===== Appendix: Countermeasure Figure (two-column compatible) =====
\begin{figure*}[!t]
  \centering
  % 全体幅を抑えて、2カラム紙面での重なりを回避
  \resizebox{0.92\textwidth}{!}{%
  \begin{tikzpicture}[x=1mm,y=1mm,>=Latex,inner sep=0.5pt]
    % 全体外形枠（はみ出し防止）
    \path[use as bounding box] (0,0) rectangle (140,50);
    ...
    \begin{scope}
      ...
      \node[font=\tiny] at (31,1.3) {BE (100 nm)};
      \node[font=\tiny] at (31,9.2) {PZT (1.2 µm)};
      \node[font=\tiny] at (31,19.0) {TE (20 nm)};

      % アクチュエータAu電極（1.3 µm）
      \fill[gray!60] (12,20) rectangle (24,33);
      \fill[gray!60] (38,20) rectangle (50,33);
      \node[font=\tiny] at (18,31.0) {Au 1.3 µm (Act)};
      \node[font=\tiny] at (44,31.0) {Au 1.3 µm (Act)};

      % NiCrライニング
      \fill[gray!50] (50,20) rectangle (51,33);
      \node[font=\tiny,anchor=west] at (51.5,26.8) {NiCr 50 nm};
      ...
      \node[font=\tiny] at (18,34.7) {COF Au 0.425 µm};
      \node[font=\tiny] at (18,40.5) {Cu 8 µm};
      \node[font=\tiny] at (44,34.7) {COF Au 0.425 µm};
      \node[font=\tiny] at (44,40.5) {Cu 8 µm};

      % TE表面のAu移動
      \draw[->,line width=0.4pt] (26,19.8) -- (36,19.8);
      \node[font=\tiny] at (31,21.0) {Au migration on TE};

      % タイトル
      \node[font=\scriptsize] at (31,47.0)
        {対策前：TE表面でAuマイグレーション発生リスク};
    \end{scope}

    % 中央仕切り線
    \draw[dashed,line width=0.4pt] (70,0) -- (70,48);
    ...
    \begin{scope}[xshift=78mm]
      ...
      \node[font=\tiny] at (31,9.2) {PZT (1.2 µm)};
      \node[font=\tiny] at (31,19.0) {TE (20 nm)};
      \node[font=\tiny,rotate=90] at (33.2,9.0) {PZTスリット};
  ...
  右：PZTスリット導入により実効沿面距離が延長され、マイグレーション抑制効果を示す。
