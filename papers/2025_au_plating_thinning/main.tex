...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[x=1mm,y=1mm]
    ...
    \def\cofAuT{1.0}  % COF: Au 厚（図示）
    \def\cofCuT{4.0}  % COF: Cu 厚（図示）
    \def\auT{3.0}     % アクチュエータ Au（図示）
    \def\teT{2.0}     % TE（図示）
    \def\pztT{10.0}   % PZT（図示）
    \def\beT{2.0}     % BE（図示）
    \def\W{62}        % 左図の横幅
    \def\bx{10}       % 左側島の左端 x
    \def\bw{14}       % Au 島の幅
    \def\pitch{28}    % 島ピッチ
    \def\slitW{6}     % スリット幅（mmスケール）
    ...
    \begin{scope}[shift={(0,43)}]
      ...
      % 下地層
      \fill[beLayer]  (0,\yBEbot)  rectangle (\W,\yPZTbot); % BE
      \fill[pztLayer] (0,\yPZTbot) rectangle (\W,\yTEbot);  % PZT
      ...
      \node[anchor=west,font=\scriptsize] at (\W+2,\yAUbot+0.35*\auT) {Au（アクチュエータ）1.3\,µm（PZT厚とほぼ同じ）};
    ...
    \begin{scope}[shift={(0,0)}]
      ...
      % PZT と TE を連続で塗り、その後中央スリットを白抜き
      \fill[pztLayer] (0,\yPZTbot) rectangle (\W,\yTEbot);  % PZT
      \fill[teLayer]  (0,\yTEbot)  rectangle (\W,\yAUbot);  % TE

      % 中央スリット（TE と PZT を同時に切る）
      \pgfmathsetmacro{\sxL}{0.5*\W-0.5*\slitW}
      \pgfmathsetmacro{\sxR}{0.5*\W+0.5*\slitW}
      \fill[white] (\sxL,\yPZTbot) rectangle (\sxR,\yAUbot);
      ...
      % 右側ラベル
      \node[anchor=west,font=\scriptsize] at (\W+2,\yTop)      {COF（Cu 8\,µm／Au 0.425\,µm）};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yAUbot+0.35*\auT) {Au（アクチュエータ）1.3\,µm};
