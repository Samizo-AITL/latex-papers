...
\usetikzlibrary{positioning,arrows.meta} % positioning が必須
\usepackage{pgfplots}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[node distance=1.5cm, >=Latex]
    % 共通スタイル
    \tikzset{flow/.style={draw, rounded corners, align=center}}

    \node[flow] (start)  {従来仕様\\Au \SI{0.5}{\micro\metre}};
    \node[flow, below=of start] (spec) {新仕様策定\\\SI{0.425 \pm 0.125}{\micro\metre}};
    \node[flow, below=of spec]  (eval) {信頼性試験\\NPC, EM, 環境};
    \node[flow, below=of eval]  (ok)   {下限 \SI{0.30}{\micro\metre} にマージン確認};
    \node[flow, below=of ok]    (cost) {コスト削減効果確認\\\textyen{}4/チップ};
    \node[flow, below=of cost]  (impact) {事業効果：年間十億円規模};
