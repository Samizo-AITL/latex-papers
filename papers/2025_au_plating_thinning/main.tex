...
%================ Figure: Au migration before/after (TikZ) ================
\begin{figure}[t]
  \centering
  \begin{tikzpicture}[x=1mm,y=1mm]
    % 共通色
    \definecolor{coforange}{RGB}{244,170,88}

    %---------------- 上段：対策前 ----------------
    \begin{scope}[shift={(0,40)}] % ← 上段ブロックの原点
      % タイトル（このベースラインが y=0）
      \node[anchor=south west,font=\small] at (0,0)
        {対策前：TE 表面で Au マイグレーション発生リスク};

      % 層の寸法（相対座標で管理）
      \def\gap{0.2}     % タイトル直下の隙間
      \def\cofT{1.0}    % COF 厚（図示用）
      \def\auT{3.0}     % Au 厚（図示用）
      \def\teT{2.0}     % TE 厚（図示用）
      \def\pztT{10.0}   % PZT 厚（図示用）
      \def\beT{2.0}     % BE 厚（図示用）
      \def\W{62}        % 図幅

      % y 位置（上から下へ積む）
      \pgfmathsetmacro{\yTop}{-\gap}
      \pgfmathsetmacro{\yCOFbot}{\yTop-\cofT}
      ...
      % 下地層
      \fill[gray!30] (0,\yBEbot) rectangle (\W,\yPZTbot); % BE
      \fill[gray!15] (0,\yPZTbot) rectangle (\W,\yTEbot); % PZT
      \fill[gray!10] (0,\yTEbot) rectangle (\W,\yAUbot); % TE

      % Au 電極（2島）
      \def\bx{10} \def\bw{14}
      \fill[gray!60] (\bx,\yAUbot) rectangle (\bx+\bw,\yCOFbot);
      \fill[gray!60] (\bx+28,\yAUbot) rectangle (\bx+28+\bw,\yCOFbot);

      % COF（オレンジ）
      \fill[coforange] (\bx,\yCOFbot) rectangle (\bx+\bw,\yTop);
      \fill[coforange] (\bx+28,\yCOFbot) rectangle (\bx+28+\bw,\yTop);

      % ラベル（右側）
      \node[anchor=west,font=\scriptsize] at (\W+2,\yTop) {COF（Au 0.425\,µm／TE 20\,nm）};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yAUbot+0.3*\auT) {Au 電極};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yTEbot+0.5*\teT) {TE 20\,nm};
      \node[anchor=west,font=\scriptsize] at (\W+2,\yPZTbot+0.5*\pztT) {PZT 1.2\,µm（模式）};
    ...
    \begin{scope}[shift={(0,0)}]
      \node[anchor=south west,font=\small] at (0,0)
        {対策後：治具開口（中央スリット）で Au マイグレーションを抑制};

      % 同じ厚みパラメータを使用
      \def\gap{0.2}\def\cofT{1.0}\def\auT{3.0}\def\teT{2.0}\def\pztT{10.0}\def\beT{2.0}\def\W{62}
      \pgfmathsetmacro{\yTop}{-\gap}
      \pgfmathsetmacro{\yCOFbot}{\yTop-\cofT}
      \pgfmathsetmacro{\yAUbot}{\yCOFbot-\auT}
      \pgfmathsetmacro{\yTEbot}{\yAUbot-\teT}
      ...
      % TE（中央スリット開口）
      \fill[gray!10] (0,\yTEbot) rectangle (\W,\yAUbot);
      \fill[white]   (0.5*\W-3,\yTEbot) rectangle (0.5*\W+3,\yAUbot); % 中央 6mm スリット

      % Au 電極（2島）— スリット上ではTEが切れている
      \def\bx{10} \def\bw{14}
      \fill[gray!60] (\bx,\yAUbot) rectangle (\bx+\bw,\yCOFbot);
      \fill[gray!60] (\bx+28,\yAUbot) rectangle (\bx+28+\bw,\yCOFbot);
  ...
  \vspace{-1mm}
  \caption{COF 構造（上：対策前、下：対策後）。各タイトル直下 0.2\,mm から層が開始するよう配置。}
