% chap3_process.tex — プロセス（改訂版・確定）
\section{製造プロセス（\texorpdfstring{$\leq$}{≤}\,\SI{400}{\celsius})}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[font=\scriptsize,>=Latex,node distance=5mm and 8mm]
    % ← TikZの予約キー 'step' と衝突しないように名称変更
    \tikzset{procstep/.style={
      draw,rounded corners,align=left,inner sep=3pt,
      minimum width=64mm,fill=gray!10}}
    \node[procstep]{(1) Si(100) 洗浄 $\to$ Poly-Si (\SI{0.2}{\micro\meter})};
    \node[procstep,below=of current bounding box.south west,anchor=west]
