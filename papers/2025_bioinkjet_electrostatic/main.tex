...
\begin{figure}[t]
\centering
\resizebox{1.05\columnwidth}{!}{% ← やや横を広げつつ縦方向を強調
\begin{tikzpicture}[
  x=1mm,y=2.2mm, % ← 縦スケール拡大で視認性Up
...
% アイソレーション境界（横線で明示）
\node[below=7mm of buf] (sep) {};
\draw[dashed, gray, thick] ($(buf.south)+(-22mm,-2mm)$) -- ($(buf.south)+(22mm,-2mm)$);
\node[anchor=west, gray] at ($(buf.south)+(-21mm,-4.8mm)$) {アイソレーション境界};
...
% ---- 電源系（右側に配置）----
\node[note, right=12mm of hvdrv, align=left] (hv) {HVレール: +45\,V\\(上限 60\,V)};
\draw[->, thick] (hv.west) -- (hvdrv.east);

\node[note, right=12mm of mcu, align=left] (lv) {3.3\,V ロジック};
\draw[->, thick] (lv.west) -- (mcu.east);

\node[note, below=6mm of hvdrv, align=center, minimum width=34mm] (isodc) {ロジック/高圧分離\\アイソレーテッド DC/DC};

% ---- マルチチャネル分岐（横に展開）----
\coordinate (tap) at ($(snub.south)!0.5!(head.north)$);
\foreach \dx/\name in {-27mm/Actuator ch., 27mm/Actuator ch.} {
  \draw[fill=black] (tap) circle (0.6pt);
  \draw[thick] (tap) -- ++(\dx,0) node[act, right=\ifdim\dx pt>0pt 0mm\else -40mm\fi of {coordinate}, minimum width=26mm] {};
}
\node at ($(head.south)+(0,-2.5mm)$) {\(\cdots\) \textit{N} ch};

% ---- 注記（下部に集約）----
\node[note, below=9mm of head, minimum width=82mm] (notes) {
\textbf{設計要点}\\[-1pt]
-- 容量性負荷ゆえ電流要求が小さく、自己発熱は小 (\(\sim0.1~\si{\micro J/shot}\) @45\,V)\\
...
\end{tikzpicture}}
\caption{縦フローの駆動電装構成（3.3\,Vロジック $\rightarrow$ 45\,V HV）。上段から順に MCU/FPGA, DAC, BUF、アイソレーション境界を経て HVドライバ、RCスナバ、アクチュエータへ信号・電力を供給。}
\label{fig:drive}
\end{figure}
