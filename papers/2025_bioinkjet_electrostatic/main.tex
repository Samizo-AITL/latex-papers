...
\begin{tikzpicture}[
  ...
  blk/.style={draw, rounded corners, minimum width=34mm, minimum height=8.5mm, align=center, inner sep=3pt},
  lgblk/.style={blk, fill=blue!8},
  hvblk/.style={blk, fill=orange!10},
  prot/.style={blk, fill=red!6},
  act/.style={blk, fill=green!10},
  note/.style={draw, rounded corners, fill=gray!10, align=left, inner sep=3pt}
]

% 縦フロー（上→下）
\node[lgblk] (mcu) {MCU/FPGA\\(3.3\,V)};
\node[lgblk, below=of mcu] (dac) {DAC\\(0--5\,V)};
\node[lgblk, below=of dac] (buf) {BUF/OPA\\(ロジック域)};

% アイソレーション境界
\draw[dashed, gray, thick] ($(buf.south)+(-22mm,-2mm)$) -- ($(buf.south)+(22mm,-2mm)$);
\node[gray,anchor=west] at ($(buf.south)+(-21mm,-4.6mm)$) {アイソレーション境界};

\node[hvblk, below=10mm of buf] (hvdrv) {HVドライバ\\(0--60\,V)};
\node[prot, below=of hvdrv] (snub) {RCスナバ\\100\,\si{\ohm}+470\,\si{\pico\farad}};
\node[act, below=of snub, minimum width=40mm] (head) {静電MEMSアクチュエータ\\(10--50\,pF/ch)};

% 信号フロー
\foreach \a/\b in {mcu/dac,dac/buf,buf/hvdrv,hvdrv/snub,snub/head}{
  \draw[->,thick] (\a) -- (\b);
}

% 電源ノート（右）
\node[note, right=12mm of hvdrv] (hv) {HVレール: +45\,V（上限60\,V）};
\draw[->, thick] (hv.west) -- (hvdrv.east);
\node[note, right=12mm of mcu] (lv) {3.3\,V ロジック電源};
\draw[->, thick] (lv.west) -- (mcu.east);

% 備考
\node[note, below=8mm of head, minimum width=82mm] (notes) {
\textbf{設計要点}\\[-1pt]
-- 容量性負荷のため自己発熱は小（\(\sim0.1\,\si{\micro J/shot}\) @45\,V）\\
-- RCスナバでリンギング抑制，配線寄生(\(<0.2\,\si{\pico\farad}/\)ch)を吸収\\
-- 周波数 5--10\,kHz，台形波 5/5/10\,\si{\micro s}，Duty 20--60\%\\
-- Pull-in安全率 \(\ge 2\)（45\,V基準），60\,V上限運用も安定
};

\end{tikzpicture}}
\caption{縦フローの駆動電装構成（3.3\,Vロジック $\rightarrow$ 45\,V HV）。}
\label{fig:drive}
\end{figure}
...
% --- Fig.5: 1カラム完全対応版（凡例は軸内） ---
\begin{figure}[t]
\centering
\begin{tikzpicture}
\begin{axis}[
  width=\columnwidth, height=3.8cm,   % ← 1カラム幅に厳密合わせ
  scale only axis,                     % ← 外余白の影響を排除
  xmin=0, xmax=1, ymin=-0.05, ymax=1.05,
  axis lines=left,
  xtick=\empty, ytick=\empty,
  xlabel={時間 $t$ (\textmu s)}, ylabel={規格化値},
  legend style={
    font=\scriptsize,
    at={(0.02,0.98)}, anchor=north west,   % ← 軸内左上に固定
...
% V(t)
\addplot[blue,domain=0:1,samples=200]
  {(x<0.25)?(4*x):((x<0.5)?1:((x<0.9)?(1-(x-0.5)/0.4):0))};
\addlegendentry{$V(t)$: drive voltage}

% x(t)
\addplot[red,dashed,domain=0:1,samples=200]
  {0.85*((x<0.3)?(3.0*x):((x<0.55)?0.9:((x<0.95)?(0.9-(x-0.55)/0.4):0)))};
\addlegendentry{$x(t)$: diaphragm displacement}

% P(t)
\addplot[orange,dotted,domain=0:1,samples=200]
  {0.9*((x<0.28)?(3.2*x):((x<0.52)?0.896:((x<0.9)?(0.896-(x-0.52)/0.38):0)))};
\addlegendentry{$P(t)$: cavity pressure}

\end{axis}
\end{tikzpicture}
\caption{台形波駆動における$V(t)$・$x(t)$・$P(t)$の時間相関（概念図）。$V(t)$と$x(t)$は同相，$P(t)$はわずかに遅延。}
\label{fig:timing}
\end{figure}
