...
% 事前に：\usepackage{tikz} の後で
% \usetikzlibrary{calc,arrows.meta,patterns}

\begin{figure}[t]
  \centering
  % \resizebox{\columnwidth}{!}{%  ←幅を必ず1カラムに収めたい場合は有効化
  \begin{tikzpicture}[x=5cm,y=3.3cm, line cap=round, line join=round]
    ...
    \def\xWL{0.10}   % WL 左端
    \def\wWL{0.18}   % WL 幅
    \def\xSN{0.36}   % SN コンタークト中心
    \def\xLOC{0.62}  % LOCOS 中心

    % ILD 上面（y=0.68）
    \draw[black!40,thick] (0,0.68) -- (1.02,0.68) node[anno, right]{\small ILD};

    % LOCOS（盛り上がり）
    \draw[oxide] (\xLOC-0.14,0.00)
      .. controls (\xLOC-0.05,0.13) and (\xLOC+0.05,0.13) ..
      (\xLOC+0.14,0.00) -- (\xLOC+0.14,-0.20) -- (\xLOC-0.14,-0.20) -- cycle;
    ...
    \draw[densely dashed,black!60] (\xWL,0.12) -- (\xWL+\wWL,0.12); % チャネル

    % SN（上部電極）と縦コンタクト
    \draw[metal] (\xSN-0.06,0.62) rectangle (\xSN+0.06,0.78);
    \node[anno] at (\xSN,0.86) {SN};
    \draw[metal] (\xSN-0.018,0.12) rectangle (\xSN+0.018,0.68); % プラグ
    ...
    % 接触端近傍ダメージ（斜線パターン）
    \fill[pattern=north east lines, pattern color=metgray, draw=metgray, very thin]
      (\xSN+0.06,0.12) ++(0.01,-0.06) rectangle ++(0.05,0.08);
