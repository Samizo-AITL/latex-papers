...
% ===================== Fig.1 (TWO columns) — overlap-free (legend fix) =====================
\begin{figure*}[t]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    ...
    % グループ枠は塗り無し・背景へ
    grp/.style={draw, rounded corners, inner sep=8pt, fill=none}
]

% ===== Sensors (top row) =====
\node[hw] (imu) {IMU\\(1 kHz)};
\node[hw, right=25mm of imu] (gps) {GNSS\\(ZED-F9P)};
\node[hw, right=25mm of gps] (env) {Env.\\Sensors};

% ===== Control stack =====
\node[sw, below=16mm of gps, minimum width=14.5cm] (hinf)
      {$H_\infty$ Controller\\Disturbance rejection, rate $\leq$ 1.0 ms};

\node[sw, below=16mm of hinf, minimum width=14.5cm] (fsm)
      {FSM Supervisor: Normal / High-alt / Comm-loss / Emergency Return};

\node[sw, below=16mm of fsm, minimum width=14.5cm] (llm)
      {LLM-assisted Planning \& Auto-tuning (simulation, policy redesign)};

% ===== Device / mechanics row =====
\node[hw, below left=18mm and -4mm of hinf] (soc) {SoC\\(65 nm FDSOI)};
\node[hw, right=28mm of soc] (esc) {ESC Drivers\\$\leq 100\,\mu$s};
\node[mech, right=28mm of esc] (vp) {Variable-pitch\\20 in Rotor};

\node[mech, below=14mm of vp] (frame) {CFRP Frame\\700--900 mm};
\node[sec,  below=14mm of soc, minimum width=5.6cm] (secblk) {TPM + PQC\\Secure Boot / Keys};
\node[hw,   below=10mm of secblk, minimum width=5.6cm] (comms) {Comms (FHSS/UHF, LTE/5G)};

% ===== Group boxes を背景レイヤーに描画 =====
...
\draw[arrow] (env.south) to[out=270, in=0] (fsm.east);

\draw[arrow] (hinf.south) -- (fsm.north);
\draw[arrow] (fsm.south) -- (llm.north);

\draw[arrow] (hinf.south west) to[out=240, in=90] (soc.north);
\draw[arrow] (soc.east) -- (esc.west);
\draw[arrow] (esc.east) -- (vp.west);
\draw[arrow] (vp.south) -- (frame.north);

\draw[arrow] (fsm.east) to[out=0, in=90] (secblk.north);
\draw[arrow] (secblk.south) -- (comms.north);

% フィードバックは外周へ
\draw[arrow, bend right=40, looseness=1.25] (frame.west) to (imu.south);
\draw[arrow] (vp.north) to[out=90, in=0, looseness=1.15] (imu.east);

% ===== Legend（さらに下へ／前面で不透明） =====
\path (gHW.south) ++(0,-18mm) coordinate (leg); % 下げた
\node[box, fill=blue!6,  draw=black, fill opacity=1, text opacity=1,
      anchor=north east] (lg1) at ($(leg)+(-36mm,0)$) {\strut Software};
\node[box, fill=gray!10, draw=black, fill opacity=1, text opacity=1,
      anchor=north]      (lg2) at (leg)               {\strut Hardware};
\node[box, fill=green!8, draw=black, fill opacity=1, text opacity=1,
      anchor=north west] (lg3) at ($(leg)+(36mm,0)$)   {\strut Mechanics};
\node[box, fill=orange!10, dashed, draw=black, fill opacity=1, text opacity=1,
      anchor=north west] (lg4)
      at ($(lg3.north east)+(6mm,0)$) {\strut Security};

\end{tikzpicture}%
}
\caption{SkyEdge system architecture（重なり解消・凡例前面）。グループ枠は背景レイヤーで描画し、凡例は枠外かつ不透明で前面に配置。}
\label{fig:sysarch}
\end{figure*}
