...
%--- Fig.1: process placement (TikZ) ---
\begin{figure}[!t]
\centering
\begin{tikzpicture}[font=\footnotesize,>=latex]
  \tikzstyle{step}=[draw,rounded corners=2pt,minimum width=40mm,minimum height=4mm]
  \node[step] (act) {Active / Isolation};
  \node[step,below=4mm of act] (well) {VT Adjust / Well};
  \node[step,below=4mm of well] (poly) {Poly Gate Definition};
  \node[step,below=4mm of poly] (ldd) {LDD / Spacer};
  \node[step,below=4mm of ldd] (sd) {Source/Drain Implant};
  \node[step,below=4mm of sd] (sal) {Salicide (Co)};
  ...
  % FeFET module (dashed box)
  \node[draw,dashed,rounded corners=2pt,fit={(well) (sd)},inner sep=6mm,label={[align=center]right:
    \strut Added mask: +1 (FE metal gate)\\
    \strut FE anneal: BEOL furnace (no extra mask)}] (box) {};
  \node at ($(well)!0.5!(sd)$) {\parbox{42mm}{\centering FeFET Gate-Last: IL/FE/CAP (ALD) + TiN (PVD/ALD)\\[1mm]
  Crystallization RTA (450--500$^\circ$C) + FGA (350$^\circ$C)}};

  \node[step,below=8mm of sal] (beol) {ILD + Vias + BEOL};
  \draw[-stealth] (sal) -- (beol);
\end{tikzpicture}
...
\begin{axis}[
  ...
  legend style={at={(0.03,0.97)},anchor=north west,draw=none,fill=none,font=\footnotesize},
  ticklabel style={/pgf/number format/fixed}
]
\addplot+[mark=*,thick] coordinates {(1e2,1.60) (5e2,1.55) (1e4,1.48) (1e5,1.40) (1e6,1.30)};
\addlegendentry{$\pm 2.5$ V, 10 $\mu$s}
\addplot+[mark=*,thick,densely dashed] coordinates {(1e2,1.58) (5e2,1.53) (1e4,1.46) (1e5,1.38) (1e6,1.29)};
...
%--- Fig.3: wake-up (top) and retention (bottom) ---  <<<< ここを縦並びに変更
\begin{figure}[!t]
\centering
% (a) Wake-up
\begin{tikzpicture}
...
\addplot+[mark=*,thick] coordinates {(0,0.92) (30,0.97) (60,0.985) (120,0.992) (200,0.995)};
\end{axis}
\end{tikzpicture}

\vspace{1mm}
\small (a) Wake-up (early cycles). \normalsize
...
\addplot+[thick,densely dashed,domain=1.8e2:1.2e6,samples=2] {1.0 - 0.02*ln(x/1e2)/ln(1e6/1e2)};
\end{axis}
\end{tikzpicture}

\vspace{1mm}
\small (b) Retention (projection). \normalsize
...
\begin{axis}[
  ...
  ylabel={$\ln[-\ln(1-F)]$},
  legend style={at={(0.97,0.03)},anchor=south east,draw=none,fill=none,font=\footnotesize}
]
\addplot+[mark=*,thick] coordinates {(0.3,-2.0) (0.7,-1.1) (1.1,-0.3) (1.6,0.6) (2.2,1.1) (2.7,1.6)};
\addlegendentry{Low $E$-field}
\addplot+[mark=square*,thick,densely dashed] coordinates {(0.35,-1.8) (0.8,-0.9) (1.2,-0.1) (1.7,0.8) (2.3,1.3) (2.85,1.9)};
