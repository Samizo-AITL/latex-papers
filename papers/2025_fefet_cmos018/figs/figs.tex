%===================== figs.tex (fixed title & 2-page split) ====================
...
\usepackage{float}            % ← [H] を使って配置を固定
\pagestyle{empty}             % ページ番号を消す（不要なら削除）
...
% ここで必ず改ページして 2 ページ目に信頼性図を出す
\clearpage

%============================ Page 2 ============================

%--------- Fig.2（Endurance：フル幅／[H]固定）---------
\begin{figure}[H]
\centering
\begin{tikzpicture}
\begin{axis}[
...
%--------- Fig.3（Wake-up & Retention：横並び／[H]固定）---------
\begin{figure}[H]
\centering
\begin{subcaptionbox{Wake-up (early cycles)}[0.48\linewidth]{%
\begin{tikzpicture}
\begin{axis}[
  width=\linewidth,height=0.30\linewidth,
  xlabel={Light P/E Cycles}, ylabel={Normalized Window},
  ymin=0.92,ymax=1.00, grid=both, tick align=outside
]
\addplot+[mark=*,thick] coordinates {(0,0.92) (30,0.98) (60,0.985) (120,0.992) (200,0.996)};
\end{axis}
\end{tikzpicture}
}}\hfill
\begin{subcaptionbox{Retention projection at 85$^\circ$C}[0.48\linewidth]{%
\begin{tikzpicture}
\begin{axis}[
  width=\linewidth,height=0.30\linewidth,
  xmode=log, log basis x=10, grid=both,
  xlabel={Time $t$ @ 85$^\circ$C (s)}, ylabel={$ \Delta V_{\mathrm{th}}(t)/\Delta V_{\mathrm{th}}(t_0)$},
  ymin=0.92,ymax=1.00, tick align=outside
]
\addplot+[mark=*,thick] coordinates {(1e2,0.995) (1e3,0.985) (1e4,0.972) (1e5,0.960) (1e6,0.950)};
\end{axis}
\end{tikzpicture}
}}
\caption{Wake-up (left) and retention projection (right).}
\label{fig:wakeup_retention}
\end{figure}

%--------- Fig.4（TDDB；凡例は右下／[H]固定）---------
\begin{figure}[H]
\centering
\begin{tikzpicture}
\begin{axis}[
