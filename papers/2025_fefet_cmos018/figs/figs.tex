...
\vspace{10mm} % Fig.1 と Table I の間を広く
...
% ===== Fig.3 Wake-up & Retention（横並び・広め） =====
\captionsetup[sub]{font=small,skip=2pt,justification=centering}
\begin{figure}[H]
  \centering
  % 左：Wake-up
  \subcaptionbox{Wake-up}[0.49\linewidth]{%
    \begin{tikzpicture}
      \begin{axis}[
        width=\linewidth, height=0.52\linewidth, % 縦幅UP
  ...
  % 右：Retention
  \subcaptionbox{Retention at 85$^\circ$C}[0.49\linewidth]{%
    \begin{tikzpicture}
      \begin{axis}[
        width=\linewidth, height=0.52\linewidth, % 縦幅UP
...
\begin{tikzpicture}
\begin{axis}[
  width=0.92\linewidth,height=0.38\linewidth, % 少し縦長に
  xlabel={$\ln(t_{\mathrm{BD}})$ (arb.)},
  ylabel={$\ln[-\ln(1-F)]$},
  grid=both, tick align=inside,
  legend style={at={(0.98,0.05)},anchor=south east}, legend cell align=left
]
