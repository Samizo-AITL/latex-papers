...
\begin{tikzpicture}[
  node distance=8mm and 14mm, % <vert , horiz> ほんの少し横方向を広げる
  ...
  % === ここから配線：矢印先端をボックス手前で止める ===
  % 入口座標（ボックスの少し手前）を用意
  \coordinate (reid_in) at ([xshift=-2.2mm]reid.west);
  ...
  \draw[line] (fs1.east) -- ++(6mm,0) |- (reid_in);      % 矢印はここまで
  \draw[-, line width=0.55pt] (reid_in) -- (reid.west);   % 最後は矢印なしの短線

  % observers -> FDI -> MRAM
  \draw[line] (rc2.east) -- ++(6mm,0) |- (fs2.west);
  \draw[line] (fs2.east) -- ++(6mm,0) |- (mram_in);
  \draw[-, line width=0.55pt] (mram_in) -- (mram.west);

  % fixed K -> safe boot -> FRAM
  \draw[line] (rc3.east) -- ++(10mm,0) |- (fs3.west);
  \draw[line] (fs3.east) -- ++(8mm,0) |- (fram_in);
  \draw[-, line width=0.55pt] (fram_in) -- (fram.west);

  % AI -> SRAM（上へ回して衝突回避）
  \draw[line] (reid.east) -| ([xshift=-6mm]sram_in) -- (sram_in);
