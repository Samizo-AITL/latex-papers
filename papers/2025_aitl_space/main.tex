...
% === Fig.1: End-to-end design flow (2-column wide, centered) ===
\begin{figure*}[t]
\centering
\scriptsize
\resizebox{0.98\textwidth}{!}{% ページ幅いっぱいに拡大
\begin{tikzpicture}[node distance=16mm]
  % Nodes
  \node[smallbox] (edu)  {EduController\\\small Modeling \& JSON export};
  \node[smallbox, right=16mm of edu]  (json) {JSON\\\small $A,B,C,D,W_1,W_2,W_3$};
  \node[smallbox, right=16mm of json] (aitl) {AITL-H\\\small $H_\infty$ synthesis $\to$ fixed-point $K$};
  \node[smallbox, right=16mm of aitl] (hil)  {FPGA HIL\\\small SEU/outage injection, safety metrics};
  \node[smallbox, right=16mm of hil]  (fem)  {SystemDK FEM\\\small Thermal/radiation/package stress};
  \node[smallbox, right=16mm of fem]  (asic) {ASIC\\\small 22FDX FDSOI};
  ...
  % Note
  \node[align=center, below=6mm of hil]
    {\small Metrics: safe-mode $<1$\,s, recovery $\ge 99\%$, ECC scrubbing efficiency};
\end{tikzpicture}}%
...
% === Fig.2: AITL architecture (single-column, centered, no overlaps) ===
\begin{figure}[t]
\centering
\scriptsize
...
\begin{tikzpicture}[node distance=16mm]
  ...
  % Robust Core (left column)
  \node[smallbox] (rc1) {$H_\infty$/MPC/SMC\\\small Robust control};
  \node[smallbox, below=6mm of rc1] (rc2) {Observers / weights\\\small $W_1,W_2,W_3$};
  \node[smallbox, below=6mm of rc2] (rc3) {Fixed-point $K$\\\small RTL/FPGA/ASIC};

  % FSM Supervisor (middle column)
  \node[smallbox, right=22mm of rc1] (fs1) {Mode management\\\small Safe/Nominal/Recovery};
  \node[smallbox, below=6mm of fs1]  (fs2) {FDI/FDII\\\small Fault detection/isolation};
  \node[smallbox, below=6mm of fs2]  (fs3) {Safe boot\\\small Watchdog};

  % AI Adaptor (right column)
  \node[smallbox, right=22mm of fs1] (ai1) {Re-identification / drift compensation};
  \node[smallbox, below=6mm of ai1]  (ai2) {Parameter update\\\small Low frequency};
  \node[smallbox, below=6mm of ai2]  (ai3) {Validation gate\\\small Apply if safe};

  % Top flow
  \draw[line] (rc1.east) -- (fs1.west);
  \draw[line] (fs1.east) -- (ai1.west);
  \draw[line] (ai3.west) -- ++(-12mm,0) |- (rc3.east); % offset to avoid overlap

  % Tri-NVM (bottom row)
  \node[smallbox, below=18mm of rc3] (sram) {SRAM\\\small Execution/ECC};
  \node[smallbox, right=18mm of sram] (mram) {MRAM\\\small Logs/code/ECC};
  \node[smallbox, right=18mm of mram] (fram) {FRAM\\\small Safe boot/FSM};

  % Memory connections
  \draw[line] (rc3.south) |- (sram.north);
  \draw[line] (ai2.south) |- (mram.north);
  \draw[line] (fs3.south) |- (fram.north);
...
% === Fig.3: Closed-loop block diagram (single-column, centered, uncluttered) ===
\begin{figure}[t]
\centering
\scriptsize
\resizebox{0.96\columnwidth}{!}{%
\begin{tikzpicture}[node distance=16mm]
  ...
  \node[smallbox, right=30mm of K, minimum width=26mm] (P) {$P$};

  % Summing junctions
  \node[sum, left=14mm of K]  (sumin) {$\Sigma$};
  \node[sum, right=16mm of P] (sumy) {$\Sigma$};
  ...
  % Feedback (lowered to avoid collisions)
  \draw[line] (sumy) |- ++(0,-22mm) -| node[pos=0.25, below] {$-$} (sumin);

  % Disturbance w
  \node[above=12mm of P] (w) {};
  \draw[line] (w.center) -- node[right] {$w$} (P.north);

  % Noise v (offset input)
  \coordinate (vstart) at ($(sumy.south)+(10mm,-10mm)$);
  \draw[line] (vstart) -- node[right] {$v$} (sumy.south);

  % Performance output z
  \node[smallbox, below=12mm of P] (W) {$W$};
  \draw[line] (P.south) -- (W.north);
  \node[right=22mm of W] (z) {};
