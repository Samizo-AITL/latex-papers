...
% ---------- Fig.2 ----------
\scriptsize
\resizebox{0.95\columnwidth}{!}{% 2カラム中の1カラム幅相当でバランス
\begin{tikzpicture}[
  node distance=8mm and 16mm, % <vert , horiz> 横を広げて重なり回避
  line/.style={-{Latex[length=2mm,width=1.1mm]}, line width=0.55pt}
]
  % Left column
  \node[smallbox] (rc1) {$H_\infty$/MPC/SMC\\Robust control};
  \node[smallbox, below=6mm of rc1] (rc2) {Observers / weights\\$W_1,\,W_2,\,W_3$};
  \node[smallbox, below=6mm of rc2] (rc3) {Fixed-point $K$\\RTL/FPGA/ASIC};

  % Middle column
  \node[smallbox, right=30mm of rc1] (fs1) {Mode management\\Safe/Nominal/Recovery};
  \node[smallbox, below=6mm of fs1] (fs2) {FDI/FDII\\Fault detection/isolation};
  \node[smallbox, below=6mm of fs2] (fs3) {Safe boot\\Watchdog};

  % Right column (NVM)
  \node[smallbox, right=38mm of fs1, yshift=4mm] (sram) {SRAM\\Execution/ECC};
  \node[smallbox, below=7mm of sram] (mram) {MRAM\\Logs/code/ECC};
  \node[smallbox, below=7mm of mram] (fram) {FRAM\\Safe boot/FSM};

  % AI adaptor（NVMの左に独立配置）
  \node[smallbox, left=16mm of mram, yshift=-4mm] (reid) {Re-identification /\\drift compensation};
  \node[smallbox, below=10mm of fram] (param) {Parameter update\\Low frequency};
  \node[smallbox, below=7mm of param] (gate) {Validation gate\\Apply-if-safe};

  % 入口座標（矢印先端を箱の手前で止めて重なり回避）
  \coordinate (reid_in) at ([xshift=-2mm]reid.west);
  \coordinate (sram_in) at ([xshift=-2mm]sram.west);
  \coordinate (mram_in) at ([xshift=-2mm]mram.west);
  \coordinate (fram_in) at ([xshift=-2mm]fram.west);

  % Routing（直交・非交差）
  \draw[line] (rc1.east) -- (fs1.west);
  \draw[line] (fs1.east) -- ++(8mm,0) |- (reid_in);     \draw[-,line width=0.55pt] (reid_in) -- (reid.west);

  \draw[line] (rc2.east) -- ++(6mm,0) |- (fs2.west);
  \draw[line] (fs2.east) -- ++(8mm,0) |- (mram_in);      \draw[-,line width=0.55pt] (mram_in) -- (mram.west);

  \draw[line] (rc3.east) -- ++(6mm,0) |- (fs3.west);
  \draw[line] (fs3.east) -- ++(8mm,0) |- (fram_in);      \draw[-,line width=0.55pt] (fram_in) -- (fram.west);
...
\end{tikzpicture}}%
\captionof{figure}{AITL architecture: three-layer control stack with a role-partitioned tri-NVM hierarchy.}
\label{fig:arch}
