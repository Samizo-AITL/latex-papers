...
\begin{tikzpicture}[
  >=Latex,
  nodebox/.style={draw, rounded corners=2pt, align=center,
                  inner sep=2.2pt, minimum height=7mm, minimum width=28mm},
  link/.style={-Latex, line width=0.55pt, rounded corners=2pt,
               shorten >=2pt, shorten <=2pt},
  matrix of nodes,
  row sep=8mm, column sep=12mm   % ＜ゆとりを確保＞
]
%----- 3列 × 3行の基本グリッド（左=Core，中=FSM，右=NVM） -----
\matrix (M) {
  |[nodebox]| {$H_\infty$/MPC/SMC\\\small Robust control}   &
  |[nodebox]| {Mode management\\\small Safe/Nominal/Recovery} &
  |[nodebox]| {SRAM\\\small Execution/ECC} \\
  |[nodebox]| {Observers / weights\\\small $W_1,\,W_2,\,W_3$} &
  |[nodebox]| {FDI/FDII\\\small Fault detection/isolation}      &
  |[nodebox]| {MRAM\\\small Logs/code/ECC} \\
  |[nodebox]| {Fixed-point $K$\\\small RTL/FPGA/ASIC}           &
  |[nodebox]| {Safe boot\\\small Watchdog}                       &
  |[nodebox]| {FRAM\\\small Safe boot/FSM} \\
  % 追加列は作らず、AI 系は FSM 列の下に縦配置
};
% FSM列の真下にAIチェーン（重なり回避）
\node[nodebox, below=10mm of M-3-2] (reid)  {Re-identification /\\ drift compensation};
\node[nodebox, below=7mm  of reid]  (param) {Parameter update\\\small Low frequency};
\node[nodebox, below=7mm  of param] (gate)  {Validation gate\\\small Apply-if-safe};

%----- 直交配線（必ず回り込み、先端はshortenで箱外に止める） -----
% 左→中（Core→FSM）
\draw[link] (M-1-1.east) -- (M-1-2.west);
\draw[link] (M-2-1.east) -- (M-2-2.west);
\draw[link] (M-3-1.east) -- (M-3-2.west);

% 中→右（FSM→各NVM）（必ず少し右へ出てから上/下に回す）
\draw[link] (M-1-2.east) -- ++(5mm,0) |- (M-1-3.west); % → SRAM
\draw[link] (M-2-2.east) -- ++(5mm,0) |- (M-2-3.west); % → MRAM
\draw[link] (M-3-2.east) -- ++(5mm,0) |- (M-3-3.west); % → FRAM

% FSM→AI、AI→SRAM（AIは中央下→右上へ“コの字”で回す）
\draw[link] (M-1-2.south) |- (reid.west);                 % FSM(top) → Re-id
\draw[link] (reid.east) -| ([xshift=-3mm]M-1-3.west);     % Re-id → SRAM

% AI 縦チェーン
\draw[link] (reid) -- (param);
\draw[link] (param) -- (gate);
