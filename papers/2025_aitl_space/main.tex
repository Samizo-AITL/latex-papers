...
% === Fig.2: AITL architecture (clean layout, no overlaps) ===
\begin{figure}[t]
\centering
\scriptsize
\resizebox{0.92\columnwidth}{!}{% ← 幅は調整可（0.88–0.95 推奨）
\begin{tikzpicture}[node distance=7mm and 10mm]
  % 共通スタイル
  \tikzset{
    box/.style={draw, rounded corners=1.6pt, align=center,
                inner sep=2.2pt, minimum height=7.2mm, minimum width=26mm},
    line/.style={-{Latex[length=2.0mm,width=1.2mm]}, line width=0.45pt}
  }

  % --- Column 1: Robust Core (left) ---
  \node[box] (rc1) {$H_\infty$/MPC/SMC\\Robust control};
  \node[box, below=of rc1] (rc2) {Observers / weights\\$W_1, W_2, W_3$};
  \node[box, below=of rc2] (rc3) {Fixed-point $K$\\RTL/FPGA/ASIC};

  % --- Column 2: FSM (middle) ---
  \node[box, right=22mm of rc1] (fs1) {Mode management\\Safe/Nominal/Recovery};
  \node[box, below=of fs1]       (fs2) {FDI/FDII\\Fault detection/isolation};
  \node[box, below=of fs2]       (fs3) {Safe boot\\Watchdog};

  % --- Column 3: AI Adaptor (right-mid) ---
  \node[box, right=22mm of fs1] (ai1) {Re-identification\\/ drift compensation};
  \node[box, below=of ai1]      (ai2) {Parameter update\\Low frequency};
  \node[box, below=of ai2]      (ai3) {Validation gate\\Apply-if-safe};

  % --- Column 4: Memory (far right) ---
  \node[box, right=16mm of ai1] (sram) {SRAM\\Execution/ECC};
  \node[box, below=of sram]     (mram) {MRAM\\Logs/code/ECC};
  \node[box, below=of mram]     (fram) {FRAM\\Safe boot/FSM};

  % ===== Connections (no text overlap; use elbows/curves) =====
  % Core → FSM → AI
  \draw[line] (rc1) -- (fs1);
  \draw[line] (fs1) -- (ai1);

  % AI validation back to Core K（肘配線で重なり回避）
  \draw[line] (ai3.west) -| ++(-8mm,0) |- (rc3.east);

  % Core/FSM/AI → Memories（全て左から右へ L 字配線）
  \draw[line] (rc3.east) -| (sram.west);
  \draw[line] (fs3.east) -| (fram.west);
  \draw[line] (ai2.east) -| (mram.west);

  % AI reads from memories（短い枝で示す・重なり回避のため分岐点を中間に）
  \path let \p1=(ai1.east), \p2=(sram.west) in
    coordinate (tapA) at (\x1+6mm,\y1);
  \draw[line] (ai1.east) -- (tapA) |- (sram.west);
  \draw[line] (tapA) |- (mram.west);
  \draw[line] (tapA) |- (fram.west);
\end{tikzpicture}}%
