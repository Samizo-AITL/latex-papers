...
% === Fig.2: AITL architecture (fixed overlaps) ===========================
\begin{figure*}[t]
\centering
\scriptsize
...
\begin{tikzpicture}[node distance=9mm]
  ...
  % --- Tri-NVM column (right side, stacked) ---
  \node[smallbox, above=10mm of ai1, align=center] (sram) {SRAM\\Execution/ECC};
  \node[smallbox, below=4.5mm of sram, align=center] (mram) {MRAM\\Logs/code/ECC};
  \node[smallbox, below=4.5mm of mram, align=center] (fram) {FRAM\\Safe boot/FSM};

  % --- Connections (avoid overlaps using "out" / "in") ---
  % RC to FSM
  ...
  % Memory connections: routed up/down to avoid text overlaps
  \draw[line] (rc3.east) -| ++(10mm,-6mm) |- (fram.west);
  \draw[line] (fs2.east) -| ++(10mm,-4mm) |- (mram.west);
  \draw[line] (fs1.east) -| ++(10mm,8mm) |- (sram.west);
