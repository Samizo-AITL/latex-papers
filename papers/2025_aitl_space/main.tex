...
% === Fig.1: End-to-end design flow (one-column, clean) ===
\begin{figure}[t]
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[node distance=12mm] % ← 少し広め
  % ノード
  \node[smallbox, align=center] (edu) {EduController\\\small モデル化 \& JSON出力};
  \node[smallbox, right=12mm of edu, align=center] (json) {JSON\\\small $A,B,C,D$, $W_1,W_2,W_3$};
  \node[smallbox, right=12mm of json, align=center] (aitl) {AITL-H\\\small $H_\infty$合成 $\to$ 固定小数点$K$};
  \node[smallbox, right=12mm of aitl, align=center] (hil) {FPGA HIL\\\small SEU/欠測注入・安全率評価};
  \node[smallbox, right=12mm of hil, align=center] (fem) {SystemDK FEM\\\small 熱/放射/実装ストレス};
  \node[smallbox, right=12mm of fem, align=center] (asic) {ASIC\\\small 22FDX FDSOI};
...
% === Fig.2: AITL architecture (one-column, uncluttered) ===
\begin{figure}[t]
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[node distance=10mm] % ← 距離を広げる
  % レイヤ見出し
  \tikzset{title/.style={font=\bfseries, inner sep=1pt}}

  % --- Robust Core ---
  \node[smallbox, align=center] (rc1) {$H_\infty$/MPC/SMC\\\small 安定化制御};
  \node[smallbox, below=6mm of rc1, align=center] (rc2) {観測器/整形重み\\\small $W_1,W_2,W_3$};
  \node[smallbox, below=6mm of rc2, align=center] (rc3) {固定小数点$K$\\\small RTL/FPGA/ASIC};

  % --- FSM Supervisor ---
  \node[smallbox, right=16mm of rc1, align=center] (fs1) {モード管理\\\small Safe/Nominal/Recovery};
  \node[smallbox, below=6mm of fs1, align=center] (fs2) {FDI/FDII\\\small 故障検出・隔離};
  \node[smallbox, below=6mm of fs2, align=center] (fs3) {セーフブート\\\small ウォッチドッグ};

  % --- AI Adaptor ---
  \node[smallbox, right=16mm of fs1, align=center] (ai1) {長期リID/ドリフト補償};
  \node[smallbox, below=6mm of ai1, align=center] (ai2) {パラメタ更新\\\small 低頻度演算};
  \node[smallbox, below=6mm of ai2, align=center] (ai3) {検証ゲート\\\small 安全境界内のみ反映};

  % --- Tri-NVM ---
  \node[smallbox, above right=2mm and 12mm of fs1, align=center] (sram) {SRAM\\\small 実行/ECC};
  \node[smallbox, below=6mm of sram, align=center] (mram) {MRAM\\\small ログ/コード/ECC};
  \node[smallbox, below=6mm of mram, align=center] (fram) {FRAM\\\small セーフブート/FSM};
  ...
  \draw[line] (fs3.east) -- ++(8mm,0) |- (fram.west);
...
% === Fig.3: Closed-loop block (one-column, tidy) ===
\begin{figure}[t]
\centering
\scriptsize
\begin{tikzpicture}[node distance=10mm]
  % ブロック
  \node[smallbox, minimum width=22mm, align=center] (K) {$K$};
  \node[smallbox, right=22mm of K, minimum width=26mm, align=center] (P) {$P$};
  ...
  % z 出力
  \node[smallbox, below=8mm of P, align=center] (W) {$W$};
