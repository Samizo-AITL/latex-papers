...
% === Fig.2: AITL architecture (clean, no overlaps) ===
\begin{figure}[t]
\centering
\scriptsize
\resizebox{0.95\columnwidth}{!}{%
\begin{tikzpicture}[
  node distance=8mm and 12mm, % <vert , horiz>
  title/.style={font=\bfseries, inner sep=1pt}
]
  % ----- Left column: Robust core -----
  \node[smallbox, align=center] (rc1) {$H_\infty$/MPC/SMC\\\small Robust control};
  \node[smallbox, below=6mm of rc1, align=center] (rc2) {Observers / weights\\\small $W_1,\,W_2,\,W_3$};
  \node[smallbox, below=6mm of rc2, align=center] (rc3) {Fixed-point $K$\\\small RTL/FPGA/ASIC};

  % ----- Middle column: FSM supervisor -----
  \node[smallbox, right=28mm of rc1, align=center] (fs1) {Mode management\\\small Safe/Nominal/Recovery};
  \node[smallbox, below=6mm of fs1, align=center] (fs2) {FDI/FDII\\\small Fault detection/isolation};
  \node[smallbox, below=6mm of fs2, align=center] (fs3) {Safe boot\\\small Watchdog};

  % ----- Right column: NVM stack (top) -----
  \node[smallbox, right=36mm of fs1, yshift=5mm, align=center] (sram) {SRAM\\\small Execution/ECC};
  \node[smallbox, below=7mm of sram, align=center] (mram) {MRAM\\\small Logs/code/ECC};
  \node[smallbox, below=7mm of mram, align=center] (fram) {FRAM\\\small Safe boot/FSM};

  % ----- AI adaptor (separated below NVM to avoid overlaps) -----
  \node[smallbox, left=12mm of mram, align=center] (reid) {Re-identification /\\ drift compensation};
  \node[smallbox, below=10mm of fram, align=center] (param) {Parameter update\\\small Low frequency};
  \node[smallbox, below=7mm of param, align=center] (gate) {Validation gate\\\small Apply-if-safe};

  % ----- Clean, orthogonal routes (no label overlaps) -----
  % core -> FSM -> AI/NVM
  \draw[line] (rc1.east) -- (fs1.west);
  \draw[line] (fs1.east) -- ++(6mm,0) |- (reid.west);
  \draw[line] (reid.east) -| ([xshift=-6mm]sram.west) -- (sram.west);

  % observers -> FDI -> MRAM
  \draw[line] (rc2.east) -- ++(6mm,0) |- (fs2.west);
  \draw[line] (fs2.east) -- ++(6mm,0) |- ([xshift=-6mm]mram.west) -- (mram.west);

  % fixed K -> safe boot -> FRAM
  \draw[line] (rc3.east) -- ++(10mm,0) |- (fs3.west);
  \draw[line] (fs3.east) -- ++(8mm,0) |- ([xshift=-6mm]fram.west) -- (fram.west);

  % AI chain downward
  \draw[line] (reid.south) |- (param.west);
  \draw[line] (param.south) -- (gate.north);
\end{tikzpicture}}%
\caption{AITL architecture: three-layer control stack with a role-partitioned tri-NVM hierarchy.}
\label{fig:arch}
\end{figure}
