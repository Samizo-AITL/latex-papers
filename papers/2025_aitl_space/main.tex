...
% === Fig.2: simplified (no overlaps, bigger, no nested float) ===
\par\vspace{10mm} % ← Fig.1 との間隔を確保
\captionsetup{type=figure}
\centering
\resizebox{0.90\textwidth}{!}{%
\begin{tikzpicture}[
  node distance=10mm and 18mm,   % <vertical, horizontal> 間隔を広めに
  >=Latex,
  line/.style={-{Latex[length=2mm,width=1mm]}, line width=0.55pt},
  box/.style={draw, rounded corners=2pt, align=center,
              inner sep=2.5pt, minimum height=7mm, minimum width=35mm},
  every node/.style={font=\normalsize}
]
  % ----- Columns: Core (L), FSM (C), NVM (R) -----
  % Core (まとめて1ブロックに簡略化)
  \node[box] (core) {$H_\infty$ core\\(observers/weights, fixed-point $K$)};

  % FSM
  \node[box, right=42mm of core] (fsm) {FSM supervisor\\Safe/Nominal/Recovery};

  % NVM 3段（右列）
  \node[box, right=42mm of fsm] (sram) {SRAM\\Execution/ECC};
  \node[box, below=9mm of sram] (mram) {MRAM\\Logs/code/ECC};
  \node[box, below=9mm of mram] (fram) {FRAM\\Safe boot/FSM};

  % AI adaptor（中央下に独立配置）
  \node[box, below=18mm of fsm] (reid) {AI adaptor\\Re-identification / updates};
  \node[box, below=8mm of reid] (gate) {Validation gate (apply-if-safe)};

  % ---- Clean orthogonal wiring（交差なし・ノード手前止め不要）----
  \draw[line] (core) -- (fsm);

  % FSM → NVM（3本をそれぞれ直交配線）
  \draw[line] (fsm.east) -- ++(6mm,0) |- (sram.west);
  \draw[line] (fsm.east) -- ++(8mm,0) |- (mram.west);
  \draw[line] (fsm.east) -- ++(10mm,0) |- (fram.west);

  % AI adaptor 連系（上へ、そして右列の各ブロックへ分岐）
  \draw[line] (reid.north) -- (fsm.south);
  \draw[line] (reid.east) -| (sram.south);
  \draw[line] (reid.east) -| (mram.south);
  \draw[line] (reid.east) -| (fram.south);

  % 検証ゲート
  \draw[line] (gate.north) -- (reid.south);
\end{tikzpicture}}%
\captionof{figure}{AITL architecture (simplified): three-layer control stack with role-partitioned tri-NVM hierarchy.}
\label{fig:arch}
\par\vspace{10mm} % ← Fig.3 との間隔
