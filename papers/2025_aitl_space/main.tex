...
% === Fig.2 (REPLACE THIS WHOLE FIGURE) =====================
\begin{figure}[t]
\centering
\scriptsize
% ローカル上書き：矢印の食い込みを避ける
\tikzset{line/.style={-{Latex[length=2.0mm,width=1.2mm]}, line width=0.45pt, shorten >=2pt, shorten <=2pt}}

\resizebox{0.96\columnwidth}{!}{%
\begin{tikzpicture}[node distance=7mm]
  % ===== Columns: Robust | FSM | AI | Memories =====
  % Robust Core (left column)
  \node[smallbox, align=center] (rc1) {$H_\infty$/MPC/SMC\\\small Robust control};
  \node[smallbox, below=7mm of rc1, align=center] (rc2) {Observers / weights\\\small $W_1, W_2, W_3$};
  \node[smallbox, below=7mm of rc2, align=center] (rc3) {Fixed-point $K$\\\small RTL/FPGA/ASIC};

  % FSM (middle-left)
  \node[smallbox, right=22mm of rc1, align=center] (fs1) {Mode management\\\small Safe/Nominal/Recovery};
  \node[smallbox, below=7mm of fs1, align=center]  (fs2) {FDI/FDII\\\small Fault detection/isolation};
  \node[smallbox, below=7mm of fs2, align=center]  (fs3) {Safe boot\\\small Watchdog};

  % AI (middle-right)
  \node[smallbox, right=22mm of fs1, align=center] (ai1) {Re-identification / drift compensation};
  \node[smallbox, below=7mm of ai1, align=center]  (ai2) {Parameter update\\\small Low frequency};
  \node[smallbox, below=7mm of ai2, align=center]  (ai3) {Validation gate\\\small Apply-if-safe};

  % Memories (right column)
  \node[smallbox, right=26mm of ai1, align=center] (sram) {SRAM\\\small Execution/ECC};
  ...
  % ===== Clean routing (no overlaps) =====
  % Between layers
  \draw[line] (rc1.east) -- (fs1.west);
  \draw[line] (fs1.east) -- (ai1.west);

  % AI->Robust back path: 折り返しで重なり回避
  \draw[line] (ai3.west) -| ++(-10mm,-10mm) |- (rc3.south);

  % To memories (jog routing to avoid boxes/lines)
  \draw[line] (rc3.east) -| ($(sram.west)+(0,2mm)$) -- (sram.west);
