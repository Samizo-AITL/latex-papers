...
% === Fig.2: AITL architecture (no overlaps, EN only) ===
\begin{figure}[t]
\centering
\scriptsize
\resizebox{0.96\columnwidth}{!}{%
\begin{tikzpicture}[node distance=7.5mm] % 少し広め

  % ----- Robust Core column -----
  \node[smallbox, align=center] (rc1) {$H_\infty$/MPC/SMC\\\small Robust control};
  \node[smallbox, below=5mm of rc1] (rc2) {Observers / weights\\\small $W_1,W_2,W_3$};
  \node[smallbox, below=5mm of rc2] (rc3) {Fixed-point $K$\\\small RTL/FPGA/ASIC};

  % ----- FSM column -----
  \node[smallbox, right=22mm of rc1] (fs1) {Mode management\\\small Safe/Nominal/Recovery};
  \node[smallbox, below=5mm of fs1] (fs2) {FDI/FDII\\\small Fault detection/isolation};
  \node[smallbox, below=5mm of fs2] (fs3) {Safe boot\\\small Watchdog};

  % ----- AI column -----
  \node[smallbox, right=22mm of fs1] (ai1) {Re-identification / drift compensation};
  \node[smallbox, below=5mm of ai1] (ai2) {Parameter update\\\small Low frequency};
  \node[smallbox, below=5mm of ai2] (ai3) {Validation gate\\\small Apply-if-safe};

  % ----- Tri-NVM (stacked above AI) -----
  \node[smallbox, above=8mm of ai1] (sram) {SRAM\\\small Execution/ECC};
  \node[smallbox, below=3.5mm of sram] (mram) {MRAM\\\small Logs/code/ECC};
  \node[smallbox, below=3.5mm of mram] (fram) {FRAM\\\small Safe boot/FSM};

  % ----- Routing lanes to avoid label overlap -----
  \coordinate (laneTop) at ($(fs1.north)!0.5!(ai1.north)+(0,8mm)$);
  \coordinate (laneMid) at ($(fs2.south)!0.5!(ai2.south)+(0,-8mm)$);

  % ----- Signals (no overlaps) -----
  \draw[line] (rc1.east) -- (fs1.west);
  \draw[line] (fs1.east) -- (ai1.west);
  \draw[line] (ai3.west) to[out=180,in=0] ($(rc3.east)+(3mm,0)$) |- (rc3.east);

  % ----- Memory connections via lanes -----
  \draw[line] (rc3.east) -| ($(laneTop)+(10mm,0)$) |- (sram.west);
  \draw[line] (fs3.east) -| ($(laneMid)+(12mm,0)$) |- (fram.west);
  \draw[line] (ai2.east) -- ++(9mm,0) |- (mram.west);
