...
\begin{tikzpicture}[node distance=6mm]
  ...
  % 全体枠
  \node[bigbox, minimum width=140mm, minimum height=56mm, align=left] (soc) { };

  % レイヤ枠（左から右へ並べる）
  \node[bigbox, colA, minimum width=40mm, minimum height=46mm, anchor=west, xshift=4mm] (robust) at (soc.west) {};
  \node[bigbox, colB, minimum width=40mm, minimum height=46mm, right=4mm of robust] (fsm) {};
  \node[bigbox, colC, minimum width=40mm, minimum height=46mm, right=4mm of fsm] (ai) {};
  ...
  \draw[line] (ai3.west) -- ++(-6mm,0) |- (rc3.east); % 更新パス戻し

  % Tri-NVM（右側に縦積み）
  \node[bigbox, minimum width=26mm, minimum height=46mm, anchor=east, xshift=-4mm, align=center] (nvm) at (soc.east) {};
  \node[title, above=0mm of nvm] {Tri-NVM};

  \node[smallbox, minimum width=22mm, align=center] (sram) at ([yshift=+12mm]nvm.center) {SRAM\\\small 実行用};
  \node[smallbox, minimum width=22mm, below=3mm of sram] (mram) {MRAM\\\small ログ/コード\\ECC};
  \node[smallbox, minimum width=22mm, below=3mm of mram] (fram) {FRAM\\\small セーフブート/FSM};
...
\begin{tikzpicture}[node distance=10mm]
  ...
  % キャプション用の小さな注釈（任意）
  \node[align=center, font=\small, below=14mm of K] {設計：$H_\infty$ で $\|T_{w\to z}\|_\infty$ を最小化};
\end{tikzpicture}
