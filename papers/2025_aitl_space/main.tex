...
% === Fig.2: AITL architecture (no overlaps, centered, larger) ============
\begin{figure*}[t]
\centering
\scriptsize
...
\begin{tikzpicture}[node distance=8.5mm]
  ...
  % --- Column: Robust Core ------------------------------------------------
  \node[smallbox, align=center] (rc1) {$H_\infty$/MPC/SMC\\Robust control};
  \node[smallbox, below=6mm of rc1, align=center] (rc2) {Observers / weights\\$W_1,\,W_2,\,W_3$};
  \node[smallbox, below=6mm of rc2, align=center] (rc3) {Fixed-point $K$\\RTL/FPGA/ASIC};

  % --- Column: FSM Supervisor --------------------------------------------
  \node[smallbox, right=28mm of rc1, align=center] (fs1) {Mode management\\Safe/Nominal/Recovery};
  \node[smallbox, below=6mm of fs1, align=center] (fs2) {FDI/FDII\\Fault detection/isolation};
  \node[smallbox, below=6mm of fs2, align=center] (fs3) {Safe boot\\Watchdog};

  % --- Column: AI Adaptor -------------------------------------------------
  \node[smallbox, right=28mm of fs1, align=center] (ai1) {Re-identification / drift compensation};
  \node[smallbox, below=6mm of ai1, align=center] (ai2) {Parameter update\\Low frequency};
  \node[smallbox, below=6mm of ai2, align=center] (ai3) {Validation gate\\Apply-if-safe};

  % --- Column: Tri-NVM (stacked, far right) ------------------------------
  \node[smallbox, above=9mm of ai1, align=center]  (sram) {SRAM\\Execution/ECC};
  \node[smallbox, below=5mm of sram, align=center] (mram) {MRAM\\Logs/code/ECC};
  \node[smallbox, below=5mm of mram, align=center] (fram) {FRAM\\Safe boot/FSM};

  % --- Straight connections (no text overlap) ----------------------------
  \draw[line] (rc1.east) -- (fs1.west);
  \draw[line] (rc2.east) -- (fs2.west);
  \draw[line] (rc3.east) -- (fs3.west);

  \draw[line] (fs1.east) -- (ai1.west);
  \draw[line] (fs2.east) -- (ai2.west);
  \draw[line] (fs3.east) -- (ai3.west);

  % --- Memory fan-outs (offset -> vertical tap -> memory), avoid overlaps
  % mapping: execution <- RobustCore, safe boot <- FSM, logs/code <- AI
