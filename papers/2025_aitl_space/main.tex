...
\begin{minipage}[t]{0.98\textwidth}
  ...
  \begin{tikzpicture}[
    node distance = {8mm and 16mm}, % 縦8, 横16
    line/.style  = {-{Latex[length=2mm,width=1.2mm]}, line width=0.6pt},
    box/.style   = {draw, rounded corners=2pt, align=center,
                    inner sep=2.4pt, minimum height=7.2mm, minimum width=26mm},
    every node/.style={font=\normalsize}
  ]
  % ---- 左列：Robust Core ----
  \node[box] (rc1) {$H_\infty$/MPC/SMC\\Robust control};
  \node[box, below=8mm of rc1] (rc2) {Observers / weights\\$W_1,\,W_2,\,W_3$};
  \node[box, below=8mm of rc2] (rc3) {Fixed-point $K$\\RTL/FPGA/ASIC};

  % ---- 中央列：FSM ----
  \node[box, right=28mm of rc1] (fs1) {Mode management\\Safe/Nominal/Recovery};
  \node[box, below=8mm of fs1] (fs2) {FDI/FDII\\Fault detection/isolation};
  \node[box, below=8mm of fs2] (fs3) {Safe boot\\Watchdog};

  % ---- 右列：AI ----
  \node[box, right=28mm of fs1] (ai1) {Re-identification / drift compensation};
  \node[box, below=8mm of ai1] (ai2) {Parameter update\\Low frequency};
  \node[box, below=8mm of ai2] (ai3) {Validation gate\\Apply-if-safe};

  % ---- 最右列：Tri-NVM ----
  \node[box, right=20mm of ai1, minimum width=28mm] (sram) {SRAM\\Execution/ECC};
  \node[box, below=8mm of sram, minimum width=28mm] (mram) {MRAM\\Logs/code/ECC};
  \node[box, below=8mm of mram, minimum width=28mm] (fram) {FRAM\\Safe boot/FSM};

  % ---- 配線（重なりゼロ：直交＋曲線）----
  % Core → FSM → AI
  \draw[line] (rc1.east) -- (fs1.west);
  \draw[line] (fs1.east) -- ++(5mm,0) |- (ai1.west);

  % AI chain
  \draw[line] (ai1.south) -- (ai2.north);
  \draw[line] (ai2.south) -- (ai3.north);

  % Observers → FDI/FDII → MRAM
  \draw[line] (rc2.east) -- ++(5mm,0) |- (fs2.west);
  \draw[line] (fs2.east) to[out=0,in=180,looseness=0.9] (mram.west);

  % K → Safe boot → FRAM
  \draw[line] (rc3.east) -- ++(8mm,0) |- (fs3.west);
  \draw[line] (fs3.east) -- ++(10mm,0) |- (fram.west);

  % K → SRAM
  \draw[line] (rc3.east) -- ++(10mm,0) |- (sram.west);
  \end{tikzpicture}}
  \captionsetup{type=figure}
