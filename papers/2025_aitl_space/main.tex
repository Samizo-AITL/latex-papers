...
% === Fig.1: End-to-end design flow (2-row, full width) ===
\begin{figure*}[t]
\centering
\Huge
...
\begin{tikzpicture}[node distance=22mm]
  ...
  % -------- Row 1 --------
  \node[smallbox]                    (edu)  {EduController\\\large Modeling \& JSON export};
  \node[smallbox, right=22mm of edu] (json) {JSON\\\large $A,B,C,D,W_1,W_2,W_3$};
  \node[smallbox, right=22mm of json](aitl) {AITL-H\\\large $H_\infty$ synthesis $\to$ fixed-point $K$};

  % -------- Row 2 --------
  \node[smallbox, below=24mm of edu] (hil)  {FPGA HIL\\\large SEU/outage injection, safety metrics};
  \node[smallbox, right=22mm of hil] (fem)  {SystemDK FEM\\\large Thermal/radiation/package stress};
  \node[smallbox, right=22mm of fem] (asic) {ASIC\\\large 22FDX FDSOI};

  % Connections
  \draw[line] (edu) -- (json);
  \draw[line] (json) -- (aitl);
  \draw[line] (aitl) |- ++(0,-12mm) -| (hil);
  \draw[line] (hil) -- (fem);
  \draw[line] (fem) -- (asic);

  % Note
  \node[align=center, below=10mm of fem]
    {\large Metrics: safe-mode $<1$\,s, recovery $\ge 99\%$, ECC scrubbing efficiency};
...
% === Fig.2: AITL architecture (full width, centered) ===
\begin{figure*}[t]
\centering
\Huge
\resizebox{0.95\textwidth}{!}{%
\begin{tikzpicture}[node distance=24mm]
  ...
  % Robust Core
  \node[smallbox] (rc1) {$H_\infty$/MPC/SMC\\\large Robust control};
  \node[smallbox, below=8mm of rc1] (rc2) {Observers / weights\\\large $W_1,W_2,W_3$};
  \node[smallbox, below=8mm of rc2] (rc3) {Fixed-point $K$\\\large RTL/FPGA/ASIC};

  % FSM Supervisor
  \node[smallbox, right=32mm of rc1] (fs1) {Mode management\\\large Safe/Nominal/Recovery};
  \node[smallbox, below=8mm of fs1]  (fs2) {FDI/FDII\\\large Fault detection/isolation};
  \node[smallbox, below=8mm of fs2]  (fs3) {Safe boot\\\large Watchdog};

  % AI Adaptor
  \node[smallbox, right=32mm of fs1] (ai1) {Re-identification / drift compensation};
  \node[smallbox, below=8mm of ai1]  (ai2) {Parameter update\\\large Low frequency};
  \node[smallbox, below=8mm of ai2]  (ai3) {Validation gate\\\large Apply if safe};

  % Flows
  \draw[line] (rc1.east) -- (fs1.west);
  \draw[line] (fs1.east) -- (ai1.west);
  \draw[line] (ai3.west) -- ++(-18mm,0) |- (rc3.east);

  % Tri-NVM (bottom row)
  \node[smallbox, below=26mm of rc3] (sram) {SRAM\\\large Execution/ECC};
  \node[smallbox, right=28mm of sram] (mram) {MRAM\\\large Logs/code/ECC};
  \node[smallbox, right=28mm of mram] (fram) {FRAM\\\large Safe boot/FSM};
...
% === Fig.3: Closed-loop block diagram (full width, centered) ===
\begin{figure*}[t]
\centering
\Huge
\resizebox{0.95\textwidth}{!}{%
\begin{tikzpicture}[node distance=26mm]
  ...
  % Blocks
  \node[smallbox, minimum width=30mm] (K) {$K$};
  \node[smallbox, right=46mm of K, minimum width=34mm] (P) {$P$};

  % Summing junctions
  \node[sum, left=20mm of K]  (sumin) {$\Sigma$};
  \node[sum, right=24mm of P] (sumy) {$\Sigma$};
  ...
  % Feedback (lower to avoid overlap)
  \draw[line] (sumy) |- ++(0,-34mm) -| node[pos=0.25, below] {$-$} (sumin);

  % Disturbance
  \node[above=16mm of P] (w) {};
  \draw[line] (w.center) -- node[right] {$w$} (P.north);

  % Noise
  \coordinate (vstart) at ($(sumy.south)+(14mm,-14mm)$);
  \draw[line] (vstart) -- node[right] {$v$} (sumy.south);

  % Performance output
  \node[smallbox, below=18mm of P] (W) {$W$};
  \draw[line] (P.south) -- (W.north);
  \node[right=30mm of W] (z) {};
