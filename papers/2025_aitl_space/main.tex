...
% === Fig.1: End-to-end design flow (2-column wide) ===
\begin{figure*}[t]
\centering
\scriptsize
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[node distance=14mm]
  ...
  \node[smallbox, right=14mm of edu] (json) {JSON\\\small $A,B,C,D,W_1,W_2,W_3$};
  \node[smallbox, right=14mm of json] (aitl) {AITL-H\\\small $H_\infty$ synthesis $\to$ fixed-point $K$};
  \node[smallbox, right=14mm of aitl] (hil) {FPGA HIL\\\small SEU/outage injection, safety metrics};
  \node[smallbox, right=14mm of hil] (fem) {SystemDK FEM\\\small Thermal/radiation/package stress};
  \node[smallbox, right=14mm of fem] (asic) {ASIC\\\small 22FDX FDSOI};
...
% === Fig.2: AITL architecture (clean layout) ===
\begin{figure}[t]
\centering
\scriptsize
...
\begin{tikzpicture}[node distance=14mm]
  ...
  % FSM Supervisor
  \node[smallbox, right=18mm of rc1] (fs1) {Mode management\\\small Safe/Nominal/Recovery};
  ...
  % AI Adaptor
  \node[smallbox, right=18mm of fs1] (ai1) {Re-identification / drift compensation};
  ...
  % NVM hierarchy
  \node[smallbox, above right=4mm and 12mm of fs1] (sram) {SRAM\\\small Execution/ECC};
  \node[smallbox, below=6mm of sram] (mram) {MRAM\\\small Logs/code/ECC};
  \node[smallbox, below=6mm of mram] (fram) {FRAM\\\small Safe boot/FSM};
  ...
  % Memory connections (offset to avoid overlap)
  \draw[line] (rc3.east) -- ++(8mm,0) |- (sram.west);
  \draw[line] (fs3.east) -- ++(10mm,0) |- (fram.west);
  \draw[line] (ai2.east) -- ++(12mm,0) |- (mram.west);
\end{tikzpicture}}%
...
% === Fig.3: Closed-loop block diagram (spaced) ===
\begin{figure}[t]
\centering
\scriptsize
\begin{tikzpicture}[node distance=14mm]
  ...
  \node[smallbox, right=28mm of K, minimum width=26mm] (P) {$P$};

  % Sums
  \node[sum, left=12mm of K] (sumin) {$\Sigma$};
  \node[sum, right=14mm of P] (sumy) {$\Sigma$};
  ...
  % Feedback (shifted lower to avoid overlap)
  ...
  \node[below=12mm of sumy] (v) {};
  \draw[line] (v.center) -- node[right] {$v$} (sumy.south);

  % z-output
  \node[smallbox, below=10mm of P] (W) {$W$};
  \draw[line] (P.south) -- (W.north);
  \node[right=18mm of W] (z) {};
