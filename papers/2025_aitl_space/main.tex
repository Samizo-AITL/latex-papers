...
\vspace{14mm} % ← Fig.1とFig.2の間隔を広げる

% === Fig.2: AITL architecture (simplified / no overlaps) ===
\begin{figure}[t]
\centering
\scriptsize
\resizebox{0.98\columnwidth}{!}{%
\begin{tikzpicture}[
  node distance = 7mm and 18mm,      % <row , col> 間隔を広めに
  box/.style  = {draw, rounded corners=2pt, align=center,
                 inner sep=2.5pt, minimum height=7mm, minimum width=28mm},
  arrow/.style= {-{Latex[length=2mm,width=1.1mm]}, line width=0.55pt, shorten >=2pt}
]

% ----- Left column: Robust core -----
\node[box] (rc1) {$H_\infty$/MPC/SMC\\\small Robust control};
\node[box, below=6mm of rc1] (rc2) {Observers / weights\\\small $W_1,\,W_2,\,W_3$};
\node[box, below=6mm of rc2] (rc3) {Fixed-point $K$\\\small RTL/FPGA/ASIC};

% ----- Middle column: FSM supervisor -----
\node[box, right=18mm of rc1] (fs1) {Mode management\\\small Safe/Nominal/Recovery};
\node[box, below=6mm of fs1]    (fs2) {FDI/FDII\\\small Fault detection/isolation};
\node[box, below=6mm of fs2]    (fs3) {Safe boot\\\small Watchdog};

% ----- Right column: NVM stack -----
\node[box, right=22mm of fs1, yshift=5mm] (sram) {SRAM\\\small Execution/ECC};
\node[box, below=7mm of sram]             (mram) {MRAM\\\small Logs/code/ECC};
\node[box, below=7mm of mram]             (fram) {FRAM\\\small Safe boot/FSM};

% ----- Far-right: AI adaptor（NVMの右に縦配置して交差回避） -----
\node[box, right=18mm of mram] (reid) {Re-identification\\\small Drift compensation};
\node[box, below=7mm of reid]  (param){Parameter update\\\small Low frequency};
\node[box, below=7mm of param] (gate) {Validation gate\\\small Apply-if-safe};

% ===== Connections (全て直交・交差なし) =====
% core -> supervisor
\draw[arrow] (rc1.east) -- (fs1.west);
\draw[arrow] (rc2.east) -- (fs2.west);
\draw[arrow] (rc3.east) -- (fs3.west);

% supervisor -> memories
\draw[arrow] (fs1.east) -- ++(6mm,0) |- (sram.west);
\draw[arrow] (fs2.east) -- ++(6mm,0) |- (mram.west);
\draw[arrow] (fs3.east) -- ++(6mm,0) |- (fram.west);

% AI chain (縦) ＋ AI -> SRAM 経路（上を回して接続）
\draw[arrow] (reid.south) -- (param.north);
\draw[arrow] (param.south) -- (gate.north);
\draw[arrow] (reid.west)  -| ([xshift=-3mm]sram.north east) |- (sram.east); % 右回し→上→SRAM

\end{tikzpicture}}%
\caption{AITL architecture (simplified): three-layer control with a role-partitioned tri-NVM hierarchy and AI adaptor, drawn with orthogonal, overlap-free routes.}
\label{fig:arch}
\end{figure}
