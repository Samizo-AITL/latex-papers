...
% ---------- Fig.2 (overlap-free, enlarged) ----------
\resizebox{0.95\textwidth}{!}{%
\begin{tikzpicture}[
  >=Latex,
  line/.style={-{Latex[length=2mm,width=1.2mm]}, line width=0.6pt},
  node distance=10mm and 18mm, % vertical , horizontal
  smallbox/.style={draw, rounded corners=2pt, align=center,
                   font=\small, minimum height=8mm, minimum width=26mm}
]
  % Left column: Robust Core
  \node[smallbox] (rc1) {$H_\infty$/MPC/SMC\\Robust control};
  \node[smallbox, below=8mm of rc1] (rc2) {Observers / weights\\$W_1,\,W_2,\,W_3$};
  \node[smallbox, below=8mm of rc2] (rc3) {Fixed-point $K$\\RTL/FPGA/ASIC};

  % Middle column: FSM Supervisor
  \node[smallbox, right=40mm of rc1] (fs1) {Mode management\\Safe/Nominal/Recovery};
  \node[smallbox, below=8mm of fs1] (fs2) {FDI/FDII\\Fault detection/isolation};
  \node[smallbox, below=8mm of fs2] (fs3) {Safe boot\\Watchdog};

  % Right column: Tri-NVM (top)
  \node[smallbox, right=44mm of fs1, yshift=2mm] (sram) {SRAM\\Execution/ECC};
  \node[smallbox, below=9mm of sram] (mram) {MRAM\\Logs/code/ECC};
  \node[smallbox, below=9mm of mram] (fram) {FRAM\\Safe boot/FSM};

  % AI adaptor (placed to avoid overlaps)
  \node[smallbox, left=14mm of mram] (reid) {Re-identification\\/ drift compensation};
  \node[smallbox, below=9mm of fram] (param) {Parameter update\\Low frequency};
  \node[smallbox, below=7mm of param] (gate) {Validation gate\\Apply-if-safe};

  % --- Orthogonal routing (no overlaps) ---
  % core -> FSM -> AI/NVM
  \draw[line] (rc1.east) -- (fs1.west);
  \draw[line] (fs1.east) -- ++(8mm,0) |- (reid.west);
  \draw[line] (reid.east) -| ([xshift=-6mm]sram.west) -- (sram.west);

  % observers -> FDI -> MRAM
  \draw[line] (rc2.east) -- ++(10mm,0) |- (fs2.west);
  \draw[line] (fs2.east) -- ++(10mm,0) |- ([xshift=-6mm]mram.west) -- (mram.west);

  % fixed K -> safe boot -> FRAM
  \draw[line] (rc3.east) -- ++(12mm,0) |- (fs3.west);
  \draw[line] (fs3.east) -- ++(12mm,0) |- ([xshift=-6mm]fram.west) -- (fram.west);

  % AI downward chain (kept away from other edges)
  \draw[line] (fram.south) -- (param.north);
