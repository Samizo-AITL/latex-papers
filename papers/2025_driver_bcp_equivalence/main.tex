...
% ---------- LuaLaTeX 日本語・記号（μ）対応：決定版 ----------
\usepackage{iftex}
\ifLuaTeX
  \usepackage{luatexja}
  \usepackage{luatexja-fontspec}      % ← presetでなくfontspecで明示指定
  \setmainjfont{IPAexMincho}          % 和文本文（明朝）
  \setsansjfont{IPAexGothic}          % 和文サンセリフ
...
\newunicodechar{μ}{\micro}            % 生“μ”→\mu に自動置換
\newunicodechar{¥}{\textbackslash}    % 日本語KB対策（任意）
