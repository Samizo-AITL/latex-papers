...
% --- Unicode直打ちの安全化（μなどを確実に通す） ---
\usepackage{newunicodechar}
\newcommand{\micro}{\ensuremath{\mu}} % 既存マクロを先に定義
\newunicodechar{μ}{\micro}            % 生の“μ”を \micro に自動置換
\newunicodechar{¥}{\textbackslash}    % 日本語キーボードの「¥」対策（任意）

% ---------- 便利パッケージ群（IEEEtran互換構成） ----------
\usepackage{graphicx}
% \graphicspath{{./figs/}}           % 外部図を使うなら有効化
...
% \usepackage{physics}               % 競合する場合はコメントアウトのまま
...
% ハイパーリンクは最後、cleverefはその直後が安定（IEEEtran推奨順）
