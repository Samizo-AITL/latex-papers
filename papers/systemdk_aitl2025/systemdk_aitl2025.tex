...
\begin{tikzpicture}[
  ...
  a/.style={->,>=Latex,semithick,shorten >=1pt,shorten <=1pt},
  ad/.style={->,>=Latex,semithick,dashed,shorten >=1pt,shorten <=1pt}
]

%--- layout (座標を素直に) ---
\node[ghost] (llm) at (0,3.3)
  {LLM Analyzer (Next):\\retunes $K_p,K_i,K_d$; regenerates FSM rules};

\node[box] (yaml) at (-4.8,1.8) {YAML Config\\FW / CSR};
\node[box] (pid)  at (-1.8,1.8) {PID Controller};
\node[box] (fsm)  at ( 1.8,1.8) {FSM Supervisor};

\node[box] (tele) at (0,0) {Telemetry:\\Delay / Temp / Jitter};

% Physics models stack (囲み)
\node[box] (fem)  at (0,-1.8) {FEM\\(Thermal / Stress)};
\node[box] (spar) at (0,-3.4) {S-parameters (SI/EMI)};
\node[ghost,fit=(fem)(spar),inner xsep=10pt,inner ysep=8pt,label=below:{\strut Compact Physics Models}] (phys) {};

\node[box] (eda)  at (-4.8,0) {EDA Flow\\(Synth / P\&R / STA)};

%--- LLM proposals (点線で助言のニュアンス) ---
\draw[ad] (llm.south) to[out=-90,in=90] (pid.north);
\draw[ad] (llm.south) to[out=-90,in=90] (fsm.north);

%--- 設定／監督 ---
\draw[a] (yaml.east) -- (pid.west);
\draw[a] (fsm.west)  to[out=180,in=0] (eda.east);

%--- 計測／物理モデル連鎖 ---
\draw[a] (tele.north) -- (pid.south);
\draw[a] (spar.north) -- (fem.south);
\draw[a] (fem.north)  -- (tele.south);

% EDA -> Telemetry：交差を避けるため、一旦右へ逃がしてから縦折れ
\path let \p1 = (eda.east) in
  \draw[a]  (\x1+6mm,\y1) |- (tele.west);
