...
% ---- Fig.1 tikz: fixed routing (no overlap with node borders) ----
\begin{figure*}[t]
\centering
\begin{tikzpicture}[
  font=\small,
...
% --- Top rail for LLM fanout (avoids odd entry to FSM/PID) ---
% rail is a little above the controllers so it won't coincide with any top edge
\coordinate (rail) at ($(pid.north)!0.5!(fsm.north) + (0,7mm)$);
\draw[dashed,->] (llm.south) -- ++(0,-1.4) -| (rail);
\draw[dashed,->] (rail) -- (pid.north);
\draw[dashed,->] (rail) -- (fsm.north);
...
% EDA -> Telemetry（左から回り込み）
\draw[->] (eda.east) -- ++(0.9,0) |- (tele.west);

% --- FSM -> EDA constraints (three-step route: left, up, left, then down) ---
% 1) 少し左へ出してノード境界から離れる
\coordinate (foutL) at ($(fsm.west)+(-0.9,0)$);
% 2) 上に持ち上げて同一直線重なりを避ける
\coordinate (foutU) at ($(foutL)+(0,7mm)$);
% 3) EDA の真上まで水平に運び、下ろす
\coordinate (edaAbove) at ($(eda.north)+(0,6mm)$);
\draw[->] (fsm.west) -- (foutL) -- (foutU) -- ($(edaAbove)!1!(edaAbove)$) -- (eda.north);
\node[lab] at ($(foutU)!0.5!(edaAbove)+(0,0.35)$) {constraints};
