...
% ---- FSM -> EDA : EDA の下辺に接続 ----
\path let \p1=(eda.south) in
  coordinate (edaBottom) at (\x1,\y1-6mm); % EDA 下辺の少し下に中継点

% FSM から縦に降ろして → EDA 下辺に接続
\draw[->] (fsm.south) |- (edaBottom) -- (eda.south);

% ラベル配置（中継点の少し下）
\node[font=\scriptsize,below=2pt,align=center] at (edaBottom) {constraints};
