...
% === 参考文献の改ページ制御：必要に応じて番号を調整 ===
\IEEEtriggeratref{6} % ← References が途中で切れる場合は 5 や 7 に変更して微調整

\bibliographystyle{IEEEtran}
\bibliography{systemdk_aitl2025}

% ===== Fig.2 : 実験結果（最終ページの手前・下部に固定） =====
% 縦並び・同サイズ・(b) 凡例はグラフ下に配置し重なり防止
\begin{figure*}[!b]
  \centering
  % 共通サイズ（同一に揃える）
  \def\figW{0.68\textwidth}   % 横幅（少し狭め）
  \def\figH{0.42\textwidth}   % 縦（やや大きめ＝約1.5倍イメージ）
  ...
  \subfloat[Delay vs.\ time (No control / PID / PID+FSM+LLM).]{
    \begin{tikzpicture}
      \begin{axis}[
        ...
        legend style={draw=none,fill=none,at={(0.5,-0.2)},anchor=north}, % 下部中央に配置
