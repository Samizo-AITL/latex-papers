...
\usepackage{balance} % 参考文献の2段バランス
...
% ===== Fig.1 : Control Loop =====
% ● 途切れ防止：全幅figure*、高さは自動で \resizebox で調整
\begin{figure*}[!t]
\centering
...
\begin{figure*}[!t]
  ...
  \subfloat[Suppression vs.\ $k_c$ and $P_{\text{burst}}$ (FEM co-sim, synthetic).]{
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot3[
          surf, shader=interp, samples=35, samples y=30, domain=0:1, y domain=0:1
  ...
  \subfloat[Delay vs.\ time (No control / PID / PID+FSM+LLM).]{
    \begin{tikzpicture}
      \begin{axis}[
        width=0.82\textwidth, height=0.30\textwidth,
        xlabel={Time (s)},ylabel={Delay deviation (norm.)},
        xmin=0,xmax=1, ymin=0, ymax=1.05,
        legend columns=3,
        legend style={at={(0.5,-0.25)},anchor=north}, % ← 凡例をグラフの外・下に
        clip=false
      ]
...
% === 参考文献は必ず図の後に来るように強制 ===
\clearpage           % ← ここで全ての浮動体(Fig.2等)を吐き出させる
\balance             % ← 参考文献の段バランスを整える
