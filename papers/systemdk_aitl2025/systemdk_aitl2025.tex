...
% ---------- Safety helpers ----------
\newcommand{\placeholder}[1]{\textit{#1}} % visually mark placeholders
\makeatletter
\newcommand{\figinclude}[2][\linewidth]{%
  \IfFileExists{#2}{\includegraphics[width=#1]{#2}}{%
    \fbox{\parbox[c][3cm][c]{#1}{\centering Placeholder for \\ \texttt{#2}}}%
  }%
}
\makeatother
...
% NOTE: avoid raw Unicode like ★ or ≤ (use LaTeX macros)
...
\label{sec:setup}
\textbf{Designs:} two SoC blocks with \textbf{25} critical paths each \placeholder{(specify)}.\\
\textbf{Node:} \SI{14}{\nano\meter} FinFET \placeholder{(replace with actual node)}.\\
\textbf{Tools:} industrial synth/P\&R/STA; SI/EMI via S-params; FEM for thermal/stress.\\
\textbf{Sensors:} ring-osc delay, thermal diodes, on-die jitter meters; bandwidth $\leq\SI{100}{kHz}$ \placeholder{(specify)}.\\
...
\textbf{Runs:} thermal stress \textbf{30} runs, EMI stress \textbf{50} runs per scheme \placeholder{(specify)}.
...
\textbf{Statistic:} baseline variation $=\,$\meanpm{\placeholder{12.4}}{\placeholder{0.9}}\,ps.
DVFS \SI{8.7}{ps}, ABB \SI{7.9}{ps}, Throttle \SI{6.8}{ps}; PID \meanpm{\placeholder{2.1}}{\placeholder{0.3}}\,ps, PID+FSM \meanpm{\placeholder{1.9}}{\placeholder{0.3}}\,ps; $p<0.01$ vs. all baselines.\\
\textbf{EDA impact:} reduced timing guardbands $\Rightarrow$ improved utilization in STA closure.

\begin{figure}[t]
\centering
\figinclude[0.95\linewidth]{figs/sim_delay_rc.pdf}
...
\begin{figure}[t]
\centering
\figinclude[0.95\linewidth]{figs/sim_thermal_response.pdf}
...
\begin{figure}[t]
\centering
\figinclude[0.95\linewidth]{figs/sim_emi_jitter.pdf}
\caption{RMS jitter (ps) under 10\,mV\textsubscript{pp} aggressor, \SIrange{2}{10}{GHz}. Scope BW \SI{12}{GHz}, N=50, mean$\pm$SD.}
\label{fig:emi}
\end{figure}
...
\begin{figure}[t]
\centering
\figinclude[0.95\linewidth]{figs/fem_thermal_map.pdf}\\[3pt]
\figinclude[0.95\linewidth]{figs/fem_stress_map.pdf}
...
\begin{figure}[t]
\centering
\figinclude[0.95\linewidth]{figs/sparam_s11s21.pdf}
...
\textbf{Threats to Validity:} sensor bandwidth ($\leq\SI{100}{kHz}$) may miss fast noise; PID may saturate on rare corners; LLM adaptation risks mis-tuning. \textbf{Mitigations:} FSM SAFE state with widened guardbands and IRQ; staged application of LLM proposals (shadow $\rightarrow$ canary $\rightarrow$ fleet).
...
% --- Fill with real recent works (replace placeholders with actual refs + DOI) ---
