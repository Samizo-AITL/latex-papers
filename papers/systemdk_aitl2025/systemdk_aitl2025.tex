...
\draw[->] (pid.east)  -- ++(0.6,0) |- (fsm.north); % optional: PID → FSM hook
...
%--- EDA couplings (reworked) ----------------------------------
\draw[->] (fsm.west) to[out=180,in=0] (eda.east);         % FSM → EDA
\draw[->] (fem.west) to[out=180,in=270] (eda.south);      % FEM → EDA（新設）

%（旧）EDA→Telemetry の矢印は削除：流れの向きを「Telemetry/Models→PID/FSM→EDA」に統一
\end{tikzpicture}
