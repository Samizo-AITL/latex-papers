...
% --- LLM -> PID / FSM : 個別に点線で提案の意を表現 ---
\draw[dashed,->,shorten >=2pt,shorten <=2pt]
  (llm.south east) to[bend right=12] (pid.north);
\draw[dashed,->,shorten >=2pt,shorten <=2pt]
  (llm.south west)  to[bend left=12]  (fsm.north);

% --- 設定/監督系 ---
\draw[->,shorten >=1pt] (yaml.east) -- (pid.west);
% FSM -> EDA : 南側に回り込まず、右→左へ直接
\draw[->,shorten >=1pt] (fsm.west) to[out=180,in=0] (eda.east);
