...
% ---- Fig.1 tikz: drop-in replacement (no block-crossing) ----
\begin{figure*}[t]
\centering
\begin{tikzpicture}[
  font=\small,
  ...
  lab/.style={font=\scriptsize, inner sep=1pt, fill=white, draw=none},
  >={Latex[length=2mm]}
]
% --- Top: LLM (dashed) ---
\node[ghost] (llm) at (0,2.9) {LLM Analyzer (Next):\\retunes $K_p,K_i,K_d$; regenerates FSM rules};

% --- Upper row: Config -> PID -> FSM ---
\node[box] (yaml) at (-4.8,0.7) {YAML Config\\FW / CSR};
\node[box] (pid)  at (-0.8,0.7) {PID Controller};
\node[box] (fsm)  [right=19mm of pid] {FSM Supervisor};

% --- Vertical stack under PID ---
...
% --- EDA hook (left column) ---
\node[box] (eda)  [left=20mm of tele] {EDA Flow\\(Synth / P\&R / STA)};

% ========== Connections (no block crossing) ==========
% Config -> PID (short straight)
\draw[->] (yaml.east) -- (pid.west);

% LLM -> PID (dashed, via short vertical)
\draw[dashed,->] (llm.south) -- ++(0,-1.2) -| (pid.north);

% LLM -> FSM (dashed, via top rail so it never crosses blocks)
\draw[dashed,->] (llm.east) ++(0.2,0) -- ++(0.9,0) |- (fsm.north);

% Telemetry up to PID (straight)
\draw[->] (tele.north) -- (pid.south);

% Physics upflow (S-params -> FEM -> Telemetry)
\draw[->] (spar.north) -- (fem.south);
\draw[->] (fem.north)  -- (tele.south);

% EDA -> Telemetry (run-time measurements), routed from left
\draw[->] (eda.east) -- ++(0.9,0) |- (tele.west);

% FSM supervisory constraints back to EDA (routed on top edge, not across blocks)
\path let \p1=(fsm.west) in
      coordinate (tap) at (\x1-0.9,\y1);
\draw[->] (fsm.west) -- (tap) |- (eda.north);
\node[lab] at ($(tap)!0.5!(eda.north)+(0,0.35)$) {constraints};

% Small label for LLM outputs
\node[lab] at ($(pid.north)!0.5!(fsm.north)+(0,0.55)$) {gain \& threshold adapt};
