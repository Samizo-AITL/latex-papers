...
% ===== Fig.2 : 縦並び強調版（(a)(b) 同サイズ・大） =====
\begin{figure*}[!t]
  \centering

  % ---- (a) Heatmap（2D表示・全青対策：スケール明示） ----
  \subfloat[Suppression vs.\ $k_c$ and $P_{\text{burst}}$ (FEM co-sim, synthetic).]{
    \begin{tikzpicture}
      \begin{axis}[
        width=0.90\textwidth,height=0.34\textwidth,
        xlabel={$k_c$}, ylabel={$P_{\text{burst}}$ (W)},
        xmin=0, xmax=1, ymin=0.1, ymax=1.0,
        view={0}{90},          % ← 2Dヒートマップ
        colormap/viridis, colorbar,
        point meta min=5, point meta max=100, % ← カラースケール固定（飽和防止）
        enlargelimits=false
      ]
        % z = min(100, max(5, 100*(1 - 0.7*x - 0.3*(y-0.1)/0.9)))
        \addplot3[
          surf, shader=flat, samples=61, samples y=51,
          domain=0:1, y domain=0.1:1.0
        ] { min(100, max(5, 100*(1 - 0.7*x - 0.3*(y-0.1)/0.9))) };
      \end{axis}
    \end{tikzpicture}
    \label{fig:heatmap}
  }\\[2.5ex]

  % ---- (b) Delay vs time（No control も線/太線・同サイズ） ----
  \subfloat[Delay vs.\ time (No control / PID / PID+FSM+LLM).]{
    \begin{tikzpicture}
      \begin{axis}[
        ...
        % --- No control（線） ---
        \addplot+[very thick, mark=none] table[row sep=\\]{x y\\
          0 1.00\\ 0.1 0.90\\ 0.2 0.80\\ 0.3 0.85\\ 0.4 0.75\\ 0.5 0.80\\
  ...
  \subfloat[EMI-induced jitter suppression (normalized RMS).]{
    \begin{tikzpicture}
      \begin{axis}[
        width=0.55\textwidth,height=0.24\textwidth,
        ybar, bar width=18pt,
