...
% ---- FSM -> EDA : Telemetry の下を通す（重なり防止）----
% 調整用クリアランス（必要に応じて 3–6mm 程度で調整）
\def\arrowclear{4mm}

% Telemetry 下の水平ラインとその中点を作る
\path let \p1=(tele.south), \p2=(eda.east), \p3=(fsm.south) in
  coordinate (fsmeda) at (\x2,\y1-\arrowclear) % EDA 側の到達点（水平）
  coordinate (fsmh)   at (\x3,\y1-\arrowclear) % FSM 直下（水平開始点）
  coordinate (fsmmid) at ($(fsmh)!0.5!(fsmeda)$); % 水平区間の中点

% ルーティング（縦→水平→EDA）とラベル
\draw[->] (fsm.south) |- (fsmeda) -- (eda.east);
\node[font=\scriptsize,below=2pt,align=center] at (fsmmid) {constraints};
