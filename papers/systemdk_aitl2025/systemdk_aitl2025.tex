...
\centering
\begin{tikzpicture}[
  block/.style={draw,rounded corners,minimum width=20mm,minimum height=10mm,align=center,fill=black!5},
  sum/.style={circle,draw,inner sep=1pt},
  line/.style={-Latex,thick},
  dashedline/.style={-Latex,dashed,thick}
]
\node[sum] (sum) at (0,0) {};
\node[block,right=8mm of sum] (pid) {PID\\$(K_p,K_i)$};
\node[block,right=12mm of pid] (fsm) {FSM\\Safety/Throttle};
\node[block,right=12mm of fsm,minimum width=30mm] (plant) {CFET Thermal--RC Plant};
\node[block,above=12mm of fsm,minimum width=35mm] (llm) {LLM Supervisor};

\draw[line] (sum) -- node[above] {$e(t)$} (pid);
\draw[line] (pid) -- (fsm);
\draw[line] (fsm) -- (plant);
\draw[line] (plant.east) -- ++(10mm,0) coordinate (out);
\draw[line] (out) |- ($(sum.south)+(0,-8mm)$) -| node[pos=0.25,below] {Measured Delay $y(t)$} (sum.south);
\draw[line] ($(sum.west)+(-10mm,0)$) -- node[above] {Reference $r(t)$} (sum.west);
\draw[dashedline] (llm) -- (pid.north);
\draw[dashedline] (llm) -- (fsm.north);
\draw[dashedline] (llm) -| (plant.north);
\end{tikzpicture}
