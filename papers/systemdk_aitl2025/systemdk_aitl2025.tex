...
% ---- compact floats / captions (詰めて3ページに収める) ----
...
\pgfplotsset{
  every axis/.append style={
    ...
    grid=both, grid style={opacity=.15}
  }
}
...
traditional DTCO by embedding \emph{control-theoretic loops} directly into EDA flows.
...
FEM analysis (thermal, stress, EM) and S-parameter measurements are injected into synthesis,
P\&R, and STA to ensure physics-aware closure.
Proof-of-concept simulations demonstrate $>100\times$ reduction in delay deviation,
...
(iii) stress-driven mobility and $V_{th}$ shifts,
...
\section{Modeling}
The delay/thermal behavior of CFET interconnects are governed by resistive,
...
\begin{equation}
\Delta V_{th}(t)=\beta_{\mathrm{stress}}\sigma(t),\quad
\Delta \mu=-\gamma \sigma(t),\qquad
v_{emi}(t)=A\sin(2\pi f t).
\end{equation}
We sweep $f=10$--$200$~MHz.
...
\draw[line] (out) |- ($(sum.south)+(0,-10mm)$) -|
    node[pos=0.27,below] {Measured Delay $y(t)$} (sum.south);
...
\begin{figure*}[b]
  ...
  \subfloat[Suppression vs.\ $k_c$ and $P_{\text{burst}}$ (FEM co-sim, synthetic).]{
    \begin{tikzpicture}
      \begin{axis}[
        width=0.42\textwidth, height=0.32\textwidth,
        xlabel={$k_c$}, ylabel={$P_{\text{burst}}$ (W)},
        xmin=0, xmax=1, ymin=0.0, ymax=1.0,
        colormap/viridis, colorbar,
        point meta min=0, point meta max=100,
        enlargelimits=false
      ]
        \addplot3[
          surf, shader=interp, samples=33, samples y=28, domain=0:1, y domain=0:1
        ] { max(5, 100*(1 - 0.7*x - 0.3*y)) };
      \end{axis}
    \end{tikzpicture}
  }\hfill
  % (b) Delay vs time（凡例は外に）
  \subfloat[Delay vs.\ time (No control / PID / PID+FSM+LLM).]{
    \begin{tikzpicture}
      \begin{axis}[
        ...
        legend style={draw=none, fill=none, at={(0.5,-0.22)}, anchor=north, /tikz/every even column/.style={column sep=4pt}},
        legend columns=3
      ]
        % No control
        \addplot+[thick,mark=none] table[row sep=\\]{x y\\
          0 1.00\\ 0.1 0.90\\ 0.2 0.80\\ 0.3 0.86\\ 0.4 0.75\\ 0.5 0.81\\
  ...
  % (c) EMI jitter bar（同じ基準幅×2で中央）
  \par\medskip
  \subfloat[EMI-induced jitter suppression (normalized RMS).]{
    \begin{tikzpicture}
      \begin{axis}[
        width=0.86\textwidth, height=0.28\textwidth,
        ybar, bar width=14pt,
...
static sign-off $\to$ dynamic runtime closure;
guardbands $\to$ adaptive loops;
reliability $\to$ cross-domain resilience (delay, thermal, stress, EMI).
Future work:
