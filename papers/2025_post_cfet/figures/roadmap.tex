...
% --- enlarge bounding box to avoid clipping (very important) ---
\path[use as bounding box] (-1.6,-6.6) rectangle (16.3,1.0);
...
\node[lane, anchor=east] at (-0.25, -1.2) {Materials};
\node[lane, anchor=east] at (-0.25, -2.6) {Integration};
\node[lane, anchor=east] at (-0.25, -4.0) {Applications};
\node[lane, anchor=east] at (-0.25, -5.4) {EDA/PDK};
...
\node[bubble, above=2pt of m2] {Contact $R_c\!\downarrow$ ($\sim$0.5--1 k$\Omega\!\cdot\!\mu$m) \\ Large-area CVD uniformity $\pm$5\%};
