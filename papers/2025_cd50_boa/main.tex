...
\begin{figure*}[!t]
  ...
  \caption{LOCOS+STIハイブリッドのプロセス順序（2カラム全幅）：HV(LOCOS)先行 $\rightarrow$ LV(STI) 後続。熱履歴の影響と面積削減のトレードオフ、BoAの寄与を併記。}
...
\begin{tikzpicture}[scale=0.85, every node/.style={font=\footnotesize}]
  %==================== 上：従来 ====================
  \node[align=center] at (3,6.1) {従来構造 (Peripheral Bump)};
  % チップ外形（長辺Lは下の図と同じ）
  \draw (0,5.0) rectangle (6,5.8);
  \node at (3,5.4) {Active Area};
  % バンプ：短辺側（外周の上辺）に整列
  \foreach \x in {0.6,1.4,2.2,3.0,3.8,4.6,5.4} {
    \draw[fill=black] (\x,5.9) circle (0.07);
  }
  % 長辺L（一定）
  \draw[<->] (0,4.85) -- (6,4.85);
  \node at (3,4.70) {長辺 $L$（一定）};
  % 短辺 W_before
  \draw[<->] (-0.30,5.0) -- (-0.30,5.8);
  \node[rotate=90] at (-0.55,5.40) {$W_{\text{before}}$};

  %==================== 下：BoA =====================
  \node[align=center] at (3,3.80) {BoA構造 (Bump on Active)};
  % チップ外形（長辺Lは同一、短辺が小さい）
  \draw (0.5,3.0) rectangle (5.5,3.6);
  \node at (3,3.3) {Active Area};
  % バンプ：内側（枠のすぐ下）に寄せて配置
  \foreach \x in {0.8,1.6,2.4,3.2,4.0,4.8,5.2} {
    \draw[fill=black] (\x,3.55) circle (0.07);
  }
  % 長辺L（一定）
  \draw[<->] (0.5,2.85) -- (5.5,2.85);
  \node at (3,2.70) {長辺 $L$（一定）};
  % 短辺 W_after
  \draw[<->] (5.8,3.0) -- (5.8,3.6);
  \node[rotate=90] at (6.05,3.30) {$W_{\text{after}}$};

  % 短辺縮小を強調
  \draw[->,thick] (6.0,5.40) -- (6.0,3.30) node[midway,right]{短辺縮小};
\end{tikzpicture}
\caption{横長ドライバICの上面模式図。従来・BoAともに片辺バンプ配置だが、BoAでは外周Padを不要とし、バンプをアクティブ領域寄りに寄せることで短辺寸法が $W_{\text{before}}\!\to\!W_{\text{after}}$ に縮小する。長辺 $L$ は変わらない。}
\label{fig:boa_vertical}
\end{figure}
