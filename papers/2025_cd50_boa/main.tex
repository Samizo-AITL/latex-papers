...
このハイブリッド統合の工程順序と設計上の要点を整理したのが
表~\ref{tab:hybrid_flow}である。
LOCOSを先行してHVブロックを形成し、その後にSTIプロセスでLVロジックを構築する。
この順序により、高温熱処理を必要とするHV工程の熱履歴が後続LV素子へ与える影響を回避できる。
一方、STI導入による面積削減とBoAによるパッド領域縮小効果を組み合わせることで、
総合的なコストダウンを実現することを狙った。
...
\caption{LOCOS+STIハイブリッド統合工程と設計上の要点}
\label{tab:hybrid_flow}
\begin{tabular}{|c|l|l|l|}
\hline
区分 & 主工程・処理内容 & 主な条件・熱履歴 & 設計上の留意点 \\ \hline
\textbf{HV Block} &
フィールド酸化（LOCOS）／チャネルストップ形成／厚酸化膜成長 &
1050$^\circ$C級, 数時間 &
高耐圧特性を確保。微細化には不利。 \\ \hline

\textbf{熱履歴影響} &
高温酸化・ドライブインによる熱予算消費 &
総熱予算 $\approx 2.4\times10^5$ &
後続LV素子の拡散／$V_{\mathrm{th}}$変動に影響。 \\ \hline

\textbf{LV Logic} &
トレンチ形成→埋め込み酸化→CMP（STI） &
950$^\circ$C, 数十分 &
素子間隔縮小・平坦化。 \\ 
& 浅拡散・微細配線形成 & RTA 1030$^\circ$C, 10 s×2 &
高密度化・低耐圧素子。 \\ \hline

\textbf{後工程} &
メタル配線／パッシベーション／BoAバンプ形成 &
$\leq$ 400$^\circ$C &
機械的応力・実装信頼性に配慮。 \\ \hline

\textbf{設計上の要点} &
LOCOS高温→STI低温の順で統合 &
--- &
$V_{\mathrm{th}}$安定化・リーク抑制。 \\ 
& BoAによりパッド領域を削減 &
--- &
面積削減・コスト低減の主因。 \\ \hline
\end{tabular}
...
LOCOS工程を先行させることで高温酸化およびドライブイン処理を完了し，
後続のSTI・RTA工程を低熱履歴条件で実施できる。
もしSTIを先行させた場合，トレンチ酸化膜や浅拡散層が高温処理で再酸化・再拡散し，
素子分離特性やロジック部のしきい値電圧に悪影響を及ぼすことが知られている。
したがって，本プロセスでは「HV（高温）→LV（低温）」の順序を採ることが必然であった。
