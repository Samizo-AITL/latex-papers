...
LOCOSを先行して高温処理を完了させ、その後にSTIやRTAなど低熱処理プロセスを行うことで、
熱履歴の干渉を最小化することができる。
表~\ref{tab:thermal_process}に両者の主要熱工程を比較して示す。

\usepackage{tabularx} % preambleに追加

\begin{table}[t]
\centering
\caption{HV工程とLV工程における主要熱処理条件の比較}
\label{tab:thermal_process}
\renewcommand{\arraystretch}{1.3} % 行間広め
\setlength{\tabcolsep}{3pt}
\normalsize % 本文と同じサイズ
\begin{tabularx}{\linewidth}{|c|X|c|c|X|}
\hline
工程区分 & 主処理内容 & 温度[℃] & 時間[分] & 特徴・目的 \\
\hline
HV工程（LOCOS系） &
フィールド酸化膜形成（LOCOS）／チャネルストップ拡散 &
1050 &
180 &
厚膜酸化による高耐圧化。熱履歴最大で、後続工程への影響大。\\
\hline
 &
ソース・ドレイン拡散ドライブイン &
1000–1050 &
...
950 &
30 &
HVデバイス向け厚膜酸化。電界耐圧を確保。\\
\hline
LV工程（STI系） &
トレンチ形成／埋め込み酸化／CMP平坦化 &
900–950 &
30–60 &
低温プロセスで微細ロジック素子用。\\
\hline
...
≤400 &
--- &
最終工程。熱履歴を抑えて応力影響を最小化。\\
\hline
\end{tabularx}
\end{table}

上表から明らかなように、HV工程では1050$^\circ$C級の高温酸化・ドライブイン処理が複数存在し、
総熱予算が極めて大きい。
これを後段のLVロジックよりも先に実施することで、
トレンチ酸化膜や浅拡散領域の再酸化・再拡散を防ぎ、素子分離特性と$V_{\mathrm{th}}$安定性を確保できる。
したがって、統合順序は「HV（高温）→LV（低温）」が必然的に選択された。
