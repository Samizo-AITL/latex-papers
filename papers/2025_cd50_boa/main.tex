...
\begin{tikzpicture}[x=1cm,y=1cm, every node/.style={font=\scriptsize}]
  ...
  \node[anchor=west] at (7.05,-0.55) {n-well (30 V)};

  % --- STI トレンチ ---
  \fill[gray!40] (0.6,0) -- (0.6,-0.8) -- (1.2,-0.8) -- (1.2,0) -- cycle;
  \draw (0.6,0) -- (0.6,-0.8) -- (1.2,-0.8) -- (1.2,0);
  \node at (0.9,-0.95) {STI};

  % --- 表面 ---
  \draw[thick] (1.2,0) -- (6.6,0);

  % --- 厚いゲート酸化膜 ---
  \fill[green!30] (2.0,0) rectangle (6.0,0.08);
  \node[green!40!black,anchor=south] at (4.0,0.1) {厚ゲート酸化膜};

  % --- 端部シンニング (STI近傍) ---
  \fill[red!55] (1.2,0) -- (2.0,0) -- (2.0,0.08) -- (1.5,0.02) -- cycle;
  \node[red!70!black,anchor=south west] at (1.25,0.08) {端部シンニング};

  % --- Polyゲート電極 ---
  \fill[gray!20] (1.6,0.08) rectangle (6.4,0.35);
  \draw (1.6,0.08) rectangle (6.4,0.35);
  \node at (4.0,0.45) {Polyゲート (0 V)};

  % --- 高電界集中領域 ---
  \fill[red!15,draw=red!60,dashed] (1.2,0) -- (1.7,0) -- (1.7,-0.15) -- (1.3,-0.15) -- cycle;
  \node[red!70!black,anchor=north west] at (1.75,-0.02) {高電界集中};

  % --- リーク矢印 (n-well→Poly) ---
  \draw[->,very thick,red!80] (1.5,-0.2) to[out=90,in=-90] (1.5,0.08);
  \node[red!80,anchor=west] at (1.55,-0.05) {リーク};

\end{tikzpicture}
\caption{STI端部シンニングによるゲートリーク模式断面（n-well 30 V → Poly 0 V）}
\label{fig:hvmos_sti_leak}
\end{figure}
