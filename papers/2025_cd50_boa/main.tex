...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners, align=left, inner sep=3pt, minimum width=0.88\linewidth},
    arrow/.style={-{Stealth}, thick},
    note/.style={font=\footnotesize, align=left}
  ]

  % Nodes
  \node[box, fill=gray!10] (hv) {%
    \textbf{HV Block (30\,V) — LOCOS first}\\
    フィールド酸化／チャネルストップ／厚酸化膜\\
    長チャネル・オフセット構造
  };

  \node[box, fill=gray!10, below=of hv] (therm) {%
    \textbf{大熱履歴}\\
    高温酸化・ドライブインによる熱予算消費
  };

  \node[box, fill=gray!10, below=of therm] (lv) {%
    \textbf{LV Logic (1.8\,V) — STI later}\\
    トレンチ形成→埋め込み酸化→CMP\\
    浅拡散・微細配線
  };

  \node[box, fill=gray!10, below=of lv] (finish) {%
    \textbf{後工程}\\
  ...
  % Side notes
  \node[note, right=8mm of therm] (risk) {%
    \textit{注意:} LOCOSの熱履歴が\\
    後続LV素子の$V_{\mathrm{th}}$・リークに影響\\
    $\Rightarrow$ 統合最適化・設計マージンが必要
  };
  \draw[arrow] (therm.east) -- (risk.west);

  \node[note, left=8mm of lv] (benefit) {%
    面積削減（STI）\\
    $\Rightarrow$ ダイ縮小・コスト低減
  };
  \draw[arrow] (benefit.east) -- (lv.west);

  \node[note, right=8mm of finish] (boa) {%
    BoAによりパッド領域縮小\\
    $\Rightarrow$ コストダウンの主役
  };
  \draw[arrow] (finish.east) -- (boa.west);

  \end{tikzpicture}
  \caption{LOCOS+STIハイブリッドのプロセス順序：HV(LOCOS)先行 $\rightarrow$ LV(STI) 後続。熱履歴と面積削減のトレードオフを示す。}
