...
\begin{tikzpicture}[x=1cm,y=1cm, every node/.style={font=\scriptsize}]
  % ---- n-well (HV PMOS region)
  \fill[blue!10] (0,-1.0) rectangle (7,0); \draw (0,-1.0) rectangle (7,0);
  \node[anchor=west] at (7.05,-0.55) {n-well（HV PMOS）};

  % ---- STI oxide (left)
  \fill[gray!40] (0.4,0) -- (0.4,-0.9) -- (1.0,-0.9) -- (1.0,0) -- cycle;
  \draw (0.4,0) -- (0.4,-0.9) -- (1.0,-0.9) -- (1.0,0);
  \node at (0.7,-0.98) {STI};

  % ---- active surface
  \draw[thick] (1.0,0) -- (6.6,0);

  % ---- thick gate oxide (HV)
  \fill[green!35] (2.0,0) rectangle (5.0,0.08);
  \node[green!40!black,anchor=south] at (3.5,0.09) {厚ゲート酸化膜};

  % ---- thin oxide wedge at STI edge
  \fill[red!55] (1.0,0) -- (2.0,0) -- (2.0,0.08) -- (1.35,0.02) -- (1.0,0) -- cycle;
  \node[red!70!black,anchor=south west] at (1.05,0.08) {端部シンニング};

  % ---- poly gate (simple block)
  \fill[gray!20] (1.6,0.08) rectangle (5.4,0.35);
  \draw (1.6,0.08) rectangle (5.4,0.35);
  \node at (3.5,0.43) {Gate（ポリシリコン）};

  % ---- p+ S/D (simple)
  \fill[pink!60] (1.1,-0.05) rectangle (2.0,-0.35);
  \fill[pink!60] (5.0,-0.05) rectangle (6.2,-0.35);
  \draw (1.1,-0.05) rectangle (2.0,-0.35);
  \draw (5.0,-0.05) rectangle (6.2,-0.35);
  \node at (1.55,-0.45) {p$^{+}$ Source};
  \node at (5.6,-0.45) {p$^{+}$ Drain};

  % ---- channel (just a label)
  \node at (3.5,-0.20) {チャネル};

  % ---- high-field region (hatched)
  \fill[red!15,draw=red!60,dashed] (1.0,0) -- (1.6,0) -- (1.6,-0.15) -- (1.1,-0.15) -- cycle;
  \node[red!70!black,anchor=north west] at (1.62,-0.02) {高電界集中};

  % ---- leakage path (bold red arrow)
  \draw[->,very thick,red!80] (1.5,0.30) to[out=-90,in=90] (1.45,0.02);
  \node[red!80,anchor=west] at (1.55,0.22) {ゲートリーク経路};

  % ---- HV note
  \node at (3.5,0.75) {30 V級 HV PMOS（模式図）};
\end{tikzpicture}
\caption{HV PMOSにおけるSTI端部シンニングとゲートリーク（極力簡略化した模式断面）}
\label{fig:sti_hvpmos_simple}
\end{figure}
