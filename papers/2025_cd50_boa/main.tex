...
\begin{figure}[t]
\centering
\begin{tikzpicture}[x=1.4cm,y=1.4cm, every node/.style={font=\scriptsize}]

  %=== クリップ（左=STI左辺, 右=Poly中心より少し右） ===
  \begin{scope}
    \clip (0,-1.10) rectangle (3.6,0.95);

    %--- n-well (30 V) ---
    \fill[blue!10] (0,-1.00) rectangle (7,0);
    \draw (0,-1.00) rectangle (7,0);

    %--- STI trench (左端がx=0) ---
    \fill[gray!40] (0,0) -- (0,-0.90) -- (0.60,-0.90) -- (0.60,0) -- cycle;
    \draw (0,0) -- (0,-0.90) -- (0.60,-0.90) -- (0.60,0);
    % \node at (0.30,-0.98) {STI};

    %--- 表面ライン ---
    \draw[thick] (0.60,0) -- (6.6,0);

    %--- ゲート酸化膜：通常厚（STI端部まで延長開始 x=0.60） ---
    \fill[green!35] (0.60,0) rectangle (3.20,0.10);

    %--- ゲート酸化膜：STI端部の薄膜表示（色を薄く＋高さを低く） ---
    %   0.60〜1.10の区間だけ薄く＆低く上書き
    \fill[green!15] (0.60,0) -- (1.10,0) -- (1.10,0.04) -- (0.60,0.04) -- cycle;

    %--- Polyゲート（STI左辺まで延長：x=0〜） ---
    \fill[gray!20] (0.00,0.10) rectangle (3.20,0.38);
    \draw (0.00,0.10) rectangle (3.20,0.38);

    %--- 高電界集中域（STI端すぐ横を強調）---
    \fill[red!15,draw=red!70,dashed] (0.60,0) -- (1.05,0) -- (1.05,-0.18) -- (0.70,-0.18) -- cycle;
  ...
  \node[anchor=east] at (3.55,0.46) {Poly = 0 V};

  \draw[->,red!80] (1.45,0.72) -- (0.88,0.055);
  \node[red!80,anchor=west] at (1.47,0.72) {端部シンニング（薄膜部）};

  \draw[->,red!80] (1.48,-0.62) -- (0.86,-0.09);
  \node[red!80,anchor=west] at (1.50,-0.62) {高電界集中};

  \draw[->,red!80] (0.25,-0.55) -- (0.88,-0.02);
  \node[red!80,anchor=west] at (0.02,-0.55) {リーク（n-well $\rightarrow$ Poly）};

  \node[anchor=north west] at (0.02,0.90) {STI 左辺};
  \draw[densely dotted] (3.20,-1.10) -- (3.20,0.95);
  \node[anchor=west] at (3.22,0.70) {Poly 中央};

\end{tikzpicture}
\caption{STI左辺からPoly中央までを拡大。ゲート膜はSTI端部まで延長し、端部の一部を薄膜として表示。PolyはSTI左辺まで延長。}
\label{fig:sti_edge_zoom_final}
\end{figure}
