...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=0.8]
    % 左側：従来
    \node at (-2,1.2) {従来構造};
    \draw[thick] (-3,0) rectangle (-1,1); % active領域
    \node at (-2,0.5) {Active};
    \draw[fill=gray!40] (-3,-0.4) rectangle (-1,0); % Pad領域
    \node at (-2,-0.2) {Pad};
    \draw[fill=black] (-2,-0.8) circle (0.2); % bump
    \node at (-2,-1.2) {Bump};

    % 右側：BoA
    \node at (2,1.2) {BoA構造};
    \draw[thick] (1,0) rectangle (3,1); % active領域
    \node at (2,0.5) {Active};
    \draw[fill=black] (2,1.3) circle (0.2); % bump on active
    \node at (2,1.7) {Bump};
  \end{tikzpicture}
  \caption{従来構造とBoA構造の比較：Pad領域削減によりダイ縮小を実現}
  \label{fig:boa_concept}
\end{figure}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Bump Pad測定における接触抵抗のばらつき：研磨により安定化}
