...
% --- 従来構造 ---
\node at (0,4.5) {従来構造 (Peripheral Bump)};
\draw (0,0) rectangle (4,4); % chip outline
\node at (2,2) {Active Area}; % active area text

% bumps along periphery
\foreach \x in {0.5,1.5,2.5,3.5} {
  \draw[fill=black] (\x,4.2) circle (0.12);
  \draw[fill=black] (\x,-0.2) circle (0.12);
}
\foreach \y in {0.5,1.5,2.5,3.5} {
  \draw[fill=black] (-0.2,\y) circle (0.12);
  \draw[fill=black] (4.2,\y) circle (0.12);
}

% --- BoA構造 ---
\node at (8,4.5) {BoA構造 (Bump on Active)};
\draw (6,0) rectangle (10,4); % chip outline
\node at (8,2) {Active Area}; % active area text

% bumps inside active
\foreach \x in {6.7,7.5,8.3,9.1} {
  \foreach \y in {0.7,1.5,2.3,3.1} {
    \draw[fill=black] (\x,\y) circle (0.12);
  }
}
