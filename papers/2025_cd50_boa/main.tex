...
\begin{tikzpicture}[x=1.5cm,y=1.5cm, every node/.style={font=\scriptsize}]
  ...
  % n-well
  \fill[blue!10,draw=none] (0,-1.00) rectangle (3.2,0);

  % STI
  \fill[gray!40,draw=none] (0,0) -- (0,-0.90) -- (0.60,-0.90) -- (0.60,0) -- cycle;

  % 薄ゲート膜
  \fill[green!18,draw=none] (0.60,0) rectangle (1.10,0.04);
  % 通常ゲート膜
  \fill[green!35,draw=none] (1.10,0) rectangle (3.20,0.10);

  % Poly（連続膜・段付き底面・枠線なし）
  \fill[top color=gray!10,bottom color=gray!30,draw=none]
    (0.00,0.00) -- (0.60,0.00) -- (1.10,0.04) -- (3.20,0.10)
    -- (3.20,0.38) -- (0.00,0.38) -- cycle;
  ...
  % --- 最小限のラベル ---
  \node[anchor=north west] at (0.02,0.88) {STI};
  \node[anchor=east] at (3.18,0.42) {Poly};
  \node[anchor=west] at (0.02,-0.95) {n-well};
  \node[red!80,anchor=west] at (1.05,-0.15) {\small リーク};
\end{tikzpicture}
\caption{STI端部におけるゲート酸化膜シンニングとリーク経路模式図。PolyはSTI上から通常酸化膜上まで連続膜。}
\label{fig:sti_edge_minimal}
\end{figure}
