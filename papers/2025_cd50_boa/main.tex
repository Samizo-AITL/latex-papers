...
LOCOS+STIハイブリッドではプロセス順序が本質的に重要である。
まず、熱履歴が大きい30\,V級HVデバイスをLOCOSプロセス（フィールド酸化、チャネルストップ、長チャネル形成 等）で先行形成し、
その後に1.8\,V級ロジックのSTIプロセス（トレンチ形成・埋め込み酸化・CMP・浅拡散 等）を適用する。
この順序により、HV素子に必要な厚酸化膜やオフセット構造を確保しつつ、ロジック部には微細STI素子を導入できる。
一方で、LOCOSの高温熱履歴がロジック素子のしきい値やリークに影響を与えるリスクがあり、工程統合・歩留まりの面で追加の最適化が不可欠となった。

\begin{figure*}[t]
  \centering
  \begin{tikzpicture}[node distance=1.5cm, font=\small]
    % HV Block
    \node (hv) [draw, rectangle, thick, text centered, minimum width=4cm, minimum height=1cm] 
      {HV Block (30V) -- LOCOS first \\ フィールド酸化・チャネルストップ・厚酸化膜構造};

    % 大規模展開
    \node (large) [below of=hv, draw, rectangle, thick, text centered, minimum width=4cm, minimum height=1cm] 
      {大規模展開 \\ 高耐圧駆動・ドライバIC量産};

    % LV Logic (STI)
    \node (lv) [below of=large, draw, rectangle, thick, text centered, minimum width=4cm, minimum height=1cm] 
      {LV Logic (1.8V) -- STI later \\ トレンチ形成・埋込み酸化膜・浅溝隔離};

    % 後工程
    \node (back) [below of=lv, draw, rectangle, thick, text centered, minimum width=4cm, minimum height=1cm] 
      {後工程 \\ メタル配線・バンプパッケージション};
    ...
    % 左にSTIのメリット
    \node (note) [left=4cm of lv, text width=3.5cm] 
      {面積削減 (STI) \\ ⇒ ダイ縮小・コスト低減};

    \draw[->, dashed] (note.east) -- (lv.west);
  \end{tikzpicture}
  \caption{LOCOS+STI ハイブリッドプロセスの流れ: HV(LOCOS)先行→LV(STI)後続。高耐圧を維持しつつ微細化・低コスト化を狙った。}
  \label{fig:locos-sti-hybrid}
\end{figure*}
