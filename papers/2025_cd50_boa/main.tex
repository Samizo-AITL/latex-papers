...
% preamble で \usepackage{tikz} を読み込み済みであること

\begin{figure}[t]
\centering
\begin{tikzpicture}[scale=0.85, every node/.style={font=\footnotesize}]
  %==================== 上：従来 ====================
  \node[align=center] at (3,5.85) {従来構造 (Peripheral Bump)};
  % チップ外形（長辺Lは下の図と同じ）
  ...
  % 短辺縮小を強調
  \draw[->,thick] (6.30,5.40) -- (6.30,3.30) node[midway,right]{短辺縮小};
\end{tikzpicture}
\caption{横長ドライバICの上面模式図。BoA適用により長辺 $L$ は変えず、短辺が $W_{\text{before}}\!\to\!W_{\text{after}}$ に縮小する。外周パッドリングが不要になり、バンプをアクティブ領域上へ再配置できるため。}
