...
\begin{tikzpicture}[scale=0.85, every node/.style={font=\footnotesize}]
  %----- 従来（Peripheral Bump）: 上 -----
  \node[align=center] at (3,5.3) {従来構造 (Peripheral Bump)};
  % チップ外形
  \draw (0,4.6) rectangle (6,5.2);
  \node at (3,4.9) {Active Area};
  % バンプ：短辺側（上辺）に並ぶ
  \foreach \x in {0.6,1.4,2.2,3.0,3.8,4.6,5.4} {
    \draw[fill=black] (\x,5.3) circle (0.07);
  }
  % 短辺寸法 before
  \draw[<->] (-0.3,4.6)--(-0.3,5.2);
  \node[rotate=90] at (-0.55,4.9) {$W_{\text{before}}$};

  %----- BoA (Bump on Active): 下 -----
  \node[align=center] at (3,3.7) {BoA構造 (Bump on Active)};
  % チップ外形：短辺を縮小
  \draw (0.4,3.0) rectangle (5.6,3.6);
  ...
  % 短辺寸法 after
  \draw[<->] (6.0,3.0)--(6.0,3.6);
  \node[rotate=90] at (6.25,3.3) {$W_{\text{after}}$};

  % 縮小を矢印で明示
  \draw[->,thick] (3,4.55) -- (3,3.65) node[midway,right] {短辺縮小};

\end{tikzpicture}
\caption{ドライバICの上面模式図。従来は短辺側にバンプを並べる必要があり、幅が大きくなる。BoAではアクティブ領域上に配置可能となり、外周パッドリングが不要となって短辺が縮小する。}
