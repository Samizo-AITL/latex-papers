...
\begin{tikzpicture}[x=1.5cm,y=1.5cm, every node/.style={font=\scriptsize}]
  ...
  % ゲート酸化膜（薄膜＋通常膜）
  \fill[green!20,draw=none] (0.6,0.0) rectangle (1.1,0.04); % シンニング部
  \fill[green!35,draw=none] (1.1,0.0) rectangle (3.2,0.10); % 通常膜厚
  \node[black] at (1.6,0.08) {Gate Oxide}; % ← 下方向に移動（0.12→0.08）

  % Polyゲート（3段階：STI上／薄膜上／通常酸化膜上）
  \fill[gray!25,draw=none] (0.0,0.0) rectangle (0.6,0.22);  % STI上
  \fill[gray!25,draw=none] (0.6,0.04) rectangle (1.1,0.28); % シンニング上
  \fill[gray!25,draw=none] (1.1,0.10) rectangle (3.2,0.38); % 通常膜上
  \node at (1.6,0.24) {Poly};

  % Leakage 矢印（下方向に文字を移動）
  \draw[->,very thick,red!80] (0.9,-0.25) -- (0.9,0.1);
  \node[red!80,anchor=west] at (1.05,-0.12) {\small Leakage}; % ← 下方向に移動（0.0→-0.12）

  \end{scope}
\end{tikzpicture}
\caption{STI端部におけるゲート酸化膜シンニングとリーク経路。PolyはSTI上／薄膜上／通常膜上で段差構造を形成し、各ラベルは中央に配置。}
\label{fig:sti_edge_leakage_gateoxide_final}
\end{figure}
