...
\begin{tikzpicture}[x=1.5cm,y=1.5cm, every node/.style={font=\scriptsize}]
  ...
  \begin{scope}
    ...
    % --- Poly ゲート（連続膜：STI上も含めて一体で塗る／境界線なし） ---
    \fill[top color=gray!10,bottom color=gray!30,draw=none]
      (0.00,0.10) -- (3.20,0.10) -- (3.20,0.38) -- (0.00,0.38) -- cycle;
  ...
  \draw[->,red!80] (2.58,0.72) -- (1.00,0.03);
  \node[red!80,anchor=west] at (2.60,0.72) {端部シンニング（薄いゲート膜）};

  \draw[->] (2.58,0.58) -- (2.20,0.09);
  \node[anchor=west] at (2.60,0.58) {通常ゲート膜厚};

  \draw[->,red!80] (2.58,-0.62) -- (0.88,-0.10);
  \node[red!80,anchor=west] at (2.60,-0.62) {高電界集中};

  \draw[->,red!80] (0.20,-0.55) -- (0.88,-0.02);
  \node[red!80,anchor=west] at (0.02,-0.55) {リーク（n-well $\rightarrow$ Poly）};

  % 右端 = Poly右端（＝n-well右端）の目安線（必要なら削除可）
  \draw[densely dotted] (3.20,-1.10) -- (3.20,0.95);
\end{tikzpicture}
\caption{STI左辺からPoly右端までを拡大。PolyはSTI上～薄膜ゲート上～通常ゲート上を連続膜として成膜。ゲート膜は端部の薄膜区間と通常膜厚の二段階。}
\label{fig:sti_edge_poly_continuous}
\end{figure}
