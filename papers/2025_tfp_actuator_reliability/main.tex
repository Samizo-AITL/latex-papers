...
\@ifundefined{figTrapezoidScreening}{%
  \newcommand{\figTrapezoidScreening}{%
    \begin{tikzpicture}[x=1cm,y=0.08cm,>=Stealth,thick]
      ...
      % ★ COM：天井（30V）の黒いCOM波形そのものへ矢印が届くようにする
      \node[anchor=south] at (\tHoldEnd-0.2,\VH+6.5) {\small COM};
      \draw[-{Stealth[length=2mm]}] (\tHoldEnd-0.2,\VH+6.1) -- (\tHoldEnd-0.2,\VH); % ← 波形に着地

      % VBS：右下余白→ -10Vラインへ
      \node[anchor=west] at (\tMax-0.8,\VBS-6.5) {\small VBS};
      \draw[-{Stealth[length=2mm]}] (\tMax-0.1,\VBS-6.1) -- (\tMax-0.1,\VBS+0.0);
    \end{tikzpicture}%
  }%
}{}
\makeatother
