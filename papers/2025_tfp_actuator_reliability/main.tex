...
\renewcommand{\figEdgeBurnoutInline}{%
  \begin{tikzpicture}[x=0.9cm,y=0.9cm]
    ...
    \def\pzt{1.20}   % PZT厚
    \def\te{0.18}    % 上電極(TE)厚
    \def\xsl{4.20}   % PZTスリット左壁のx
    \def\gap{0.25}   % スリット幅

    % BE（下部電極）
    \fill[gray!55] (0,0) rectangle (\W,\be);

    % PZT層（スリットで左右に分割）
    \fill[gray!15] (0,\be) rectangle (\xsl,\be+\pzt);            % 左：駆動部
    \fill[gray!15] (\xsl+\gap,\be) rectangle (\W,\be+\pzt);      % 右：配線側

    % スリット側壁（電界集中部）
    \draw[line width=1.0pt] (\xsl,\be) -- (\xsl,\be+\pzt);
    \draw[line width=1.0pt] (\xsl+\gap,\be) -- (\xsl+\gap,\be+\pzt);

    % 上電極 TE（左・駆動部のみ）
    \fill[gray!80] (0.40,\be+\pzt) rectangle (3.60,\be+\pzt+\te);

    % 右側：BEへ落とすコンタクト柱（2本例）
    \fill[gray!70] (5.20,\be) rectangle (5.80,\be+\pzt);
    \fill[gray!70] (7.20,\be) rectangle (7.80,\be+\pzt);

    % Au配線パッド（COM）＝柱をブリッジ
    \fill[gray!30] (4.80,\be+\pzt) rectangle (8.20,\be+\pzt+0.60);

    % ラベル（控えめ）
    \node[anchor=east] at (0.32,0.08)             {\small BE};
    \node[anchor=east] at (0.32,\be+0.60)         {\small PZT};
    \node[anchor=east] at (0.32,\be+\pzt+0.02)    {\small TE};
    \node[anchor=east] at (0.32,\be+\pzt+\te/2)   {\small VBS};
    \draw (8.00,\be+\pzt+0.60) -- (8.00,\be+\pzt+1.00);
    \node[anchor=west] at (8.05,\be+\pzt+0.80)    {\small COM};

    % 焼損箇所の指示（スリット左壁）
    \draw[->,line width=0.6pt]
      (\xsl+0.20,\be+0.18) .. controls (\xsl+0.32,\be+0.70) ..
      (\xsl+0.02,\be+0.95);
    \node[anchor=west] at (\xsl+0.24,\be+0.14) {\scriptsize 焼損};

    % 参考：AlOx保護提案（破線で側壁に薄く表示）
    \draw[gray!70,dashed,line width=0.6pt] (\xsl-0.15,\be) -- (\xsl-0.15,\be+\pzt);
    \draw[gray!70,dashed,line width=0.6pt] (\xsl+\gap+0.15,\be) -- (\xsl+\gap+0.15,\be+\pzt);
  \end{tikzpicture}%
}
