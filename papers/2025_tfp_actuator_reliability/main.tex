...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      % ---- Before (No pre-wet, Nlots=12) ----
      \addplot+[fill=blue!30,draw=blue!60] coordinates {
        (7,2) (8,3) (9,3) (10,2) (11,2)  % 7–12%分布（クラック＋焼損）
      };

      % ---- After (Acetic pre-wet, Nlots=12) ----
      \addplot+[fill=red!30,draw=red!60] coordinates {
        (2,10) (3,2)  % 焼損2%付近に集中
      };

      \legend{Before (No pre-wet, $N_{\text{lots}}=12$), After (Acetic pre-wet, $N_{\text{lots}}=12$)}
    \end{axis}
  \end{tikzpicture}
  \caption{%
    ユニットスクリーニング不良率のヒストグラム（Before/After，共通）／
