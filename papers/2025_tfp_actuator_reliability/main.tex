...
This study analyzes the causes and proposes countermeasures for two major reliability issues observed in mass production of Epson $\mu$TFP thin-film PZT ($d_{33}$ mode) actuators:  
(1) diaphragm cracks and (2) segment edge burnout.  
For the cracks, wafer-level screening revealed concentric defect distribution, and cross-sectional analysis identified voids localized within specific PZT layers. These were attributed to surface hydrophobicity induced by residual airborne contaminants after RTA, leading to bubble entrapment during sol–gel spin coating. Introducing an acetic-acid pre-wet step restored surface hydrophilicity, suppressed bubble incorporation, and reduced the defect rate from 5–10\% to nearly zero, establishing stable mass-production conditions.

For edge burnout, failures were found at exposed PZT sidewalls where the COM bottom electrode (via Au wiring) and VBS top electrode are in closest proximity. A potential difference up to \SI{40}{V}, combined with transient currents during trapezoidal wave rise, was concluded to cause local insulation breakdown. As a permanent solution, sidewall passivation by AlOx films using atomic layer deposition (ALD) was proposed. While requiring new equipment investment, this provides a strategic pathway toward reliable high-voltage operation.

The findings elucidate defect mechanisms unique to thin-film PZT actuators, demonstrate a permanent process-based countermeasure, and propose a design-oriented solution for structural limitations. These results contribute to achieving higher nozzle density and improved high-voltage reliability in future MEMS inkjet devices.
\end{abstract}
