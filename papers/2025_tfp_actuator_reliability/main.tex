...
% ===== Fig.3: 端部焼損模式図（スケッチ準拠） =====
\newcommand{\figEdgeBurnoutSketch}{%
  \begin{tikzpicture}[x=0.8cm,y=0.8cm]

    % --- パラメータ ---
    \def\W{8}   % 全体幅
    \def\pztH{1.2} % PZT層厚み（スケール用）
    \def\beH{0.3}  % 下電極厚み
    \def\teH{0.2}  % 上電極厚み
    \def\auW{2.2}  % Au配線の幅
    \def\auH{0.8}  % Au配線の厚み
    ...
    \node[anchor=east,font=\scriptsize] at (-0.1,0.15) {BE};

    % --- PZT層 ---
    \fill[gray!15] (0,\beH) rectangle (\W,\beH+\pztH);
    \node[anchor=east,font=\scriptsize] at (-0.1,\beH+0.6) {PZT};

    % --- Top Electrode (TE, 左側のみ) ---
    \fill[gray!75] (0,\beH+\pztH) rectangle (3,\beH+\pztH+\teH);
    \node[anchor=east,font=\scriptsize] at (-0.1,\beH+\pztH+0.1) {TE};

    % --- Au配線（右側） ---
    \fill[gray!35] (4,\beH+\pztH) rectangle (4+\auW,\beH+\pztH+\auH);
    \node[anchor=west,font=\scriptsize] at (6.3,\beH+\pztH+0.4) {Au配線};

    % --- COM電極 (上に伸ばす) ---
    \draw[line width=0.5pt] (5,\beH+\pztH+\auH) -- ++(0,0.8);
    \node[anchor=west,font=\scriptsize] at (5.1,\beH+\pztH+\auH+0.6) {COM};

    % --- VBS電極 (左TEから上に伸ばす) ---
    \draw[line width=0.5pt] (1.5,\beH+\pztH+\teH) -- ++(0,0.8);
    \node[anchor=west,font=\scriptsize] at (1.6,\beH+\pztH+\teH+0.6) {VBS};

    % --- 寸法線（PZT厚み 1.2µm） ---
    \draw[<->,line width=0.4pt] (-0.6,\beH) -- (-0.6,\beH+\pztH);
    \node[anchor=east,font=\scriptsize] at (-0.7,\beH+0.6) {1.2 µm};
