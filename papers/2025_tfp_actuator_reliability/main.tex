...
\@ifundefined{figEdgeBurnoutInline}{%
  \newcommand{\figEdgeBurnoutInline}{%
    \begin{tikzpicture}[x=0.9cm,y=0.9cm]
      ...
      \def\xPZT{4.0}   % PZT左ブロックの右端(=側壁位置)
      \def\xGapR{6.0}  % スリット右端（PZTが無い領域の右端）
      % ========= 下地〜層 =========
      % 下部電極 BE
      ...
      % 右側は PZT を設けずスリット（4.0〜6.0 は空白）
      % COM 側 Au 配線（上面パッド）
      \fill[gray!30] (\xGapR,\beH+\pztH) rectangle (\W,\beH+\pztH+0.60);
      % COM→BE のコンタクト柱（2本）
      \fill[gray!30] (6.3,\beH) rectangle (6.9,\beH+\pztH);
      \fill[gray!30] (8.4,\beH) rectangle (9.0,\beH+\pztH);
