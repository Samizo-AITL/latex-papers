...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      ymajorgrids=true,
      grid style={gray!30},
      legend style={draw=none, at={(0.5,-0.25)}, anchor=north, legend columns=-1},
    ]
      % ---- Before (No pre-wet) ----
      \addplot+[fill=blue!40] coordinates {
        (6,2) (7,2) (8,3) (9,2) (10,3)
      };
      \addlegendentry{Before（前処理なし）\\Before (No pre-wet)}

      % ---- After (Acetic pre-wet) ----
      \addplot+[fill=red!40] coordinates {
        (0,11) (2,1)
      };
      \addlegendentry{After（酢酸プレウェット）\\After (Acetic pre-wet)}
    \end{axis}
  \end{tikzpicture}
  \caption{%
...
\begin{table}[t]
  ...
  \setlength{\tabcolsep}{3pt} % ← 列幅調整
  \begin{tabular}{@{} p{0.33\columnwidth} p{0.27\columnwidth} p{0.27\columnwidth} @{}}
    \toprule
