...
\begin{tikzpicture}[x=1cm,y=0.08cm,>=Stealth,thick]
  ...
  % -------- COM 急峻スロープ（黒）--------
  % 区間：基準(10V) -> 急峻立上り -> 高位保持 -> 立下り -> 戻り(10V)
  \draw[semithick]
    (\t0,\V0) -- (\tpre,\V0)                          % プリ区間
    -- (\tRiseEndFast,\VH)                             % 急峻立上り
    -- (\tHoldEnd,\VH)                                 % 保持
    -- (\tFallEnd,\V0)                                 % 立下り
    -- (\tMax,\V0);                                    % 終了水平
...
%   \caption{台形波形の概念図：立上り勾配を緩和すると
%   $I=C\,\mathrm{d}V/\mathrm{d}t$ が低下し、瞬間電流ピークが抑制される}
% \end{figure}
