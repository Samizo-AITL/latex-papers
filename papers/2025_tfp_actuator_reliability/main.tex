...
EDXでは\emph{炭素由来成分}が検出された。
これらはRTA後の外気吸着により表面が疎水化し，
スピン塗布時に気泡が巻き込まれたことに起因すると推定される。

さらに，RTA装置のロードロック内に長時間滞留させたウエハを用い，
PZT塗布・焼成後に反射条件を最適化した表面検査を行ったところ，
PZT内部ボイドに起因すると考えられる\emph{同心円状の欠陥分布}が観測された。
...
\begin{figure}[t]
  ...
  \caption{%
    ...
    この層内欠陥が膜厚方向に応力集中を誘発し，クラック進展の起点となる。
...
\subsection{原因仮説}
RTA後の外気暴露により表面が疎水化し，
その結果スピン塗布時に気泡が巻き込まれて層内ボイドが形成。
焼成時にボイド周辺で応力集中が生じ，膜厚方向にクラックが進展する。
すなわち，外気由来汚染と親水性低下が起点である。

\subsection{対策立案と実施}
対策として以下を実施した：
\begin{itemize}
  \item 酢酸2\%による\emph{プレウェット処理(30\,s)}をRTA直後に追加し，表面を親水化。
...
光学顕微鏡・SEMでは炭化痕が確認され，EDXで金属蒸発痕を検出。
この部位は構造上不可避な露出箇所であり，空気絶縁では耐圧が不足している。

\subsection{原因仮説}
PZT側壁の露出により局所電界が集中し，
立上り過渡の瞬間電流 $I=C\,\mathrm{d}V/\mathrm{d}t$ によるジュール加熱が重畳。
その結果，局所絶縁破壊が発生する。
実効電界は $E\simeq3.3\,\mathrm{MV/cm}$ に達すると推定された。
