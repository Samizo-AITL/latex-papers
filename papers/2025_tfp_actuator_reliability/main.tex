...
% ---------------- 数式まわり（\coloneqq などの未定義対策） ----------------
\usepackage{amsmath}
\usepackage{mathtools} % \coloneqq ほか拡張

% ---------------- General packages (IEEE互換の範囲) ----------------
\usepackage{siunitx}
\sisetup{
  % detect-all は非推奨なので等価設定に分解
  mode = match,
...
\urlstyle{same}
% 見出しに \\ を入れたときの PDF しおり警告を抑制（必要なら）
...
\begin{itemize}
  ...
  \item 恒久策：ゾル–ゲル塗布工程のFERコーターにおいて，
        塗布前処理として酢酸2\,\%による\emph{プレウェット処理(30\,s)}を追加し，
        PZT表面の親水性をリセット。
\end{itemize}
...
\subsection{酢酸プレウエット導入による副作用リスクと評価}

酢酸プレウェットはボイド起因のクラック不良を劇的に低減させる一方，
ゾル–ゲル塗布工程内に1処理ステップを追加することにより，
PZT表面の濡れ性分布や乾燥条件が微妙に変化し，
\emph{膜厚バラツキの増大}を誘発する懸念がある。
表面エネルギーの局所差，乾燥不均一，およびゾル粘度変化に起因して，
スピン塗布時の流動場が面内でわずかに変化し，
結果として中心厚‐外周薄の勾配が強調される傾向が観測された。

このPZT膜厚バラツキは，電界 $E = V/t_\mathrm{PZT}$ の面内分布を変化させ，
局所変位 $\Delta x = d_{33} E t_\mathrm{PZT}$ の列方向非一様性として現れる。
すなわち，膜厚の微小変動がノズル列間の変位差・吐出量差につながる可能性がある。
特に複数列並列駆動構造では，
膜厚差に起因する\emph{列間駆動バラツキ（banding）}が発生するリスクがある。

この懸念を検証するため，
酢酸プレウェット導入後のPZT層について，
以下の多段階評価を実施した：
\begin{enumerate}
  \item \textbf{XRD測定}：
        各ロット全チップ（$N = 15{,}696$）を対象に，
        (110)回折角，FWHM，および配向比 $I(001)/I(110)$ を評価。
        結晶配向の変動は工程内ばらつき範囲に収まり，有意差は認められなかった。
  \item \textbf{ヒステリシス特性 (P--E)}：
        各チップに設けたPZT専用TEG構造を用いて，
        ノズル列方向の上端（1番ノズル側）および下端（300番ノズル側）で
        $P_\mathrm{m}$ および $V_\mathrm{c}$ を測定した。
        上下TEGの差（傾き：$\Delta P_\mathrm{m}/\Delta y$，$\Delta V_\mathrm{c}/\Delta y$）を算出し，
        列方向の電気特性分布を評価項目に加えた。
        平均値および傾きいずれも工程変更前後で統計的差異はなかった。
  \item \textbf{共振周波数 $f_a$}：
        各チップの1番ノズルから300番ノズルまで，
        10ノズル間隔で20点を手動測定し，
        各点の$f_a$およびその列方向傾き（$\Delta f_a/\Delta y$）を評価した。
        膜厚差に起因する周波数勾配は検出されず，
        $f_a$，$Q$値，およびその傾きはいずれも工程前後で有意差がなかった。
  \item \textbf{吐出特性評価}：
        工程切替え前後でそれぞれ10ヘッドを用いて，
        吐出体積，液滴速度，噴射角度，およびサテライト率を測定した。
        列間濃度ムラやbanding傾向は観測されず，
        統計的にも有意差は認められなかった。
\end{enumerate}

これらの結果から，
酢酸プレウェット導入による膜厚バラツキは
材料構造・圧電応答・共振挙動・吐出性能のいずれにも
統計的な影響を与えないことが確認された。
すなわち，
\emph{酢酸プレウェットはクラック不良を抑制しつつ，
膜厚分布変動および列間特性傾きを許容範囲内に維持できる
プロセス条件である}ことが実証された。
...
\@ifundefined{figTrapezoidScreening}{%
  \newcommand{\figTrapezoidScreening}{%
    \begin{tikzpicture}[x=1cm,y=0.08cm,>=Stealth,thick]
      ...
      % -------- I = C dV/dt（重ならないよう上に）--------
      \node[fill=white, inner sep=1pt] at (\tpre+0.5,\VH+6.2) {$I = C\,\mathrm{d}V/\mathrm{d}t$};
      ...
      % -------- COM / VBS ラベル配置 --------
      \node[anchor=west] at (\tMax-1.0,\VH+7.0) {\small COM};
      \draw[-{Stealth[length=2mm]}] (\tMax-0.4,\VH+6.6) -- (\tMax-0.4,\VH+0.2);
      \node[anchor=west] at (\tMax-1.0,\VBS-5.0) {\small VBS};
      \draw[-{Stealth[length=2mm]}] (\tMax-0.2,\VBS-6.1) -- (\tMax-0.2,\VBS+0.3);
    \end{tikzpicture}%
  }%
}{}
\makeatother
...
\begin{figure*}[!t]
  ...
  % --- Fig.4：台形波形の概念図（60%縮小） ---
  \resizebox{0.40\textwidth}{!}{\figTrapezoidScreening}
...
% =====================================================
% Appendix: 列間バラツキ評価（fa/FaKT）
% =====================================================
\appendices
\section{列間バラツキ評価における \texorpdfstring{$f_a$}{fa} 自動測定と FaKT ランク分類
\\ \normalfont\small Automated Column-to-Column Variation Assessment by \texorpdfstring{$f_a$}{fa} Measurement and FaKT Ranking}

\subsection{評価の目的と背景 / Purpose}
列間バラツキの一次指標として，振動板の機械共振周波数 $f_a$ を用いる。
$f_a$ はインピーダンスアナライザで容易に自動取得でき，
変位量（電気機械結合）と強い相関を持つため，量産スクリーニングに適する。
一般に $f_a$ が高いほど振動板拘束が強く（剛性が高く）変位しにくく，
$f_a$ が低いほど変位しやすい。

\subsection{測定セットアップ / Setup}
\begin{itemize}
  \item デバイス：$\mu$TFP アクチュエータユニット（2列，各 300 ノズル）
  \item 計測：広帯域インピーダンススイープ（例：10～\SI{500}{kHz}，分解能 \SI{100}{Hz}）
  \item プロービング：列方向に等間隔サンプリング（例：10 ノズルごと計 20 点/列）
  \item 解析：各点のインピーダンス極小（または位相極大）を $f_a$ と定義
\end{itemize}

\subsection{傾き指標と正規化 / Slope and Normalization}
列方向 $y$（上端 $y=0$，下端 $y=L$）に沿う $f_a(y)$ の一次傾きを
\[
  k_{f_a} \coloneqq \frac{\mathrm{d} f_a}{\mathrm{d} y}\quad[\mathrm{Hz/mm}]
\]
と定義し，最小二乗直線 $f_a(y)=a+by$ の回帰係数 $b$ を採用する。
チップ間比較のため，公称値 $f_{a0}$ で正規化した無次元傾き
\[
  K_{f_a} \coloneqq \frac{1}{f_{a0}}\frac{\mathrm{d} f_a}{\mathrm{d} y}
\quad[-]
\]
も併記する。ここで $f_{a0}$ は列内平均 $\bar f_a$ で良い。

\subsection{FaKT の定義 / Definition}
本稿では \textbf{FaKT（fa-slope based Kolumn Tolerance）} を
\[
  \mathrm{FaKT} \coloneqq \max\left(\left|K_{f_a}^{(1)}\right|,\ \left|K_{f_a}^{(2)}\right|\right)
\]
と定義する（上段・下段の2列に対する正規化傾きの絶対値の最大）。
必要に応じて直線当てはめの信頼性として $R^2$ を併記し，
非線形傾向が強い個体をフラグ（例：$R^2<0.90$）する。

\subsection{ランク基準 / Ranking Rules}
用途別に許容度の異なるヘッド選別を行うため，FaKT に基づきランク分けする。
しきい値は製品/用途で最適化すべきだが，初期値の例を示す。

\begin{table}[h]
  \centering
  \caption{FaKT ランク例（初期基準）}
  \label{tab:fakt-rank}
  \vspace{3pt}
  \setlength{\tabcolsep}{6pt}
  \renewcommand{\arraystretch}{1.15}
  \begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} l c l @{}}
    \toprule
    ランク & FaKT（$|K_{f_a}|$） & 推奨用途 \\
    \midrule
    A（厳） & $\le 1.0\times10^{-3}$ & 列間バンディング厳格仕様 \\
    B       & $\le 2.0\times10^{-3}$ & 標準仕様 \\
    C（緩） & $\le 3.5\times10^{-3}$ & 許容度の高い用途 \\
    D（除外） & $> 3.5\times10^{-3}$ & リワーク/除外 \\
    \bottomrule
  \end{tabular*}
\end{table}

\noindent
\textit{運用指針}：列間バラツキ規格が厳しい用途には A～B ランクを，
緩い用途には B～C ランクを優先的に割り当てる。

\subsection{自動測定フロー / Automated Flow}
\begin{enumerate}
  \item 列ごとにサンプル点 $\{y_i\}$ を巡回し，インピーダンスから $f_a(y_i)$ を抽出。
  \item 直線回帰で $k_{f_a}$（＝$b$）と $R^2$ を算出，$K_{f_a}=k_{f_a}/\bar f_a$ を得る。
  \item 2 列分の $|K_{f_a}^{(1)}|,|K_{f_a}^{(2)}|$ の最大値を FaKT とする。
  \item 表\ref{tab:fakt-rank} の基準でランクを決定し，ヘッド ID に紐づけて出荷振り分け。
\end{enumerate}

\subsection{備考 / Notes}
\begin{itemize}
  \item $f_a$ は変位量の代理指標として有効だが，波形条件や固定治具のばらつきが
  系統誤差を生むため，\textbf{基準治具・温度・スイープ条件}を工程で固定する。
  \item 工程変更（例：側壁パッシベーション導入，薄膜応力条件変更）時には，
  ランク境界の再最適化（ROC カーブ評価など）を推奨する。
  \item バンドパスの設定によりスプリアス共振を拾うことがある。
  品質保証では \textbf{主モードの一貫判定規則}（最大ピーク優先＋位相検証）を設ける。
\end{itemize}
...
\emph{Jpn. J. Appl. Phys.}, vol.~24, no.~7, pp.~1234--1240, 1985.

\bibitem{TFP2014}
S.~Uemura, Y.~Kato, and M.~Tanaka,
``Thin-film piezoelectric MEMS technology for high-density inkjet printheads,''
in \emph{Proc. IEEE MEMS Conf.}, 2014, pp.~456--459.

\bibitem{Samizo2025}
S.~Samizo,
``Reliability improvement of thin-film PZT actuators in PrecisionCore μTFP printheads,''
Tech. Rep., 2025, unpublished.
