...
% ====== Fig.3: ユニットスクリーニング不良率ヒストグラム（2カラム） ======
\begin{figure*}[t]
  \centering
  \begin{tikzpicture}
    \begin{axis}[
      ...
      legend style={
        ...
        nodes={scale=0.85, transform shape, align=left}
      },
    ]
      ...
      \addlegendentry{Before（前処理なし） / Before (no pre-wet), $N_\mathrm{lots}=12$}

      % ---- After ----
      \addplot+[ybar, bar shift=+3pt, fill=red!40] coordinates {
        (0,2) (1,3) (2,5) (3,2)
      };
      \addlegendentry{After（酢酸プレウェット） / After (acetic pre-wet), $N_\mathrm{lots}=12$}
