...
% ====== Fig.2: 6層のPZT断面（中央層にボイド）======
\renewcommand{\figPZTLayersVoidInline}{%
  \begin{tikzpicture}[x=0.9cm,y=0.9cm]
    % 6本の水平ライン
    \def\W{6.4}\def\Pitch{0.55}
    \foreach \k in {0,...,5}{
      \draw[line width=0.8pt,gray!40] (0,\k*\Pitch) -- (\W,\k*\Pitch);
    }
    % 中央付近のボイド矩形（ハッチは手描き線で再現：追加lib不要）
    \def\xL{2.10}\def\xR{2.80}
    \def\yC{2.5*\Pitch}\def\hH{0.18}
    \draw[fill=gray!35,draw=gray!70,line width=0.6pt]
      (\xL,\yC-\hH) rectangle (\xR,\yC+\hH);
    % 斜めハッチ
    \foreach \t in {-0.20,-0.10,0,0.10,0.20}{
      \draw[gray!60,line width=0.6pt]
        ({\xL+0.02},{\yC-\hH+0.04+\t}) -- ({\xR-0.02},{\yC+\hH-0.04+\t});
    }
  \end{tikzpicture}%
}
