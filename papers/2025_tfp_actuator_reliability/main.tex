...
% ---- Fig.2: 6層PZT断面（1層だけボイド枠でハイライト）----
\newcommand{\figPZTLayersVoidInline}[1][0.90]{%
  \begin{tikzpicture}[x=#1cm,y=#1cm]
    % パラメータ（この図のローカル変数）
    \def\W{6.0}   % 横幅
    \def\H{0.45}  % 各層厚
    \def\G{0.10}  % 層間ギャップ（見やすさ用）
    \def\voidLayer{4} % ボイドを示す層（1..6）

    % 6層のみ（0..5 で6回）※絶対に増えない
    \foreach \i in {0,...,5}{%
    ...
    \pgfmathsetmacro{\totalH}{6*(\H+\G)}%
    \draw[line width=0.6pt,gray!70] (0,\totalH+0.10) -- (\W,\totalH+0.10);
  \end{tikzpicture}%
}
