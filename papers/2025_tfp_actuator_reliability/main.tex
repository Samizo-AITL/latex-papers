...
\newcommand{\figVoidDonutInline}{%
  \begin{tikzpicture}[scale=0.055]
    ...
    \foreach \i in {1,...,320}{%
      ...
      \fill[gray!30] ({\ang}:{\rad}) circle (0.9); % 数式展開は {} で囲む
    }
  \end{tikzpicture}%
}
  ...
  \begin{tikzpicture}[x=0.9cm,y=0.9cm]
    ...
    \foreach \i in {0,...,5}{%
      \draw[fill=gray!15,draw=black,line width=0.4pt]
        (0,{ \i*(\H+\G) }) rectangle (\W,{ \i*(\H+\G) + \H });
    }
    % 第4層のボイド（破線でハイライト）
    \draw[line width=0.8pt,gray!50,dashed]
      (2.1,{ 3*(\H+\G) + 0.08 }) rectangle (3.0,{ 3*(\H+\G) + \H - 0.08 });
    % 上下の基準線（電極イメージ）
...
\section{考察}
\textbf{クラック：} RTA後の外気由来付着でPZT表面が疎水化し、スピン時に気泡を巻き込み$\to$膜内ボイド化。
暫定的にロードロック防壁、恒久的に酢酸プレウェットを導入。
XRD配向率、ヒステリシス、吐出特性で従来条件との差は有意差なし。

\textbf{焼損：} 構造上避け難い側壁露出＋高電位差で電界集中。
対策案としてALD--AlOx端部被覆、電圧マージン緩和（例：+\SI{25}{V}/\SI{-5}{V}）や版下改良を提示。

\section{結論}
クラックは酢酸プレウェット導入で5--10\%からほぼ0\%へ撲滅。
焼損は原因を特定し、AlOx被覆を主案として提案した。
薄膜PZT $d_{33}$方式の量産信頼性向上に資する。
