...
Fig.~\ref{fig:yield}に、ロット順（ユニットスクリーニング順）に対する
不良率の推移を示す。各ロットの試験数は $N=500$（ダイ）で一定とし、
不良率は $\mathrm{defect\ rate}=n_{\mathrm{fail}}/N$ で定義した。
従来条件（Before）で5--10\%のばらつきを示していたが、
酢酸プレウェット導入後（After）には全ロットで0\%に収束した。
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      ymajorgrids=true,
      grid style={gray!30},
      legend style={draw=none, at={(0.02,0.98)}, anchor=north west, nodes={scale=0.9, transform shape}},
      ...
      \addlegendentry{Before (No pre-wet)}
      ...
      \addlegendentry{After (Acetic pre-wet)}

      % ---- N=表記（凡例の下に注記） ----
      \node[anchor=north west,align=left,font=\footnotesize] at (rel axis cs:0.02,0.12) {%
        $N=\!500$/lot, binomial CI as error bars};

    \end{axis}
  \end{tikzpicture}
  \caption{%
    歩留まりのロット別推移（酢酸プレウェット導入前後）／
    Yield trend by lot (before vs. after acetic pre-wet).%
  }
  \label{fig:yield_trend}
\end{figure}

% ====== Table: 比較プロセス条件（1カラムでも2カラムでもOK） ======
\begin{table}[t]
  \centering
  \caption{%
