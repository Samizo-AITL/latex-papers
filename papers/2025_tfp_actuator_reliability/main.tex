...
% ====== Fig.2：6層・水平ライン＋中央のハッチ矩形（スケッチ準拠） ======
\newcommand{\figPZTLayersVoidInline}{%
  \begin{tikzpicture}[x=1cm,y=1cm]
    % パラメータ（必要なら数値だけ触ってください）
    \def\W{6.4}      % 横幅
    \def\Pitch{0.55} % 層ピッチ（行間）
    \def\LW{0.8pt}   % ラインの太さ

    % 6本の水平ライン（完全水平）
    \foreach \k in {0,...,5}{
      \draw[line width=\LW,gray!40] (0,\k*\Pitch) -- (\W,\k*\Pitch);
    }

    % 中央層の矩形ボイド（スケッチ位置）
    \def\xL{2.10}\def\xR{2.80}               % 左右位置
    \def\yC{2.5*\Pitch}\def\hH{0.18}         % 中央高さ・半高さ
