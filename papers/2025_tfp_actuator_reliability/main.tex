...
% ====== Fig.3: ユニットスクリーニング不良率ヒストグラム（1カラム） ======
\begin{figure}[t]
  \centering
  \begin{tikzpicture}
    \begin{axis}[
      ...
      legend style={
        ...
        nodes={scale=0.82, transform shape, align=left}
      },
    ]

      % ---- Before (No pre-wet) :: 12 lots, 1%ビン中心にカウントを配置 ----
      % 例：6–10%帯に集中（クラック起因）
      \addplot+[ybar, bar shift=-3pt, fill=blue!40] coordinates {
        (6,2) (7,2) (8,3) (9,2) (10,3)
      };
      \addlegendentry{\shortstack[l]{Before（前処理なし）\\Before (no pre-wet), $N_\mathrm{lots}=12$}}

      % ---- After (Acetic pre-wet) :: 12 lots, 焼損を含むため≈2%が残存 ----
      % 例：0–3%帯に分布（クラック激減、焼損残り）
      \addplot+[ybar, bar shift=+3pt, fill=red!40] coordinates {
        (0,2) (1,3) (2,5) (3,2)
      };
      \addlegendentry{\shortstack[l]{After（酢酸プレウェット）\\After (acetic pre-wet), $N_\mathrm{lots}=12$}}
  ...
  \caption{%
    ...
    ※ スクリーニング時点ではクラック／焼損の判別不可（分解観察で識別）。%
  }
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      % ---- Before (No pre-wet): 12 lots, 1%ビン中心でカウント配置 ----
      \addplot+[ybar, bar shift=-3pt, fill=blue!40] coordinates {
        (6,2) (7,2) (8,3) (9,2) (10,3)
      };
      \addlegendentry{\shortstack[l]{Before（前処理なし）\\Before (no pre-wet), $N_\mathrm{lots}=12$}}

      % ---- After (Acetic pre-wet): 12 lots, 焼損を含むため2%前後が残存 ----
      \addplot+[ybar, bar shift=+3pt, fill=red!40] coordinates {
        (0,2) (1,3) (2,5) (3,2)
      };
      \addlegendentry{\shortstack[l]{After（酢酸プレウェット）\\After (acetic pre-wet), $N_\mathrm{lots}=12$}}
  ...
  \label{fig:yield_hist} % ★ ラベル名
\end{figure}

% ====== Table 2: 比較プロセス条件（1カラム） ======
\begin{table}[t]
  \centering
  \caption{%
  ...
  \begin{tabular}{@{} lll @{}}
    ...
    RTA後搬送 / Post-RTA handling
      & 大気露出 / Air exposure
      & 窒素パージミニロードロック / N\textsubscript{2}-purged mini-LL \\
    スピン塗布 / Spin coating
      & 同一条件 / Same
      & 同一条件 / Same \\
    PZT積層 / PZT stack
