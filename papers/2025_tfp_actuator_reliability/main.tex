...
\renewcommand{\figEdgeBurnoutInline}{%
  \begin{tikzpicture}[x=1.1cm,y=1.0cm]
    % 寸法
    \def\W{8.2}     % 全幅
    \def\beH{0.25}  % BE厚さ（見せ方）
    \def\pztH{1.10} % PZT高さ
    \def\teH{0.15}  % TE高さ
    \def\slitX{3.30}% スリットX
    % ---------- BE（下部電極） ----------
    \fill[gray!60] (0,0) rectangle (\W,\beH);
    \draw[line width=1.0pt] (0,\beH) -- (\W,\beH); % BE上面を太線

    % ---------- PZT（スリットで左右分割） ----------
    % 左ブロック（駆動部）
    \fill[gray!15] (0,\beH) rectangle (\slitX,\beH+\pztH);
    % 右ブロック（COM配線側）
    \fill[gray!15] (\slitX+0.45,\beH) rectangle (\W,\beH+\pztH);
    % 界面・スリットの線を強調
    \draw[line width=1.0pt] (0,\beH+\pztH) -- (\W,\beH+\pztH); % PZT上面
    \draw[line width=1.0pt] (\slitX,\beH) -- (\slitX,\beH+\pztH); % スリット側壁

    % ---------- TE（上電極：左側のみ） ----------
    \fill[gray!80] (0,\beH+\pztH) rectangle (2.2,\beH+\pztH+\teH);
    \draw[line width=0.8pt] (0,\beH+\pztH+\teH) -- (2.2,\beH+\pztH+\teH);
    % VBS端子（簡素なピン）
    \draw (0.2,\beH+\pztH+\teH) -- ++(0,0.55);
    \node[scale=0.75,anchor=south] at (0.2,\beH+\pztH+\teH+0.55) {VBS};

    % ---------- Au配線（COM）とBEコンタクト ----------
    % 二本のポスト（BEに接する）
    \fill[gray!30] (4.6,\beH) rectangle (5.0,\beH+\pztH+0.35);
    \fill[gray!30] (6.2,\beH) rectangle (6.6,\beH+\pztH+0.35);
    % 上部バーで接続
    \fill[gray!30] (4.6,\beH+\pztH+0.35) rectangle (6.6,\beH+\pztH+0.60);
    % COMピン
    \draw (5.6,\beH+\pztH+0.60) -- ++(0,0.70);
    \node[scale=0.75,anchor=south] at (5.6,\beH+\pztH+1.30) {COM};
    % 右側PZT上面の細いAuランディング（視覚補助）
    \draw[gray!50,line width=0.6pt] (\slitX+0.45,\beH+\pztH) -- (7.6,\beH+\pztH);

    % ---------- 焼損位置（スリット根元） ----------
    \draw[->,line width=0.8pt] (\slitX-0.35,\beH+0.40) -- ++(-0.60,0.35);
    \node[scale=0.75,anchor=south east] at (\slitX-0.35,\beH+0.40) {焼損};

    % 余白の枠は描かない（IEEE図っぽく簡素）
  \end{tikzpicture}%
}
