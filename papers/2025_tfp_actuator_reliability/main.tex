...
\begin{tikzpicture}[x=1cm,y=0.08cm,>=Stealth,thick]
  ...
  \draw[semithick]
    ...
    -- (\tFallEnd,\V0)
    -- (\tMax,\V0);
  \node[anchor=west] at (\tHoldEnd-0.1,\VH+2.5) {COM};
  ...
  \node[fill=white,inner sep=1pt] at ($(MidFast)+(0,10)$)
    {$I = C\,\mathrm{d}V/\mathrm{d}t$};
  \node[anchor=west,align=left] at ($(MidFast)+(0.3,5)$)
    {\small 急峻スロープ：大きい$\mathrm{d}V/\mathrm{d}t$ $\Rightarrow$ 大きい$I$};
  \node[anchor=west,align=left,red] at ($(MidSlow)+(0.3,5)$)
    {\small 緩和スロープ：小さい$\mathrm{d}V/\mathrm{d}t$ $\Rightarrow$ 小さい$I$};

  % -------- 凡例 --------
  \draw[semithick] (\tMax-2.9,\VH-22) -- ++(0.9,0) node[right]{\small 急峻立上り};
...
\begin{figure}[!t]
  ...
  瞬間電流ピークが抑制される。スクリーニング波形は印字波形と異なるため，
  変更時は印字非劣化の同等性評価が必要。}
