...
（Fig.~\ref{fig:edge-burnout}）。なお，本節で示す台形波形は
ユニットスクリーニング専用であり，印字駆動波形とは異なる。
したがってスクリーニング波形の立上り勾配を変更する場合は，
印字の\emph{非劣化（同等性）}を確認する評価計画が前提となる
（Fig.~\ref{fig:trapezoid-screening} 参照）。
...
\begin{tikzpicture}[x=1cm,y=0.08cm,>=Stealth,thick]
  % -------- パラメータ（編集OK） --------
  \def\VBS{-10}      % VBS レベル [V]
  \def\V0{10}        % COM の初期レベル [V]
  \def\VH{30}        % COM の高レベル（台形天井）[V]
  \def\t0{0}         % 開始時刻
  \def\tpre{1.2}     % 立上り前の水平区間
  \def\trf{0.6}      % 急峻立上り 時間
  \def\trs{1.7}      % 緩和立上り 時間（赤）
  \def\thold{2.8}    % 天井空間（立上り後〜立下り前）
  \def\tfall{0.7}    % 立下り時間
  \def\textra{0.8}   % 終了後の水平区間

  % 時間目盛（描画用）
  \pgfmathsetmacro{\tRiseEndFast}{\tpre+\trf}
  \pgfmathsetmacro{\tRiseEndSlow}{\tpre+\trs}
  \pgfmathsetmacro{\tHoldStart}{\tRiseEndFast}
  \pgfmathsetmacro{\tHoldEnd}{\tHoldStart+\thold}
  ...
  \draw[semithick]
    ...
    -- (\tFallEnd,\V0)
    -- (\tMax,\V0);
  \node[anchor=west] at (\tHoldEnd-0.15,\VH+2.5) {COM};

  % -------- COM 緩和スロープ（赤で上書き）--------
  \draw[line width=1pt,red]
    (\tpre,\V0) -- (\tRiseEndSlow,\VH);

  % -------- 注釈：I = C dV/dt とピーク電流の違い --------
  ...
  \node[fill=white,inner sep=1.5pt] at ($(MidFast)+(0,10)$)
    {$I = C\,\mathrm{d}V/\mathrm{d}t$};
  \node[align=center,anchor=west] at ($(MidFast)+(0.25,5)$)
    {\small 急峻スロープ\\[-1pt]\small（大きい$\mathrm{d}V/\mathrm{d}t$ $\Rightarrow$ 大きい$I$）};
  \node[align=center,anchor=west,red] at ($(MidSlow)+(0.25,5)$)
    {\small 緩和スロープ\\[-1pt]\small（小さい$\mathrm{d}V/\mathrm{d}t$ $\Rightarrow$ 小さい$I$）};

  \draw[semithick] (\tMax-2.9,\VH-22) -- ++(0.9,0) node[right,black]{\small 急峻立上り};
  \draw[red,line width=1pt] (\tMax-2.9,\VH-27) -- ++(0.9,0) node[right,red]{\small 勾配緩和};

  \draw[dashed] (\tpre,\VH) -- ++(0.3,0);
\end{tikzpicture}%
}
%================ ここまで =====================================================
...
\begin{figure}[t]
  \centering
  \figTrapezoidScreening
  \caption{ユニットスクリーニング用の台形波形の概念図。
  立上り勾配を緩和すると $I=C\,\mathrm{d}V/\mathrm{d}t$ が低下し，
  瞬間電流ピークが抑制される。印字波形とは異なるため，
  変更時は印字非劣化の同等性評価が必須である。}
  \label{fig:trapezoid-screening}
\end{figure}
...
よって，当面はプロセス側（側壁パッシベーション等）を先行し，
波形緩和は必要最小の自由度（立上り dV/dt のみ）で限定的に検討する。
