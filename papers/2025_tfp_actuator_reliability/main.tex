...
\@ifundefined{figEdgeBurnoutInline}{%
  \newcommand{\figEdgeBurnoutInline}{%
    \begin{tikzpicture}[x=0.9cm,y=0.9cm]
      ...
      \def\xGapR{6.0}  % Auパッド開始x（左端）
      \def\xViaL{7.6}  % コン タクト開口 左端
      \def\xViaR{8.2}  % コン タクト開口 右端
      ...
      \fill[gray!80] (0,\beH+\pztH) rectangle (\xPZT,\beH+\pztH+\teH);      % TE (VBS)

      % 右：Au配線下にも BE/PZT/TE スタック（TEだけ抜かない）
      % PZT（コンタクト開口で分割）
      \fill[gray!15] (\xGapR,\beH) rectangle (\xViaL,\beH+\pztH);
      \fill[gray!15] (\xViaR,\beH) rectangle (\W,\beH+\pztH);
      % TE（同様に分割して Au の下にも敷設）
      \fill[gray!80] (\xGapR,\beH+\pztH) rectangle (\xViaL,\beH+\pztH+\teH);
      \fill[gray!80] (\xViaR,\beH+\pztH) rectangle (\W,\beH+\pztH+\teH);

      % Au パッド（COM）— 最前面
      \fill[gray!30] (\xGapR,\beH+\pztH) rectangle (\W,\beH+\pztH+0.60);

      % Au のコンタクト柱（1本）：BEへ接続する開口
      \fill[gray!30] (\xViaL,\beH) rectangle (\xViaR,\beH+\pztH);
      ...
      \draw[->] (7.2,\beH+\pztH+0.9) -- (7.2,\beH+\pztH+0.62);
      \node[anchor=south] at (7.2,\beH+\pztH+0.9) {\scriptsize COM};
      % 層名
