...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      xlabel={ロット / Lot index}, ylabel={不良率 / Defect rate [\%]},
      ...
      \addplot+[mark=*, thick] coordinates
        {(1,6.8) (2,8.9) (3,5.4) (4,9.7) (5,7.6)};
      \addlegendentry{Before（従来）}

      \addplot+[mark=square*, thick] coordinates
        {(6,0.0) (7,0.0) (8,0.0) (9,0.0) (10,0.0)};
      \addlegendentry{After（酢酸プレウェット）}
    \end{axis}
  \end{tikzpicture}
  \caption{%
    歩留まりの推移（各ロット $N=500$）／
    Yield trend with $N=500$ dies per lot
  }
  \label{fig:yield}
\end{figure}
...
\begin{table}[t]
  \centering
  \caption{比較プロセス条件 / Compared process conditions}
  \label{tab:proc-compare}
  \begin{tabular}{ll}
    \toprule
    項目 / Item & 条件 / Condition \\
    \midrule
    共通 & RTA, PZT 200\,nm$\times$6, 評価N=500/lot \\
    従来（Before） & プレウェット無し，ロードロック簡易遮蔽 \\
    改善（After） & 酢酸プレウェット（親水化），他同一 \\
    \bottomrule
  \end{tabular}
\end{table}
