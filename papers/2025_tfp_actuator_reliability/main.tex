...
% ====== Fig.3: スリット構造（左=駆動部，右=COMがBEへ接続）======
\renewcommand{\figEdgeBurnoutInline}{%
  \begin{tikzpicture}[x=0.9cm,y=0.9cm]
    ...
    \def\pztH{1.20}     % PZT厚み
    \def\teH{0.25}      % 上電極厚み
    \def\slitX{4.0}     % スリット位置（左駆動/右配線の境目）
    \def\auW{3.0}       % COM配線幅
    \def\auH{0.50}      % COM配線厚み（描画上）
    \def\viaX{5.5}      % COM→BEの縦コンタクトX位置
    \def\viaW{0.35}     % 縦コンタクト幅

    % --- Bottom Electrode (BE) 全体 ---
    \fill[gray!55] (0,0) rectangle (\W,\beH);

    % --- 左側：PZT駆動部（BE上にPZT→TE） ---
    \fill[gray!15] (0,\beH) rectangle (\slitX,\beH+\pztH);             % PZT
    \fill[gray!80] (0,\beH+\pztH) rectangle (\slitX,\beH+\pztH+\teH);  % 上電極(TE/VBS)

    % --- 中央スリット（PZT/TEが無い縦の溝） ---
    \fill[white] (\slitX,0) rectangle (\slitX+0.02,\beH+\pztH+\teH+\auH); % ほんの細い空隙表示
    \draw[line width=0.8pt,black] (\slitX,0) -- (\slitX,\beH+\pztH+\teH+\auH); % スリット境界線（片側だけ太め）

    % --- 右側：COM（Au配線）---
    % 上面に走るCOMトレース（PZTが無い右側上面）
    \fill[gray!30] (\slitX+2,\beH+\pztH) rectangle (\slitX+2+\auW,\beH+\pztH+\auH);

    % COM→BE への縦コンタクト（スケッチの“落とし”）
    \fill[gray!30] (\viaX-\viaW/2,\beH) rectangle (\viaX+\viaW/2,\beH+\pztH+\auH);

    % 輪郭線（必要最小限、斜め線なし）
    \draw[line width=0.6pt,black] (0,0) -- (\W,0);                                % BE下面
    \draw[line width=0.6pt,black] (0,\beH) -- (\slitX,\beH);                       % 駆動部BE上面
    \draw[line width=0.6pt,black] (0,\beH+\pztH) -- (\slitX,\beH+\pztH);           % PZT上面
    \draw[line width=0.6pt,black] (0,\beH+\pztH+\teH) -- (\slitX,\beH+\pztH+\teH); % TE上面
    \draw[line width=0.6pt,black] (\slitX+2,\beH+\pztH+\auH) -- (\slitX+2+\auW,\beH+\pztH+\auH); % COM上端

    % 最小ラベル（必要なら）
    \node[anchor=north west,scale=0.75] at (0.1,\beH+\pztH+\teH) {駆動部（PZT）};
    \node[anchor=south,scale=0.75] at (\slitX+2+\auW/2,\beH+\pztH+\auH) {COM（Au）};
    \node[anchor=west,scale=0.7] at (\viaX+\viaW/2+0.1,\beH+\0.15) {BEに接続};
  \end{tikzpicture}%
}
