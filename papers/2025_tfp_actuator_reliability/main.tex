...
% ==== Fig.3 端部焼損（左は現状維持／Auコンタクト1本／Au下もBE-PZT-TE）====
\makeatletter
\@ifundefined{figEdgeBurnoutInline}{}{\renewcommand{\figEdgeBurnoutInline}{}}
\newcommand{\figEdgeBurnoutInline}{%
  \begin{tikzpicture}[x=0.9cm,y=0.9cm]
    ...
    \def\pztH{1.20}  % PZT厚
    \def\teH{0.20}   % TE厚
    \def\xPZT{4.00}  % 左PZTの右端（=側壁）
    \def\gap{0.20}   % 側壁のスリット幅（左は触らない）
    \def\xR{6.20}    % Auパッド左端
    \def\xPadR{9.20} % Auパッド右端
    \def\xViaL{7.60} % Auコンタクト（1本）の左端
    \def\xViaR{8.10} %                右端
    ...
    % 右側にも BE/PZT/TE を敷く（側壁直後に細スリット gap を残す）
    \fill[gray!15] (\xPZT+\gap,\beH) rectangle (\W,\beH+\pztH);
    \fill[gray!80] (\xPZT+\gap,\beH+\pztH) rectangle (\W,\beH+\pztH+\teH);

    % Auパッド（COM）
    \fill[gray!30] (\xR,\beH+\pztH) rectangle (\xPadR,\beH+\pztH+0.60);
    ...
    \draw[->] ( (\xR+\xPadR)/2 ,\beH+\pztH+0.9) -- ( (\xR+\xPadR)/2 ,\beH+\pztH+0.62);
    \node[anchor=south] at ( (\xR+\xPadR)/2 ,\beH+\pztH+0.9) {\scriptsize COM};

    % 焼損注記（側壁）
    \draw[->,thick] (\xPZT+0.7,\beH+0.6) -- (\xPZT+0.05,\beH+0.6);
    \node[anchor=west] at (\xPZT+0.75,\beH+0.6) {\scriptsize 焼損発生点（PZT側壁露出）};
  \end{tikzpicture}%
}
\makeatother
