...
\begin{enumerate}
  ...
  \item \textbf{ヒステリシス特性 (P--E)}：
        同一チップ群に対し，$P_\mathrm{r}$，$E_\mathrm{c}$，$d_{33}^\mathrm{eff}$，$\tan\delta$ を全数測定。
        平均値・分散とも工程変更前後で統計的差異はなかった。
  \item \textbf{共振周波数 $f_a$}：
        手動測定20チップに対して共振応答を取得。
        膜厚差に起因する共振周波数シフトは検出されず，
        $f_a$および$Q$値はいずれも工程前後で有意差がなかった。
  \item \textbf{吐出特性評価}：
...
膜厚分布変動を許容範囲内に維持できるプロセス条件である}ことが実証された。
