...
% 評価・積み上げ分岐（負の and をやめ xshift で左右オフセット）
\node (H1) [tech, below=6mm of H, xshift=-2mm] {環境データ積み上げ判定\\(EChemSkip)\\\emph{Environmental Compliance}};
\node (H2) [tech, below=6mm of H, xshift= 2mm] {コスト積み上げ\\\emph{Cost Roll-up}};
\draw[flow] (H) |- (H1);
\draw[flow] (H) |- (H2);
...
\node (P3) [eval, above=3mm of E, xshift=4mm] {計測器情報\\{\scriptsize 三次元測定機・マイクロメータ・真円度計 等}\\\emph{Measurement Instruments}};
\draw[ext] (P3) -- (E);
