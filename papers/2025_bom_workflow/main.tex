...
% preamble（例）：\usepackage{tikz} を入れておく
% \usepackage{tikz}
...
% 評価・積み上げ分岐
\node (H1) [tech, below left=6mm and -2mm of H] {環境データ積み上げ判定\\(EChemSkip)\\\emph{Environmental Compliance}};
\node (H2) [tech, below right=6mm and -2mm of H] {コスト積み上げ\\\emph{Cost Roll-up}};
\draw[flow] (H) |- (H1);
\draw[flow] (H) |- (H2);
...
% 評価・計測情報（外部入力・点線）
%--------------------------------
\node (P1) [eval, above left=5mm and -2mm of B] {試作評価\\{\scriptsize 寸法測定・性能検証}\\\emph{Prototype Evaluation}};
\draw[ext] (P1) -- (B);
