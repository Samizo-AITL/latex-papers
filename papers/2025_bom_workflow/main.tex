...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    chip/.style={draw=black, fill=black!15, rounded corners=1pt, 
    ...
    % --- Symbolic traces ---
    \draw[thin] (drv.south) -- ++(0,-0.9) -- (1.7,0);
    \draw[thin] (hcs2.south) -- ++(0,-0.9) -- (6.2,0);

    % --- Labels ---
    \node[note, anchor=west] at (0.1,3.05) {ポリイミド基材（COF）};
    \node[note, anchor=west] at (0.1,-0.75) {端子列（TAB/AOF接続）};
  \end{tikzpicture}
  \caption{HCS2搭載COFの簡易模式構造（Appendix用）。}
