...
% IEEEtran が NFSS 名 ptm/phv/pcr を期待するため、欧文フォントに割当てる
...
% 和文：太字を持つ HaranoAji を既定に（警告低減）
\setmainjfont{HaranoAjiMincho}[
  Script=CJK,
  YokoFeatures={JFM=ujis}, TateFeatures={JFM=ujisv}
]

% IPAex を使いたい場合はこちらに差し替え（太字/斜体は擬似で補う）
% \setmainjfont{IPAexMincho}[
%   Script=CJK,
%   YokoFeatures={JFM=ujis}, TateFeatures={JFM=ujisv},
%   AutoFakeBold=2.0, AutoFakeSlant=0.15
% ]

% --- 数式は Times 系（IEEEtran との相性◎） ---
\usepackage{newtxmath}

% --- 他パッケージ ---
\usepackage{amsmath}            % amssymb は newtxmath に内包されるので基本不要
% \usepackage{bm}               % 太字ベクトル等が必要なら任意で
\usepackage{graphicx}
\usepackage{xcolor}
\usepackage{cite}
...
\bibliography{refs}  % ← refs.bib を用意しない場合はこの2行を外す
