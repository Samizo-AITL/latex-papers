...
\tikzset{
  ...
  satL/.style={circle, draw, fill=gray!40, inner sep=0pt, minimum size=2.6pt} % 少し大きめ
}

% ===== 互換: \node[node] 用（ink_classification_tikz.tex が要求） =====
% ここでの 'node' は TikZ オプションのキー名。コマンド \node とは衝突しません。
\tikzset{
  node/.style={circle, draw, inner sep=0pt, minimum size=5.5mm, thick},
  nodeF/.style={node, fill=gray!12},  % 薄め塗り
  nodeS/.style={node, minimum size=4.5mm},
  nodeDot/.style={circle, fill, inner sep=0pt, minimum size=2.2pt} % 極小点
}
