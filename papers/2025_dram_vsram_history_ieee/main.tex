...
% ==== Fig. Stack vs Trench (vertical; stack top aligned, red=pn only) ====
\begin{figure}[t]
\centering
...
\begin{tikzpicture}[x=1cm,y=1cm]
  % p- substrate (surface at y=0)
  \fill[gray!20] (-2,0) rectangle (2,-0.8);
  \node[anchor=east] at (-2,0.15) {\small $p^-$ substrate};

  % n+ diffusion (top aligned to surface y=0)
  \fill[gray!55] (-0.45,0) rectangle (0.45,-0.6);
  \node[anchor=west] at (0.55,-0.30) {\small $n^+$ diffusion};

  % storage node contact/electrode (over the diffusion)
  \fill[black!30] (-0.12,0.00) rectangle (0.12,1.20);
  \node[anchor=west] at (0.20,0.95) {\small Storage Node (electrode)};

  % pn junction (sides+bottom only; no top edge)
  \draw[red, very thick] (-0.45,0) -- (-0.45,-0.6);
  \draw[red, very thick] (0.45,0) -- (0.45,-0.6);
  \draw[red, very thick] (-0.45,-0.6) -- (0.45,-0.6);
  \node[red, anchor=north] at (0,-0.65) {\scriptsize pn 接合（側壁＋底面）};
...
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  % Parameters to tweak strap size quickly
  \def\strapW{0.40}   % strap width (change here to visualize larger/smaller junction)
  \def\strapH{0.45}   % strap height

  % p- substrate
  \fill[gray!20] (-2,0) rectangle (2,-2.2);
  \node[anchor=east] at (-2,0.15) {\small $p^-$ substrate};

  % trench cavity outline
  \draw[black,thick] (-0.45,0) -- (-0.45,-1.9) -- (0.45,-1.9) -- (0.45,0);

  % node electrode inside trench (insulated by dielectric)
  \fill[black!30] (-0.35,-0.08) rectangle (0.35,-1.80);
  % dielectric (schematic thin line along walls)
  \draw[black, line width=0.25pt] (-0.45,0) -- (-0.45,-1.9);
  \draw[black, line width=0.25pt] (0.45,0) -- (0.45,-1.9);
  \draw[black, line width=0.25pt] (-0.45,-1.9) -- (0.45,-1.9);

  % buried strap n+ diffusion (small by default)
  \path (0.55,-0.60) coordinate (SBL);
  \fill[gray!55] (SBL) rectangle ($(SBL)+(\strapW,\strapH)$);
  \node[anchor=west] at ($(SBL)+(\strapW+0.08,0.5*\strapH)$) {\small buried strap $n^+$ diffusion};

  % pn junction = boundary of strap in p- substrate
  \draw[red, very thick] (SBL) rectangle ($(SBL)+(\strapW,\strapH)$);
  \node[red, anchor=west] at ($(SBL)+(\strapW+0.08,-0.02)$) {\scriptsize pn 接合（面積は設計依存）};

  % (optional) surface diffusion landing (schematic)
  \fill[gray!55] ($(SBL)+(0.10,\strapH)$) rectangle ($(SBL)+(\strapW-0.10,\strapH+0.12)$);
...
\caption{Stack と Trench のセル概念断面。赤線は
pn 接合（$n^+$ 拡散—$p^-$ 基板）の境界のみを示す。
(a) Stack は拡散が表面に露出しており、pn 接合は側壁と底面に限定される。
(b) Trench ではノード自体は誘電体で基板と絶縁され、pn 接合は
\emph{buried strap} 周囲のみに形成される。ストラップ寸法により接合面積は
小さく設計することも可能だが、実際の高温リークは面積だけでなく
周長・界面欠陥・電界集中にも依存し、評価結果（Fig.~\ref{fig:trench_area_leak}）では
Trench 側で増えやすい傾向が観測された。}
\label{fig:stack_trench_final}
\end{figure}
