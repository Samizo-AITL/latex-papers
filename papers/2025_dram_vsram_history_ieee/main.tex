...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % n+ diffusion (aligned at surface)
...
\caption{Stack と Trench キャパシタの比較図。
(a)(b)：断面図。Stack では拡散が表面にあり、pn 接合は側壁と底面に限定される。
Trench では埋め込みストラップ周囲に pn 接合が形成される。
(c)(d)：上面図。Stack は単一矩形セルで周辺長が短いが、
Trench は複数のトレンチ列により周辺長が大きくなる。}
\label{fig:stack_trench_complete}
\end{figure}
