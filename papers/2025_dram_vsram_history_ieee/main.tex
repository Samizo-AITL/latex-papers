...
% ==== Fig. Stack vs Trench (断面図＋上面図) ====
\begin{figure}[t]
\centering

% ---------- (a) Stack Capacitor Cross-section ----------
\begin{minipage}{0.46\linewidth}
\centering
\begin{tikzpicture}[scale=0.9]
  % substrate
  \fill[gray!20] (-1.6,0) rectangle (1.6,-0.8);
  \node[anchor=east] at (-1.6,0.15) {\small $p^-$ substrate};

  % n+ diffusion
  \fill[gray!50] (-0.6,0) rectangle (0.6,-0.6);
  \node[anchor=west] at (0.7,-0.3) {\small $n^+$ diffusion};

  % storage node electrode
  \fill[black!30] (-0.15,0) rectangle (0.15,1.2);
  \node[anchor=west] at (0.25,0.9) {\small Storage Node};

  % pn junction (sides+bottom only)
  \draw[red,very thick] (-0.6,0) -- (-0.6,-0.6);
  \draw[red,very thick] (0.6,0) -- (0.6,-0.6);
  \draw[red,very thick] (-0.6,-0.6) -- (0.6,-0.6);
  \node[red,anchor=north] at (0,-0.65) {\scriptsize pn 接合（側壁＋底面）};

  % title
  \node at (0,1.45) {\bfseries (a) Stack Cross-section};
\end{tikzpicture}
\end{minipage}
\hspace{0.05\linewidth}
% ---------- (b) Trench Capacitor Cross-section ----------
\begin{minipage}{0.46\linewidth}
\centering
\begin{tikzpicture}[scale=0.9]
  % substrate
  \fill[gray!20] (-1.6,0) rectangle (1.6,-2.0);
  \node[anchor=east] at (-1.6,0.15) {\small $p^-$ substrate};

  % trench outline
  \draw[black,thick] (-0.5,0) -- (-0.5,-1.7) -- (0.5,-1.7) -- (0.5,0);

  % node electrode (inside trench)
  \fill[black!30] (-0.35,-0.1) rectangle (0.35,-1.6);

  % buried strap diffusion
  \fill[gray!50] (0.55,-0.5) rectangle (0.95,-0.9);
  \node[anchor=west] at (1.0,-0.7) {\small buried strap $n^+$};

  % pn junction
  \draw[red,very thick] (0.55,-0.5) rectangle (0.95,-0.9);
  \node[red,anchor=west] at (1.0,-1.0) {\scriptsize pn 接合};

  % title
  \node at (0,1.0) {\bfseries (b) Trench Cross-section};
\end{tikzpicture}
\end{minipage}
...
\begin{tikzpicture}[scale=0.9]
  ...
  % n+ diffusion (one block)
  \fill[gray!50] (1.1,1.1) rectangle (1.9,1.9);
  \draw[red,very thick] (1.1,1.1) rectangle (1.9,1.9);

  \node at (1.5,3.3) {\bfseries (c) Stack Top view};
  \node[red,below] at (1.5,1.05) {\scriptsize 周辺長 = 短い};
\end{tikzpicture}
\end{minipage}
\hspace{0.05\linewidth}
\begin{minipage}{0.46\linewidth}
...
\begin{tikzpicture}[scale=0.9]
  ...
  % multiple trenches inside same cell
  \foreach \x in {0.6,1.4,2.2}{
    \fill[gray!50] (\x,0.6) rectangle +(0.3,1.8);
    \draw[red,very thick] (\x,0.6) rectangle +(0.3,1.8);
  }

  \node at (1.5,3.3) {\bfseries (d) Trench Top view};
  \node[red,below] at (1.5,0.55) {\scriptsize 周辺長 = 長い};
\end{tikzpicture}
\end{minipage}

\caption{Stack と Trench キャパシタの比較。
(a)(b) は断面図、(c)(d) は同一世代・同一セル面積での上面レイアウト。
Stack は単一拡散で周辺長が短いが，
Trench は複数の細溝で容量を稼ぐため周辺長が長く，
高温リークに不利。}
\label{fig:stack_trench_full}
\end{figure}
