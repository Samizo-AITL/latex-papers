...
% ==== Fig. X : 0.18µm Stack vs. Trench (vertical; captions unified) ====
\begin{figure}[t]
\centering

{\large \bfseries 0.18\,\textmu m Generation — Cross-sections}\par\vspace{0.6em}

% ---------- (a) Stack Cross-section ----------
\begin{tikzpicture}[x=1cm,y=1cm]
  % p- substrate
  \fill[gray!20] (-3.0,0) rectangle (3.0,-1.0);
  \node[anchor=east] at (-3.0,0.12) {\small $p^-$ substrate};

  % n+ diffusion (細く修正)
  \fill[gray!55] (-1.1,0) rectangle (1.1,-0.65);

  % storage node contact
  \fill[black!30] (-0.25,0) rectangle (0.25,1.00);

  % junction highlight（周辺は短め）
  \draw[red,very thick] (-1.1,0) -- (-1.1,-0.65) -- (1.1,-0.65) -- (1.1,0);
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % trench cavity
  \draw[black,thick] (-0.9,0) -- (-0.9,-1.6) -- (0.9,-1.6) -- (0.9,0);

  % trench electrode (inside)
  \fill[black!30] (-0.60,-0.10) rectangle (0.60,-1.50);

  % buried strap（右側例）
  \fill[gray!55] (1.10,-0.70) rectangle (1.55,-1.10);

  % pn junction highlight at strap
  \draw[red,very thick] (1.10,-0.70) rectangle (1.55,-1.10);
\end{tikzpicture}
\caption*{(b) Trench Cross-section}

\vspace{1.2em}
{\large \bfseries 0.18\,\textmu m Generation — Top views}\par\vspace{0.6em}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % n+ active（適正サイズに）
  \fill[gray!55] (-1.1,-1.1) rectangle (1.1,1.1);
  % storage node contact (center)
  \fill[black!30] (-0.35,-0.35) rectangle (0.35,0.35);
  % junction perimeter（赤で強調）
  \draw[red,very thick] (-1.1,-1.1) rectangle (1.1,1.1);
\end{tikzpicture}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  % substrate window
  \fill[gray!20] (-2.0,-2.0) rectangle (2.0,2.0);
  % trench opening & electrode
  \draw[black,thick] (-1.2,-1.2) rectangle (1.2,1.2); % trench cavity outline
  \fill[black!30] (-0.9,-0.9) rectangle (0.9,0.9);   % electrode inside

  % multiple buried straps around the trench
...
Cross-section（a,b）では Stack は拡散接合の周辺長が限定的であるのに対し，
Trench は \emph{buried strap} で局所接合が複数形成される。
Top view（c,d）から，Trench は 1 セル当たりの \textbf{総周辺長} が増え，
高温条件での接合リーク増大リスクが高いことが分かる。}
