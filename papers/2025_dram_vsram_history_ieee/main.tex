...
\begin{figure}[t]
\centering
{\large\bfseries 0.18\,µm Generation — Cross-sections}\par\vspace{0.6em}

% ---- (a) Stack Cross-section ----
\begin{tikzpicture}[x=1cm,y=1cm]
  % substrate
  \fill[gray!18] (-2,0) rectangle (2,-1.0);
  \draw (-2,0) -- (2,0);
  \node[anchor=east] at (-2,0.15) {\scriptsize $p^-$ substrate};

  % n+ diffusion
  \fill[gray!55] (-0.6,0) rectangle (0.6,-0.25);
  \node[anchor=west] at (0.7,-0.1) {\scriptsize $n^+$ diffusion};

  % electrode
  \fill[gray!45] (-0.2,0) rectangle (0.2,1.0);

  % pn junction perimeter
  \draw[red,very thick] (-0.6,0) -- (-0.6,-0.25) -- (0.6,-0.25) -- (0.6,0);
\end{tikzpicture}

\caption*{(a) Stack Cross-section}   % ← 図の下にキャプションを固定配置

\vspace{1.2em}

% ---- (b) Trench Cross-section ----
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[gray!18] (-2,0) rectangle (2,-2.0);
  \draw (-2,0) -- (2,0);
  \node[anchor=east] at (-2,0.15) {\scriptsize $p^-$ substrate};

  % trench cavity
  \draw[thick] (-0.8,0) -- (-0.8,-1.6) -- (0.8,-1.6) -- (0.8,0);

  % trench electrode
  \fill[gray!45] (-0.35,-0.1) rectangle (0.35,-1.5);

  % buried strap
  \fill[gray!55] (0.95,-0.6) rectangle (1.15,-1.0);
  \draw[red,very thick] (0.95,-0.6) rectangle (1.15,-1.0);
  \node[anchor=west] at (1.25,-0.8) {\scriptsize buried strap};
\end{tikzpicture}

\caption*{(b) Trench Cross-section}

\vspace{1.2em}
{\large\bfseries 0.18\,µm Generation — Top views}\par\vspace{0.8em}

% ---- (c) Stack Top view ----
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[gray!55] (-1,-1) rectangle (1,1);
  \fill[gray!45] (-0.35,-0.35) rectangle (0.35,0.35);
  \draw[red,very thick] (-1,-1) rectangle (1,1);
\end{tikzpicture}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % multiple straps
  \foreach \x/\y in {1.3/0, -1.3/0, 0/1.3, 0/-1.3}{
...
\caption{0.18µm世代における Stack / Trench キャパシタの概念比較。
Stack は拡散接合の周辺長が限定的だが，Trench は複数の buried strap により総周辺長が増加し，高温リークリスクが高い。}
\label{fig:018_stack_trench}
\end{figure}
