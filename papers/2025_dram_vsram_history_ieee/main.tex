...
ライン欠陥やエッジ集中はなく，不良は全体にランダム分布していた（Fig.~\ref{fig:failmap}）。
...
その物理的起源を概念的に示したものが Fig.~\ref{fig:dram_cross_section} である。
...
\end{tikzpicture}
\caption{DRAM セル断面の概念図。赤矢印は $n^+ \rightarrow p^-$ へのリーク電流 $I_{\mathrm{leak}}$ を示す。}
...
\begin{tikzpicture}[scale=0.22]
  ...
  \end{scope}
\end{tikzpicture}
\caption{Pause Refresh Fail のフェイルビットマップ例（ランダム分布）。}
\label{fig:failmap}
\end{figure}
...
\begin{table}[t]
  ...
  \begin{tabular}{p{0.25\linewidth} p{0.65\linewidth}}
    ...
    歩留まり & 65\% $\rightarrow$ 80\%台後半 \\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:resist_flow} に示す切替により，Pause Refresh Fail は大幅に減少し，
...
\begin{axis}[
  ...
  xlabel={プロセス世代}, grid=both,
  legend style={at={(0.5,-0.28)}, anchor=north, draw=none, fill=none, legend columns=2},
  clip=false
]
...
Fig.~\ref{fig:pause_temp_junc} に示すように，温度上昇とともに
ジャンクションリーク $I_{\mathrm{junc}}$ は指数的に増加し，
90\,$^\circ$C 条件下では特に顕著となる。
また，バックバイアスを強化することでリーク増大を有効に抑制できることが確認された。
...
\end{axis}
\end{tikzpicture}
\caption{Disturb Refresh：短チャネル効果で $I_{\mathrm{off}}$ が増大。温度上昇でも指数的に増える。}
...
\subsection{評価対象と背景}
VSRAM立ち上げ後の次世代候補として，
台湾NANYA社の0.18\,$\mu$m DRAMプロセスを利用した
VSRAM試作評価が検討された。
...
ジャンクションリーク $I_{\mathrm{junc}}$ が増大し，
...
\begin{itemize}
  ...
  \item \textbf{第2章}：0.25\,$\mu$m世代64M DRAMでは，
        ...
        プラズマダメージ対策によって歩留まりを80\%台後半まで改善した。
  \item \textbf{第3章}：VSRAM（疑似SRAM）の立ち上げでは，
        モバイル用途特有の低消費電力・90\,$^\circ$C高温要求により
        Pause/Disturb Refresh不良が顕在化した。
        HF洗浄制御やバックバイアス強化，ゲート寸法管理といった施策により，
        歩留まりを30\%から80\%台へ改善することができた。
  \item \textbf{第4章}：NANYA 0.18\,$\mu$mトレンチDRAMプロセスは，
        高温保持特性の不足によりモバイル用途への適用を断念。
        この評価を契機に，エプソンはメモリ事業から撤退し，
        液晶ドライバーICを中心とする高耐圧混載CMOS開発へ戦略を集中させた。
\end{itemize}
...
さらにインクジェット薄膜ピエゾアクチュエータおよび \emph{PrecisionCore} プリントヘッドの製品化に従事した。
