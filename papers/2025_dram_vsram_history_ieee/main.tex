...
\usepackage{float}        % [H] を使う
\usepackage{caption}      % キャプション間隔の調整
...
高性能端末への移行期であり，「シャープ製の世界初のカメラ付き携帯」の実現に、エプソンは先駆けて参入した。
その中で求められたのが，低消費電力かつ高温（90$^\circ$C）動作保証を備えた大容量モバイル向けメモリであった。
...
\begin{itemize}
  \item Pause対策：
    \begin{itemize}
      \item ゲートエッチング後のゲート酸化残膜を確保するため、LDD工程のHF洗浄回数を最小化し，SNコンタクトリークを抑制。
...
\begin{itemize}
  ...
  \item ゲート酸化残膜確保のためのHF洗浄回数最小化，バックバイアス強化，ゲート寸法管理により歩留まりは30\%から80\%台へ改善。
...
\subsection{構造的起因の解析}
Fig.~\ref{fig:stack_trench_018} に示すように，  
...
% ==== 0.18 µm Stack vs. Trench（本文のその位置に固定して余白最小）====
\begin{figure}[H]
\captionsetup{skip=2pt}   % 図とキャプションの間を詰める
\centering

{\large \bfseries 0.18\,\textmu m Generation — Cross-sections}\par\vspace{0.4em}

% ------ (a) Stack Cross-section ------
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[gray!20] (-3.0,0) rectangle (3.0,-1.0);                 % p- substrate
  \node[anchor=east] at (-3.0,0.12) {\small $p^-$ substrate};
  \fill[gray!55] (-0.80,0) rectangle (0.80,-0.50);               % n+ diffusion（細め・浅め）
  \fill[black!30] (-0.25,0) rectangle (0.25,1.00);               % Storage node contact
  \draw[red,very thick] (-0.80,0) -- (-0.80,-0.50) -- (0.80,-0.50) -- (0.80,0); % pn junction perimeter
\end{tikzpicture}
\caption*{(a) Stack Cross-section}

\vspace{1.0em}

% ------ (b) Trench Cross-section（左右に buried strap） ------
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[gray!20] (-3.0,0) rectangle (3.0,-2.0);
  \node[anchor=east] at (-3.0,0.12) {\small $p^-$ substrate};
  \draw[black,thick] (-0.9,0) -- (-0.9,-1.6) -- (0.9,-1.6) -- (0.9,0); % cavity
  \fill[black!30] (-0.60,-0.10) rectangle (0.60,-1.50);                 % electrode
  \fill[gray!55] ( 1.10,-0.70) rectangle ( 1.55,-1.10);                 % strap (R)
  \fill[gray!55] (-1.55,-0.70) rectangle (-1.10,-1.10);                 % strap (L)
  \draw[red,very thick] ( 1.10,-0.70) rectangle ( 1.55,-1.10);          % 接合強調
  \draw[red,very thick] (-1.55,-0.70) rectangle (-1.10,-1.10);
\end{tikzpicture}
\caption*{(b) Trench Cross-section}

\vspace{2.0em}
{\large \bfseries 0.18\,\textmu m Generation — Top views}\par\vspace{0.4em}

% ------ (c) Stack Top view（アクティブさらに小さく） ------
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[gray!20] (-2.0,-2.0) rectangle (2.0,2.0);
  \fill[gray!55] (-0.75,-0.75) rectangle (0.75,0.75);        % ← 0.90 → 0.75 に縮小
  \fill[black!30] (-0.32,-0.32) rectangle (0.32,0.32);       % SN contact
  \draw[red,very thick] (-0.75,-0.75) rectangle (0.75,0.75); % 周辺長の強調
\end{tikzpicture}
\caption*{(c) Stack Top view}

\vspace{1.0em}

% ------ (d) Trench Top view（複数 strap で総周辺長が増大） ------
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[gray!20] (-2.0,-2.0) rectangle (2.0,2.0);
  \draw[black,thick] (-1.2,-1.2) rectangle (1.2,1.2);
  \fill[black!30] (-0.9,-0.9) rectangle (0.9,0.9);
  \foreach \x/\y in {-1.65/0, 1.65/0, 0/1.65, 0/-1.65}{
    \fill[gray!55] (\x-0.35,\y-0.35) rectangle (\x+0.35,\y+0.35);
    \draw[red,very thick] (\x-0.35,\y-0.35) rectangle (\x+0.35,\y+0.35);
  }
\end{tikzpicture}
\caption*{(d) Trench Top view（複数 strap により総周辺長が増大）}

\vspace{2.0em}
\caption{0.18\,\textmu m 世代における Stack/Trench キャパシタの概念比較。
Stack は接合周辺長が限定的，Trench は \emph{buried strap} による局所接合が複数形成され，
1セル当たりの総周辺長が大きくなる。}
\label{fig:stack_trench_018}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{第3章}：VSRAMではモバイル要求（低消費・90$^\circ$C保証）がPause/Disturb不良を顕在化させた。
        ゲート酸化残膜確保のためのHF洗浄回数の最小化，バックバイアス強化，ゲート寸法管理などにより，
...
\appendices
\section{エンベデッドDRAM（eDRAM）技術開発の補遺}
\label{app:edram}

\subsection{背景}
\SI{0.25}{\micro\meter} DRAM の量産終了後，筆者は同プロセスを基盤に，
メモリセル形成以降へエプソンの配線プロセスを統合した
システムLSI向け eDRAM の開発・機種担当を担った。
第二層間膜（ILD2）形成後，メモリセル領域とロジック領域の段差を解消するため
\emph{CMP平坦化}を導入したが，過不足いずれも歩留まりを直撃する難課題であった。

\subsection{CMP平坦化の難しさ}
大面積チップでは局所パターン密度の差により除去量の空間ばらつきが増幅され，
\begin{itemize}
  \item 研磨不足：段差残り $\Rightarrow$ 上層配線／コンタクトの形成不良，
  \item 過研磨：メモリセル電極（ストレージノード）露出 $\Rightarrow$ リーク起点化
\end{itemize}
が顕在化した。CMP条件（スラリー，圧力，速度）に加え，
ダミーパターン最適化を含む\emph{設計密度均し（DFM）}の効きが大きかった。

\subsection{配線プロセスの課題と対策}
配線は4層構成で，メモリセル側を含め全工程は\(\sim\)500工程に達し，
当時の酒田Fabで最長フローであった。
層間膜の \emph{HDP酸化膜} に起因するダメージと応力により，
Al配線に\emph{楔（くさび）型ボイド}が形成される課題が発生した。
対策として，
\begin{itemize}
  \item HDP成膜条件の再最適化（バイアス電力／圧力／温度）でイオンダメージと内部応力を低減，
  \item 最終 \SI{400}{\celsius} 水素シンターの温度最適化で金属／誘電体界面の欠陥準位を緩和
\end{itemize}
を実施し，ボイドは減少，配線信頼性指標（オープン／エレクログラマイグレーション加速試験）は改善傾向を示した。
しかしビジネス案件がロストとなり，量産移行前に開発は終了した。

\subsection{業界動向と位置づけ}
1990年代後半はシステムLSI文脈で \emph{エンベデッドDRAM} が注目されたが，
\begin{enumerate}
  \item 平坦化・配線応力・熱履歴の複合最適化が難しく，
  \item 安定量産の要求（良品率とスループット）との両立が厳しい
\end{enumerate}
という現実が明確になった。
業界の関心は次第に，DRAMとロジックを\emph{実装で混載}（マルチチップや3D実装）する方向へシフトした。

\subsection{その後：VSRAMの歩留まり改善へ}
eDRAM案件終息後，筆者は並行立ち上げ中だった VSRAM（疑似SRAM）の
\(\sim\)30\%という初期歩留まり課題に対応する役割に移行した。
高温・長周期のモバイル要求下で顕在化した
Pause/Disturbの抑制（HF洗浄回数最小化，バックバイアス強化，ゲートCD管理など）により，
量産歩留まりを80\%台まで回復させた（本文 第\ref{fig:yield_all} 図の推移参照）。
