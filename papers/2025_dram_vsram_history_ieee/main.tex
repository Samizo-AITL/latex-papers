...
\begin{scope}
  ...
  \node[anchor=east] at (-2,0.15) {$p^-$ substrate};

  % n+ diffusion
  \fill[gray!50] (-0.6,0) rectangle (0.6,-0.6);
  \node[anchor=west] at (0.7,-0.3) {$n^+$ diffusion};

  % storage node
  \fill[black!30] (-0.2,0) rectangle (0.2,1.2);
  \node[anchor=west] at (0.3,0.8) {Storage Node};

  % pn junction (sides+bottom only)
  \draw[red,very thick] (-0.6,0) -- (-0.6,-0.6) -- (0.6,-0.6) -- (0.6,0);

  % title
  \node at (0,-1.4) {\bfseries (a) Stack Capacitor (断面)};
\end{scope}
...
\begin{scope}[xshift=7cm]
  ...
  \draw[black,thick] (-0.6,0) -- (-0.6,-2) -- (0.6,-2) -- (0.6,0);

  % electrode inside trench
  \fill[black!30] (-0.5,-0.1) rectangle (0.5,-1.9);
  \node[anchor=west] at (0.7,-1.0) {Trench Node};

  % buried strap diffusion
  \fill[gray!50] (0.6,-0.6) rectangle (1.0,-0.2);
  \node[anchor=west] at (1.1,-0.4) {buried strap $n^+$};

  % pn junction around trench sidewalls + strap
  \draw[red,very thick] (-0.6,0) -- (-0.6,-2) -- (0.6,-2) -- (0.6,0);
  \draw[red,very thick] (0.6,-0.6) rectangle (1.0,-0.2);

  % title
  \node at (0,-2.6) {\bfseries (b) Trench Capacitor (断面)};
\end{scope}

\end{tikzpicture}
\caption{Stack と Trench の断面構造。
Stackは小さな拡散層に限定された接合だが，Trenchは側壁・底面＋buried strap周辺に広い接合が形成される。}
\label{fig:stack_trench_cross}
\end{figure}
...
\begin{scope}
  ...
  \node at (1,1) {$n^+$ diffusion};
  % pn junction perimeter
  \draw[red,very thick] (0,0) rectangle (2,2);
  \node[red,below] at (1,0) {\scriptsize 周辺長短い};
  \node at (1,2.3) {\bfseries (a) Stack Top view};
\end{scope}
...
\begin{scope}[xshift=6cm]
  % substrate block
  \fill[gray!30] (0,0) rectangle (2,2);
  % trench pattern (multiple deep grooves)
  \foreach \x in {0.4,1.0,1.6}{
    \fill[white] (\x,0.3) rectangle +(0.25,1.4);
    \draw[red,very thick] (\x,0.3) rectangle +(0.25,1.4);
  }
  \node[red,below] at (1,0) {\scriptsize 周辺長長い};
  \node at (1,2.3) {\bfseries (b) Trench Top view};
\end{scope}

\end{tikzpicture}
\caption{上面図によるStackとTrenchの接合境界比較。
Stackは矩形で単純な周辺長だが，Trenchは多数の側壁に沿ってpn接合が形成され，
周辺長が増大する。}
\label{fig:stack_trench_top}
\end{figure}
