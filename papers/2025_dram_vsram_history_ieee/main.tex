...
% --- (a) Stack Capacitor ---
\node at (0,3.2) {\textbf{(a) Stack Capacitor}};

% diffusion region
\draw[thick] (-0.5,2.0) rectangle (0.5,2.3);
\node at (0,1.8) {n$^+$ diffusion};

% storage node contact
\draw[thick] (-0.1,2.3) -- (-0.1,3.0) -- (0.1,3.0) -- (0.1,2.3);
\node at (1.2,2.6) {Storage Node};

% 赤線 (junction area stack)
\draw[red,very thick] (-0.5,2.0) -- (0.5,2.0);
\node[red,below] at (0,1.9) {接合面積（小）};

% --- (b) Trench Capacitor ---
\node at (0,1.0) {\textbf{(b) Trench Capacitor}};

% substrate
\node at (0,0.7) {p$^-$ substrate};

% trench diffusion
\draw[thick] (-0.5,0.5) rectangle (0.5,-1.5);
\node at (0,-1.7) {n$^+$ diffusion};

% 赤線 (junction area trench)
\draw[red,very thick] (-0.5,0.5) -- (-0.5,-1.5); % left wall
\draw[red,very thick] (0.5,0.5) -- (0.5,-1.5);   % right wall
\draw[red,very thick] (-0.5,-1.5) -- (0.5,-1.5); % bottom
\node[red,right] at (0.6,-0.5) {接合面積（大）};

\end{tikzpicture}
\caption{Stack型とTrench型キャパシタの構造比較。赤線はジャンクション接合面積を示す。
Stackは拡散層上にストレージノードを積層するため接合面積が小さいが，
Trenchは側壁・底面全体が接合面となるためリーク経路が大きい。}
