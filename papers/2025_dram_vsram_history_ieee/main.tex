...
% --- Cross-sections ---
\textbf{Cross-sections}\\[3pt]

(a) Stack Cross-section\\
\begin{tikzpicture}[x=1cm,y=1cm]
  % stack example
  \fill[gray!20] (-1,-1) rectangle (1,1);
  \fill[black!30] (-0.3,-1) rectangle (0.3,1);
  \draw[red,very thick] (-0.3,0.5) rectangle (0.3,1);
\end{tikzpicture}\\[8pt]
...
\begin{tikzpicture}[x=1cm,y=1cm]
  % trench example
  \fill[gray!20] (-1,-1) rectangle (1,1);
  \draw[black,thick] (-0.7,-1) -- (-0.7,1) (0.7,-1) -- (0.7,1);
  \fill[black!30] (-0.5,-1) rectangle (0.5,1);
  \draw[red,very thick] (-0.5,0.6) rectangle (0.5,1);
\end{tikzpicture}\\[8pt]
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % 複数 strap
  \fill[gray!55] (0.80,-0.25) rectangle (1.15,0.25);
  \draw[red,very thick] (0.80,-0.25) rectangle (1.15,0.25);

  \fill[gray!55] (-0.25,0.80) rectangle (0.25,1.15);
  \draw[red,very thick] (-0.25,0.80) rectangle (0.25,1.15);

  \fill[gray!55] (-0.25,-1.15) rectangle (0.25,-0.80);
  \draw[red,very thick] (-0.25,-1.15) rectangle (0.25,-0.80);
\end{tikzpicture}

\caption{Stack と Trench キャパシタ構造の比較：
断面 (a,b) および上面 (c,d)。Stack は接合周辺長が限定的だが，
Trench は \textit{buried strap} が 1セル内に複数配置されるため
接合の総周辺長が増大し，高温リークのリスクが高い。}
