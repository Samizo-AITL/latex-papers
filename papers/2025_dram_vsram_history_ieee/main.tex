...
\begin{scope}[shift={(-3,4)}]
  \fill[gray!20] (-1.5,0) rectangle (1.5,-0.8); % substrate
  \fill[gray!55] (-0.5,0) rectangle (0.5,-0.6); % n+ diffusion
  \fill[black!30] (-0.15,0) rectangle (0.15,1.0); % electrode
...
\begin{scope}[shift={(3,4)}]
  \fill[gray!20] (-1.5,0) rectangle (1.5,-2.0); % substrate
  \draw[black,thick] (-0.5,0) -- (-0.5,-1.6) -- (0.5,-1.6) -- (0.5,0); % trench cavity
  \fill[black!30] (-0.35,-0.1) rectangle (0.35,-1.5); % electrode
  \fill[gray!55] (0.55,-0.6) rectangle (0.95,-1.0); % buried strap
  \draw[red,very thick] (0.55,-0.6) rectangle (0.95,-1.0); % pn junction
  \node[anchor=north] at (0,-2.2) {\bfseries (b) Trench Cross-section};
\end{scope}

% ==== (c) Stack Top view ====
\begin{scope}[shift={(-3,-2)}]
  \fill[gray!20] (-1.5,-1.5) rectangle (1.5,1.5); % substrate
  \fill[gray!55] (-0.8,-0.8) rectangle (0.8,0.8); % n+ diffusion area
  % electrode only in the center (no faint square)
  \fill[black!30] (-0.3,-0.3) rectangle (0.3,0.3); % electrode contact
  \draw[red,very thick] (-0.8,-0.8) rectangle (0.8,0.8); % pn junction perimeter
...
\begin{scope}[shift={(3,-2)}]
  \fill[gray!20] (-1.5,-1.5) rectangle (1.5,1.5); % substrate
  \draw[black,thick] (-0.8,-0.8) rectangle (0.8,0.8); % trench cavity outline
  \fill[black!30] (-0.5,-0.5) rectangle (0.5,0.5); % trench electrode
  \fill[gray!55] (0.9,-0.3) rectangle (1.3,0.3); % buried strap contact
  \draw[red,very thick] (0.9,-0.3) rectangle (1.3,0.3); % pn junction at strap
  \node[anchor=north] at (0,-1.9) {\bfseries (d) Trench Top view};
\end{scope}

\end{tikzpicture}

\caption{Stack型とTrench型キャパシタ構造の断面図（上段）と上面図（下段）の比較。
Stackは接合周辺長が限定的であるのに対し，
Trenchはburied strap起点で周辺長が増大し，高温リークのリスクが高い。}
