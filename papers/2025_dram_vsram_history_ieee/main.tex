...
% ===== (a) Stack Capacitor =====
\node[font=\bfseries] at (0,3.2) {(a) Stack Capacitor};

% p-substrate
\fill[gray!10] (-1.4,1.9) rectangle (1.4,1.2);
\node at (0,2.15) {$p^-$ substrate};

% n+ diffusion (planar)
\fill[gray!60] (-0.5,1.2) rectangle (0.5,1.0);
\node at (0,0.8) {$n^+$ diffusion};

% storage-node contact + stack node
\fill[black!35] (-0.1,1.2) rectangle (0.1,2.8);
\node[anchor=west] at (0.15,2.2) {Storage Node};

% junction (pn) = red perimeter at diffusion/ substrate interface
\draw[red,very thick] (-0.5,1.0) -- (0.5,1.0);
\draw[red,very thick] (-0.5,1.0) -- (-0.5,1.2);
\draw[red,very thick] (0.5,1.0) -- (0.5,1.2);
\node[red,below] at (0,0.95) {pn 接合（面積小）};

% ===== (b) Trench Capacitor with Buried Strap =====
\node[font=\bfseries] at (0,0.3) {(b) Trench Capacitor};

% p-substrate
\fill[gray!10] (-1.6,-0.2) rectangle (1.6,-1.0);
\node at (0,0.05) {$p^-$ substrate};

% deep trench (node poly inside)
\draw[thick] (-0.35,-0.2) -- (-0.35,-1.6) -- (0.35,-1.6) -- (0.35,-0.2);
\node at (0,-1.8) {Trench (node)};

% buried strap connecting node to diffusion
\fill[black!35] (0.35,-0.6) -- (0.7,-0.6) -- (0.7,-0.2) -- (0.35,-0.2) -- cycle;
\node[anchor=west] at (0.72,-0.38) {Buried Strap};

% n+ diffusion extended along strap region
\fill[gray!60] (0.6,-1.0) rectangle (1.2,-0.2);
\node[anchor=west] at (1.25,-0.6) {$n^+$ diffusion};

% pn junction perimeter (larger due to lateral extension)
\draw[red,very thick] (0.6,-0.2) -- (1.2,-0.2);   % top edge of diffusion
\draw[red,very thick] (0.6,-1.0) -- (1.2,-1.0);   % bottom edge
\draw[red,very thick] (0.6,-1.0) -- (0.6,-0.2);   % left edge
\draw[red,very thick] (1.2,-1.0) -- (1.2,-0.2);   % right edge
% optional: small red along strap-diffusion corner to stress perimeter increase
\draw[red,very thick] (0.35,-0.2) -- (0.6,-0.2);

\node[red,below right] at (1.2,-1.0) {pn 接合（面積・周長が大）};
\end{tikzpicture}

\caption{Retention に効くリークは \emph{pn 接合（n$^+$拡散とp$^-$基板）のリーク}。
(a) Stack では平面拡散が小さく接合面積が小さい。
(b) Trench ではノードを結ぶ \emph{buried strap} により拡散が側方へ広がり、
接合面積・周長が増えてリーク経路が増大しやすい。誘電体側壁は pn 接合ではない。}
\label{fig:stack_trench_junction_correct}
\end{figure}
