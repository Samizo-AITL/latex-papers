...
\end{tikzpicture}
\label{fig:trench_xsec_b}}
\\[0.8em]

% --- c,d（横並び、1カラムに収める） ---
\subfloat[(c) Stack Top view]{%
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[gray!20] (-1.5,-1.5) rectangle (1.5,1.5); % p-sub: 残す
  \fill[gray!55] (-0.8,-0.8) rectangle (0.8,0.8);
  % 中央の薄い四角は削除（前回指摘反映）
  % \fill[black!15] (-0.3,-0.3) rectangle (0.3,0.3);
  \draw[red,very thick] (-0.8,-0.8) rectangle (0.8,0.8);
\end{tikzpicture}
\label{fig:stack_top_c}}
\hfill
...
\end{tikzpicture}
\label{fig:trench_top_d}}

\caption{Stack型とTrench型キャパシタ構造：断面（a,b）と上面（c,d）の比較。}
