...
\begin{tikzpicture}[x=1cm,y=1cm,scale=0.85,
  ...
  \node at (-3.5,-0.6) {$p^{-}$~substrate};

  % --- n+ 拡散層（SN接続領域） ---
  \fill[gray!60] (-0.6,-0.6) rectangle (1.2,0);
  \node at (0.3,-0.35) {$n^{+}$};

  % --- SN へのコンタクト2本 ---
  \fill[white] (-0.25,0) rectangle (-0.05,1.5);
  \draw (-0.25,0) rectangle (-0.05,1.5);
  \fill[white] (0.85,0) rectangle (1.05,1.5);
  \draw (0.85,0) rectangle (1.05,1.5);

  % --- SN（セルキャパシタ側の上部構造を矩形で表現） ---
  \draw (-0.7,1.5) rectangle (1.25,2.4);
  \node at (0.28,1.95) {\textbf{SN}};

  % --- WL（ポリシリコン） ---
  \fill[gray!40] (-2.2,-0.05) rectangle (-1.9,0.9);
  \draw (-2.2,-0.05) rectangle (-1.9,0.9);
  \node[above] at (-2.05,0.9) {WL};

  % --- n+ ブラケット表示（任意） ---
  \draw[decorate,decoration={brace,amplitude=5pt}] (-0.6,-0.75) -- (1.2,-0.75);
  \node at (0.3,-1.0) {$n^{+}$ diffusion};

  % --- LOCOS（右フィールド酸化膜の盛り上がり） ---
  \begin{scope}
    \fill[gray!20]
      (1.4,0) .. controls (1.9,0.55) and (2.5,0.55) .. (3.0,0) -- (1.4,0) -- cycle;
    \draw (1.4,0) .. controls (1.9,0.55) and (2.5,0.55) .. (3.0,0);
  \end{scope}
  \node[right] at (3.05,0.25) {LOCOS};

\end{tikzpicture}
\caption{DRAM セル断面の概念図（SN 接続／WL／$n^{+}$拡散層／$p^{-}$基板／LOCOS）。}
