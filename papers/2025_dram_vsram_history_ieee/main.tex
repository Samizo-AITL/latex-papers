...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[anchor=north west] at (0,-0.05) {\footnotesize $p^-$ substrate};
  % n+ diffusion
  \fill[gray!50] (1.5,-0.01) rectangle (2.5,-0.3);
  \node[anchor=north] at (2,-0.35) {\footnotesize $n^+$ diffusion};
  % Stack capacitor
  \draw[thick] (1.5,0) -- (1.5,0.8) -- (2.5,0.8) -- (2.5,0);
  \node[anchor=south] at (2,0.85) {\footnotesize Stack Capacitor};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.9]
  % 基板
  \fill[gray!20] (0,0) rectangle (4,-1.5);
  \node[anchor=north west] at (0,-0.05) {\footnotesize $p^-$ substrate};
  % Trench
  \fill[gray!50] (1.9,0) rectangle (2.1,-1.2);
  \node[anchor=south] at (2,0.05) {\footnotesize Trench Capacitor};
  % 拡散層
  \draw[red,very thick] (1.9,-1.2) -- (2.1,-1.2);
  \node[anchor=north] at (2,-1.25) {\footnotesize $n^+$ diffusion};
\end{tikzpicture}

\caption{(a) スタックキャパシタ（接合面積は上面）と (b) トレンチキャパシタ（側壁で接合面積が大）。縦に並べて比較。}
