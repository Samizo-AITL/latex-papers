...
% 要: \usepackage{circuitikz}
\begin{figure}[t]
\centering
\begin{circuitikz}[scale=0.9]
  % 基本ノード
  \coordinate (SN)  at (0,2.4);    % Storage Node
  \coordinate (TOP) at (0,4.2);    % キャパシタ上板
  \coordinate (SUB) at (-1.2,0);   % サブストレート
  \coordinate (BL)  at (3.8,2.4);  % ビットライン

  % セルキャパシタ + プレート電源 Vcell
  \draw (SN) to[C,l_=$C_{\mathrm{cell}}$] (TOP)
        (TOP) to[V,l=$V_{\mathrm{cell}}$] ++(0,1.6) node[ground]{};

  % ジャンクションリーク SN -> SUB
  \draw (SN) to[R,l=$R_{\mathrm{junc}}$] (SUB);
  \draw (SUB) node[ground]{};
  \draw[->] (0.6,1.3) -- ++(0.9,0) node[midway,below]{$I_{\mathrm{junc}}$};

  % 酸化膜リーク (Ccell 経路の微小リークを破線で示す)
  \draw[dashed] ($(SN)+(0.55,0.9)$) to[R,l_=$R_{\mathrm{ox}}$] ++(0,1.1);
  \draw[->] (1.3,3.1) -- ++(0.9,0) node[midway,above]{$I_{\mathrm{ox}}$};

  % NMOS アクセス (SN–BL) と WL
  \draw (SN) -- ++(0.7,0) node[nmos, anchor=S, scale=0.9] (Macc) {};
  \draw (Macc.D) -- (BL) node[right]{BL};
  \draw (Macc.G) -- ++(0,1.0) node[right]{WL};
  \node[left] at (SN) {SN};

  % バルク(ボディ)端子を取り出して Vbs を印加
  \draw (Macc.B) |- (-2.0,1.0) to[V,l_=$V_{bs}$] (-2.0,0) node[ground]{};
\end{circuitikz}

\caption{1T1Cセルの等価回路とリーク経路（$I_{\mathrm{leak}}=I_{\mathrm{junc}}+I_{\mathrm{ox}}+\cdots$）。
バックバイアス $V_{bs}$ を印加して $V_{\mathrm{th}}$ を上げ，リーク／誤反転を抑制。}
