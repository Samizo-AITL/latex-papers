...
% ==== Fig. X  0.18µm Generation : Stack vs Trench (Simple Cross-sections & Top views) ====
\begin{figure}[t]
\centering

{\large\bfseries 0.18\,$\mu$m Generation — Cross-sections}\par\vspace{0.6em}

% ---------- (a) Stack Cross-section ----------
\begin{tikzpicture}[x=1cm,y=1cm]
  % substrate
  \fill[gray!18] (-2.0,0) rectangle (2.0,-1.0);
  \draw (-2.0,0) -- (2.0,0);
  \node[anchor=east] at (-2.0,0.12) {\scriptsize $p^-$ substrate};

  % n+ diffusion
  \fill[gray!55] (-0.9,0) rectangle (0.9,-0.25);
  \node[anchor=west] at (1.0,-0.12) {\scriptsize $n^+$ diffusion};

  % storage node (stacked electrode)
  \fill[gray!45] (-0.20,0) rectangle (0.20,1.0);

  % pn junction perimeter
  \draw[red,very thick] (-0.9,0) -- (-0.9,-0.25) -- (0.9,-0.25) -- (0.9,0);

  \node at (0,1.2) {\small (a) Stack Cross-section};
\end{tikzpicture}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  % substrate
  \fill[gray!18] (-2.0,0) rectangle (2.0,-2.0);
  \draw (-2.0,0) -- (2.0,0);
  \node[anchor=east] at (-2.0,0.12) {\scriptsize $p^-$ substrate};

  % trench cavity
  \draw[thick] (-0.5,0) -- (-0.5,-1.6) -- (0.5,-1.6) -- (0.5,0);
  % electrode
  \fill[gray!45] (-0.35,-0.1) rectangle (0.35,-1.5);

  % buried strap
  \fill[gray!55] (0.55,-0.6) rectangle (0.95,-1.0);
  \draw[red,very thick] (0.55,-0.6) rectangle (0.95,-1.0);
  \node[anchor=west] at (1.05,-0.8) {\scriptsize buried strap};

  \node at (0,-1.9) {\small (b) Trench Cross-section};
\end{tikzpicture}

\vspace{2.0em}
{\large\bfseries 0.18\,$\mu$m Generation — Top views}\par\vspace{0.8em}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % electrode
  \fill[gray!45] (-0.3,-0.3) rectangle (0.3,0.3);
  % pn junction perimeter
  \draw[red,very thick] (-1.0,-1.0) rectangle (1.0,1.0);

  \node at (0,-1.3) {\small (c) Stack Top view};
\end{tikzpicture}

\vspace{1.0em}

% ---------- (d) Trench Top view (multiple straps) ----------
\begin{tikzpicture}[x=1cm,y=1cm]
  % trench cavity
  \draw[thick] (-1.0,-1.0) rectangle (1.0,1.0);
  % electrode
  \fill[gray!45] (-0.6,-0.6) rectangle (0.6,0.6);

  % multiple buried straps
  \foreach \x/\y in {1.2/0.2, -1.2/-0.3, 0.2/-1.2, -0.2/1.2}{
    \fill[gray!55] (\x-0.2,\y-0.2) rectangle (\x+0.2,\y+0.2);
    \draw[red,very thick] (\x-0.2,\y-0.2) rectangle (\x+0.2,\y+0.2);
  }

  \node at (0,-1.5) {\small (d) Trench Top view（複数 strap で総周辺長が増大）};
\end{tikzpicture}

\caption{ \SI{0.18}{\micro\meter} 世代における Stack / Trench キャパシタの概念比較。
Stack は接合周辺長が限定的だが，Trench は埋め込み strap による局所 $pn$ 接合が複数形成され，
セル全体として総周辺長が増え，高温リークのリスクが高い。}
\label{fig:018_stack_trench_simple}
\end{figure}
