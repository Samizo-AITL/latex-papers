...
% ===== 世代別 歩留まり推移（0.5µm, 0.35µm, 0.25µm, VSRAM）=====
\begin{figure}[t]
\centering
\begin{tikzpicture}
\begin{axis}[
  ...
  ymin=0, ymax=100,
  ylabel={歩留まり(\%)},
  % ← ここで内部キーはASCIIだけにする（エラー回避）
  symbolic x coords={p05,p035,p025,vsram},
  xtick=data,
  % 見た目のラベルはここで整形
  xticklabels={
    $0.5\,\mu\mathrm{m}\ (16\mathrm{M})$,
    $0.35\,\mu\mathrm{m}\ (64\mathrm{M})$,
    $0.25\,\mu\mathrm{m}\ (64\mathrm{M})$,
    VSRAM
  },
  ...
  \addplot+[semithick,mark=*]
    coordinates {(p05,95) (p035,20) (p025,65) (vsram,30)};
  \addlegendentry{初期}

  % 改善後
  \addplot+[semithick,mark=square*]
    coordinates {(p05,95) (p035,86) (p025,88) (vsram,80)};
  \addlegendentry{改善後}
\end{axis}
\end{tikzpicture}
\caption{酒田Fabにおける世代別歩留まり推移（初期／改善後）。}
\label{fig:yield_all}
\end{figure}
