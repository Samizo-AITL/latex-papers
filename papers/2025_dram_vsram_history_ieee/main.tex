...
% 前文で:
% \usepackage{float} % <== これを入れる

% ==== 0.18µm Stack vs. Trench 図（本文中に固定配置）====
\begin{figure}[H] % ← H で本文のその位置に固定
\centering
...
% ---------- (b) Trench Cross-section（左右に strap） ----------
\begin{tikzpicture}[x=1cm,y=1cm]
  % p- substrate
  ...
  \fill[gray!55] (-1.55,-0.70) rectangle (-1.10,-1.10);
...
% ---------- (c) Stack Top view（アクティブ縮小） ----------
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[gray!20] (-2.0,-2.0) rectangle (2.0,2.0);   % substrate window
  \fill[gray!55] (-0.90,-0.90) rectangle (0.90,0.90); % ← 小さめの n+ active
  \fill[black!30] (-0.35,-0.35) rectangle (0.35,0.35); % SN contact
  \draw[red,very thick] (-0.90,-0.90) rectangle (0.90,0.90); % 周辺強調
\end{tikzpicture}
\caption*{(c) Stack Top view}

\vspace{0.8em}

% ---------- (d) Trench Top view（複数 strap で総周辺長増） ----------
\begin{tikzpicture}[x=1cm,y=1cm]
  \fill[gray!20] (-2.0,-2.0) rectangle (2.0,2.0);
  \draw[black,thick] (-1.2,-1.2) rectangle (1.2,1.2); % cavity outline
  \fill[black!30] (-0.9,-0.9) rectangle (0.9,0.9);   % electrode
...
Stack は接合周辺長が限定的，Trench は \emph{buried strap} による局所接合が複数でき，
Top view から 1 セル当たりの総周辺長が大きくなることが分かる。}
