...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \fill[black!30] (-0.12,0.00) rectangle (0.12,1.2);
  \node[anchor=west] at (0.20,0.9) {\small Storage Node};

  % pn junction (sides+bottom)
  \draw[red, very thick] (-0.45,0) -- (-0.45,-0.6);
  \draw[red, very thick] (0.45,0) -- (0.45,-0.6);
  \draw[red, very thick] (-0.45,-0.6) -- (0.45,-0.6);

  % title
  \node[anchor=west] at (-2,1.45) {\bfseries (a) Stack Cross-section};
\end{tikzpicture}
\end{minipage}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % trench electrode (storage node)
  \fill[black!30] (-0.35,-0.08) rectangle (0.35,-1.8);

  % buried strap diffusion
  \fill[gray!55] (0.45,-0.5) rectangle (0.85,-0.95);
  \node[anchor=west] at (0.9,-0.7) {\small buried strap $n^+$};

  % pn junction boundary
  \draw[red, very thick] (0.45,-0.5) rectangle (0.85,-0.95);

  % title
  \node[anchor=west] at (-2,0.8) {\bfseries (b) Trench Cross-section};
\end{tikzpicture}
\end{minipage}
...
% --- (c) Stack Top view ---
\begin{tikzpicture}[x=1cm,y=1cm]
  \draw[red, thick] (0,0) rectangle (1,1); % n+ diffusion square
  \node at (0.5,-0.3) {\small n$^+$ diffusion};
\end{tikzpicture}
\node at (0,-0.8) {\bfseries (c) Stack Top view};
\end{minipage}
\hspace{1em}
\begin{minipage}{0.45\linewidth}
\centering
% --- (d) Trench Top view (multi trench per cell) ---
\begin{tikzpicture}[x=1cm,y=1cm]
  \draw[red, thick] (0,0) rectangle (0.3,1.0);
  \draw[red, thick] (0.5,0) rectangle (0.8,1.0);
  \draw[red, thick] (1.0,0) rectangle (1.3,1.0);
  \node at (0.65,-0.3) {\small multiple trenches};
\end{tikzpicture}
\node at (0,-0.8) {\bfseries (d) Trench Top view};
\end{minipage}

\caption{Stack と Trench キャパシタ構造比較。
(a)(b)：断面図。Stackは表面拡散の底面＋側壁がpn接合領域。
Trenchはburied strap周辺がpn接合となる。
(c)(d)：上面図。Stackは1セル1拡散で周辺長が短い。
Trenchは複数のトレンチを並列配置でき、周辺長が長くリークが増えやすい。}
\label{fig:stack_trench_layout}
\end{figure}
