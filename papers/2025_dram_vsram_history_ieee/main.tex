...
\textbf{(English)}\\
This paper reviews the period from 1997 to 2001, during which Seiko Epson's Sakata Fab
rapidly assimilated DRAM process technologies transferred from Mitsubishi Electric,
covering the 0.5\,$\mu$m $\rightarrow$ 0.35\,$\mu$m $\rightarrow$ 0.25\,$\mu$m generations.
Through these transfers, the fab not only ramped DRAM but also leveraged VSRAM
(pseudo-SRAM) as a bridge to apply the acquired knowledge to logic and mixed-voltage CMOS
devices such as LCD drivers.
Based on the author’s first-hand experience, this paper organizes the technical and
strategic aspects of that transition, highlighting the physical origins and countermeasures
of key failure modes (Pause/Disturb Refresh), the yield evolution, and how the acquired
know-how directly contributed to subsequent high-voltage mixed CMOS development.
\end{abstract}
...
本プロジェクトでは，従来の一括条件適用方式ではなく，
\emph{Short Cycle Feedback (SCF)} 方式が採用された。
これは短いサイクルで評価とフィードバックを繰り返し，
工程条件を迅速に最適化する手法である。
...
対策として以下が実施された。
\begin{itemize}
  \item アッシング条件を低パワー・短時間化
  \item ゲートエッチ後にサクリフィシャル酸化膜を再成長
  \item 低残渣レジストを導入し，アッシング時間を削減
  \item 高温アニールを組み込み，欠陥準位を回復
\end{itemize}

これらの対策の結果，
Pause Refresh Fail は大幅に減少し，
歩留まりは65\%から80\%台後半へ改善した。
また90\,$^\circ$C条件での信頼性試験でも保持時間規格を満足し，
量産移行可能なレベルに到達した。
...
プロセス最適化により，歩留まりを大幅に改善することができた。
この知見は，単なるDRAM流用にとどまらず，
以後の高耐圧混載CMOS開発にも活かされる基盤となった。
