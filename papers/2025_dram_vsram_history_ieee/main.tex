...
% --- Stack Capacitor ---
\draw[thick] (0,0) rectangle (1,0.5); % diffusion
\node at (0.5,-0.3) {n$^+$ diffusion};

\draw[thick] (0.4,0.5) -- (0.4,1.5) -- (0.6,1.5) -- (0.6,0.5); % storage node
\node at (1.6,1.0) {Storage Node};

\draw[thick] (0.4,0.5) -- (0.4,0.0);
\draw[thick] (0.6,0.5) -- (0.6,0.0);
\node at (0.5,0.8) {Contact};

% --- 赤線 (junction area for Stack) ---
\draw[red,very thick] (0,0) -- (1,0);
\node[red,below] at (0.5,-0.6) {接合面積（小）};

% Label
\node at (0.5,2) {\textbf{(a) Stack Capacitor}};

% --- Trench Capacitor ---
\begin{scope}[xshift=5cm]
\draw[thick] (0,0) rectangle (1,-2); % trench
\node at (0.5,0.3) {p$^-$ substrate};
\node at (0.5,-2.3) {n$^+$ diffusion};

% --- 赤線 (junction area for Trench) ---
\draw[red,very thick] (0,-0.05) -- (0,-2); % left wall
\draw[red,very thick] (1,-0.05) -- (1,-2); % right wall
\draw[red,very thick] (0,-2) -- (1,-2);   % bottom
\node[red,right] at (1.1,-1.2) {接合面積（大）};

% Label
\node at (0.5,0.8) {\textbf{(b) Trench Capacitor}};
\end{scope}

\end{tikzpicture}
\caption{Stack型とTrench型キャパシタの構造比較。
赤線はジャンクション接合面積を示す。Stackは小さいが，
Trenchは側壁・底面が広いためリークが増加しやすい。}
\label{fig:stack_trench_junction}
\end{figure}
