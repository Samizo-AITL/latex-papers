...
%==================== 2カラム断面図：PZT上電極-ACF-COF／下端子-SUS島 ====================
\begin{figure*}[t]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  x=1mm,y=1mm,>=Latex,
  lw/.style={line width=0.35pt},
  bx/.style={draw, lw, fill=#1},
  lab/.style={font=\scriptsize},
  call/.style={lab, align=left, anchor=west}
]

% -------- 凡例（上段） --------
\begin{scope}[shift={(0,82)}]
  \draw[bx=black!8]   (0,0) rectangle (10,4);   \node[lab,anchor=west] at (12,2){COF};
  \draw[bx=black!15] (28,0) rectangle (38,4);   \node[lab,anchor=west] at (40,2){PZT（内部多層電極）};
  \draw[bx=black!60] (78,0) rectangle (88,4);   \node[lab,anchor=west] at (90,2){SUS島（振動フィルム上）};
  \draw[bx=black!5]  (112,0) rectangle (122,4); \node[lab,anchor=west] at (124,2){Si};
  \draw[bx=black!2]  (146,0) rectangle (156,4); \node[lab,anchor=west] at (158,2){振動フィルム（PPT?）};
  \draw[bx=black!28] (182,0) rectangle (192,4); \node[lab,anchor=west] at (194,2){ACF接合（出力）};
  \draw[bx=black!45] (212,0) rectangle (222,4); \node[lab,anchor=west] at (224,2){はんだ接合（入力）};
\end{scope}

% -------- Si と キャビティ／振動フィルム --------
\draw[bx=black!5] (0,0) rectangle (210,12);          % Siベース
\draw[lw, fill=white] (34,0.4) rectangle (176,12);    % キャビティ
\node[lab] at (105,2.2){キャビティ};
\node[lab,anchor=west] at (177,2.2){Si（基板）};

% 振動フィルム（PPT?）
\draw[bx=black!2] (0,12) rectangle (210,13.2);
\node[lab,anchor=west] at (211,12.6){振動フィルム（PPT?）};

% -------- 振動フィルム上の SUS島（PZT下端子の受け） --------
\draw[bx=black!60] (96,13.2) rectangle (114,16.2);   % SUS island
\node[lab,anchor=west] at (115,15){SUS島（下端子の接合部）};

% -------- 中央：バルクPZT（上：電極／下：端子） --------
\draw[bx=black!15] (96,16.2) rectangle (114,68);     % PZT本体
% 内部の垂直方向 多層電極（d31表現：薄いスライス線）
\foreach \y in {20,26,32,38,44,50,56,62}{
  \draw[lw,black!40] (96,\y) -- (114,\y);
}
% 上部「電極」ラベル帯
\draw[lw, fill=black!10] (96,60) rectangle (114,68);
\node[lab] at (105,64){上部電極};
% 下端子（金属パッドを明示）
\draw[lw, fill=black!30] (100,16.2) rectangle (110,17.8);
\node[lab,anchor=east] at (95.5,17.0){下端子};

% -------- 右：COF と IC --------
\draw[bx=black!8]  (150,16.2) rectangle (158,68);   % COF縦ストリップ（右）
\node[lab,rotate=90] at (154,42){COF};
\draw[draw, lw, rounded corners=0.9mm, fill=black!25] (146,58) rectangle (158,66); % IC
\node[lab,anchor=west] at (159,62){IC};
\draw[->,lw] (154,68) -- (154,74);
\node[lab,anchor=south] at (154,74){配線（信号 / $-10$V 等）};

% -------- 出力：PZT上部電極 と COF出力端子を ACF で密着接合 --------
% ACFブリッジ（隙間なしで密着）: PZT上部電極（右端）と COF左端
\draw[bx=black!28] (114,62) rectangle (150,66);
\draw[->,lw] (170,56) -- (150,64);
\node[call] at (171,56){出力（PZT上電極$\leftrightarrow$COF）\\ACF接合（密着）};

% -------- 入力：COF→Si基板（下部）はんだ --------
\draw[bx=black!45] (151,18.0) rectangle (157,19.8);  % COF下端～Siパッド
\draw[lw] (154,16.2) -- (154,18.0);
\draw[->,lw] (130,18.9) -- (151,18.9);
\node[lab,anchor=east] at (129.5,18.9){入力（COF$\rightarrow$基板） はんだ接合（Sn系）};

% -------- PZT下端子 → SUS島への電気・機械接合 --------
\draw[->,lw] (110,17.0) -- (114,17.0);
\node[call,anchor=east] at (109.5,21){PZT下端子\\SUS島に接合};

% -------- 溶剤蒸気 膨潤リスク（ACF付近） --------
\draw[->,lw] (40,66) -- (114,64);
\node[call,anchor=east] at (39,66){溶剤蒸気環境\\ACF樹脂の膨潤リスク};

% -------- I/O分離強調 --------
\node[lab,anchor=west] at (5,34){I/O分離：出力＝ACF（上電極）／入力＝はんだ（基板）};

\end{tikzpicture}
}
\caption{断面概念図（スケッチ準拠・2カラム）。中央の\textbf{バルクPZT}は内部に\textbf{垂直方向の多層電極}を持ち（d31動作）、\textbf{上部は電極}、\textbf{下部は端子}。上部電極は\textbf{COFの出力端子とACFで密着接合}し、下端子は\textbf{振動フィルム上のSUS島}に接合する。PZTはキャビティ上、フィルム（PPT?）を介してSi基板に載る。}
\label{fig:mach_dual_bond_xsec_pzt_topElectrode}
\end{figure*}
%=================================================================================
