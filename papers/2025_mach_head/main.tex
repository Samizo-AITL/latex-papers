...
% 色・塗り分け（TikZのfill指定に必須）
\usepackage{xcolor}

% 日本語・英語のフォント設定（任意：体裁を整える）
\usepackage{luatexja-fontspec}
\setmainfont{TeX Gyre Termes}     % IEEE向けTimes系
...
% 表のX列の縦中央寄せ（TabularXの見栄え向上）
\renewcommand\tabularxcolumn[1]{m{#1}}

% 最終ページの段落バランス（2段組の空白防止）
\usepackage{balance}
% \balance  % 結論セクション直前あたりで有効化
...
\begin{tikzpicture}[x=1mm,y=1mm,>=Latex,
  block/.style={draw, line width=0.3pt},
  si/.style={block, fill=black!5},
  pzt/.style={block, fill=black!10},
  cavity/.style={block, fill=white},
  film/.style={block, fill=black!2},
  bondACF/.style={block, fill=black!25},   % ACF（濃いめの灰）
  bondSolder/.style={block, fill=black!40},% はんだ（さらに濃い灰）
  lab/.style={font=\scriptsize}
]

% ---- 基板Si（左右の脚）とキャビティ上面
\draw[si] (0,0) rectangle (18,6);            % Siベース
\draw[cavity, line width=0.4pt, fill=white] (4,0.2) rectangle (14,6); % キャビティ（開口）
\node[lab] at (9,1.2) {キャビティ};
\node[lab] at (16.2,1.3) {Si};

% ---- キャビティ上の薄膜（保護/絶縁膜 相当）
\draw[film] (0,6) rectangle (18,6.5);
\node[lab, anchor=west] at (18.2,6.25) {保護/絶縁フィルム};

% ---- バルクPZTブロック
\draw[pzt] (8,6.5) rectangle (12,26);
\node[lab, anchor=west] at (12.2,16) {バルクPZT};

% ---- COF（縦のフィルム配線）
\draw[film] (6,6.5) rectangle (7,26);
\node[lab, rotate=90] at (6.5,18) {COF};

% ---- 出力側 接合（ACF：COF→PZT）
\draw[bondACF] (7,15) rectangle (8,17);
\node[lab, anchor=east] at (6.6,16) {接合（ACF/Sn）};
\draw[->, line width=0.3pt] (6.4,16) -- (7,16);

% ---- 入力側 接合（はんだ：COF→基板）※下側に別位置で示す
\draw[bondSolder] (6.2,7.2) rectangle (6.8,8.4);
\node[lab, anchor=east] at (5.9,7.8) {接合（はんだ：Sn系）};
\draw[->, line width=0.3pt] (5.7,7.8) -- (6.2,7.8);

% ---- 溶剤蒸気リスクの注記（ACF付近）
\draw[->, line width=0.3pt] (13.8,17.5) -- (8.0,16.8);
\node[lab, align=left, anchor=west] at (14.0,17.5)
{溶剤蒸気環境\\膨潤リスク（ACF）};

% ---- 端子・電圧イメージ（任意）
\draw[->, line width=0.3pt] (6.5,26) -- (6.5,28);
\node[lab, anchor=south] at (6.5,28) {配線（信号/電圧）};

% ---- 凡例
\begin{scope}[shift={(0,29)}]
  \node[lab, anchor=west] at (0,0) {凡例：};
  \draw[film] (8, -2) rectangle (12, 2); \node[lab] at (15,0) {COF/薄膜層};
  \draw[pzt] (24,-2) rectangle (28, 2); \node[lab] at (33,0) {PZT};
  \draw[si] (40,-2) rectangle (44, 2);  \node[lab] at (49,0) {Si};
  \draw[bondACF] (56,-2) rectangle (60, 2); \node[lab, anchor=west] at (61,0) {ACF接合（出力）};
  \draw[bondSolder] (80,-2) rectangle (84, 2); \node[lab, anchor=west] at (85,0) {はんだ接合（入力）};
\end{scope}

% ---- ラベル（左縦）
\node[lab, anchor=east] at (-0.5,22) {出力（COF→PZT）};
\node[lab, anchor=east] at (-0.5,8.0) {入力（COF→基板）};
