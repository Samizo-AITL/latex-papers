...
\begin{scope}[shift={(0,78)}]
  \draw[solidbox=black!8]   (0,0) rectangle (10,4);  \node[lab,anchor=west]  at (12,2){COF};
  \draw[solidbox=black!15] (32,0) rectangle (42,4); \node[lab,anchor=west]  at (44,2){PZT};
  \draw[solidbox=black!5]  (64,0) rectangle (74,4); \node[lab,anchor=west]  at (76,2){Si};
  \draw[solidbox=black!2]  (96,0) rectangle (106,4);\node[lab,anchor=west]  at (108,2){保護フィルム（PPT?）};
  \draw[solidbox=black!28] (148,0) rectangle (158,4);\node[lab,anchor=west] at (160,2){ACF 接合（出力）};
  \draw[solidbox=black!45] (200,0) rectangle (210,4);\node[lab,anchor=west] at (212,2){はんだ接合（入力）};
\end{scope}

% ----------------- 下地：Si と キャビティ -----------------
\draw[solidbox=black!5] (0,0) rectangle (200,12);           % Siベース
\draw[lw, fill=white] (36,0.4) rectangle (164,12);          % キャビティ開口
\node[lab] at (100,2.2){キャビティ};
\node[lab,anchor=west] at (165,2.2){Si（基板）};

% 保護フィルム（PPT?）
\draw[solidbox=black!2] (0,12) rectangle (200,13.2);
\node[lab,anchor=west] at (201,12.6){保護フィルム（PPT?）};

% ----------------- バルクPZT（中央） -----------------
\draw[solidbox=black!15] (116,13.2) rectangle (154,70);
\node[lab,anchor=west] at (155,43){バルク PZT};

% ----------------- 左：COF 縦配線 と IC -----------------
\draw[solidbox=black!8] (74,13.2) rectangle (82,70);        % COFストリップ
\node[lab,rotate=90] at (78,42){COF};

% COF上のIC
\draw[draw, lw, rounded corners=0.9mm, fill=black!25] (70,58) rectangle (82,66);
\node[lab,anchor=east] at (69.5,62){IC};
% IC配線（上向き）
\draw[->,lw] (76,70) -- (76,75); \node[lab,anchor=south] at (76,75){配線（信号 / $-10$V 等）};

% ----------------- 出力：COF→PZT（ACF） -----------------
% ACFブリッジ（スケッチ赤部の高さ）
\draw[solidbox=black!28] (82,36) rectangle (116,40);
% ラベルと矢印
\draw[->,lw] (156,56) -- (116,38);
\node[call] at (157,56){出力（COF$\rightarrow$PZT）\\ACF 接合（導電粒子）};

% ----------------- 入力：COF→基板（はんだ） -----------------
\draw[solidbox=black!45] (75,14.0) rectangle (81,16.0);     % COF下端～Siパッドのはんだ
\draw[lw] (78,13.2) -- (78,14.0);                            % COFと接続
\draw[->,lw] (54,15.0) -- (75,15.0);
\node[lab,anchor=east] at (53.5,15.0){入力（COF$\rightarrow$基板） はんだ接合（Sn 系）};

% ----------------- 溶剤蒸気 膨潤リスク（ACF付近） -----------------
\draw[->,lw] (160,42) -- (116,39.2);
\node[call] at (161,42){溶剤蒸気環境\\ACF 樹脂の膨潤リスク};

% ----------------- I/O分離の強調 -----------------
\node[lab,anchor=west] at (5,32){I/O 分離：出力＝ACF ／ 入力＝はんだ};

\end{tikzpicture}
}% end resizebox
\caption{Mach 世代インクジェットヘッドの断面概念図（スケッチ準拠）。左のCOFは上部ICに接続し、\textbf{出力側（COF$\rightarrow$PZT）はACF}、\textbf{入力側（COF$\rightarrow$基板）ははんだ}で接合する二重方式。中央のバルクPZTはキャビティ上に配置され、保護フィルム（PPT?）を介してSi基板上に載る。溶剤蒸気環境ではACF樹脂の膨潤リスクがある。}
\label{fig:mach_dual_bond_xsec_final}
\end{figure*}
%===============================================================================
