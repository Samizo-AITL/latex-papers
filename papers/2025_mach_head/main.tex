...
% === 下層構造（Si・振動膜・キャビティ・SUS島） ===
\filldraw[fill=gray!10, draw=black] (0,6) rectangle (80,11) node[anchor=west] at (80,8.5) {Si 基板};
\filldraw[fill=gray!20, draw=black] (0,11) rectangle (80,12.6) node[anchor=west] at (80,12.0) {振動フィルム};
\draw[dashed] (6,6.3) -- (74,6.3) node[anchor=west] at (80,6.3) {キャビティ};

\filldraw[fill=gray!60, draw=black] (36,12.6) rectangle (44,14.2);
\node[anchor=west] at (44.5,13.5) {SUS 島};

% === PZT（バルク、内部電極付き） ===
\filldraw[fill=gray!5, draw=black] (38,14.2) rectangle (42,45) node[anchor=east] at (38,30) {バルクPZT};
\foreach \y in {16,19,22,25,28,31,34,37,40,43}{\draw[gray!60] (38,\y) -- (42,\y);}

% === COF 縦配線・IC ===
\filldraw[fill=gray!15, draw=black] (50,14.2) rectangle (54,56);
\node[anchor=west] at (54.5,35){COF};
\filldraw[fill=gray!30, draw=black] (49.5,40) rectangle (54.5,48);
\node[anchor=west] at (55,44){IC};

% === SUS放熱板（ICに沿って配置） ===
\filldraw[fill=gray!55, draw=black] (54.8,18) rectangle (57.8,56);
\node[anchor=west,align=left] at (58,38){SUS放熱板\\（IC放熱）};

% === 出力端子（下側 ACF接合） ===
\filldraw[fill=gray!35, draw=black] (42,30) rectangle (50,34);
\draw[->] (60,32) -- (50,32);
\node[anchor=west] at (60,32){出力端子：ACF 接合（COF→PZT）};

% === 入力端子（上側 はんだ接合） ===
\filldraw[fill=gray!25, draw=black] (48,56) rectangle (80,59) node[anchor=west] at (80,57.5) {PCB};
\filldraw[fill=gray!50, draw=black] (52,55.2) rectangle (55.8,56);
\draw[->] (57,55.6) -- (55.8,55.6);
\node[anchor=west] at (57.2,55.6){入力端子：はんだ接合（COF→PCB）};

% === 補助ラベル ===
\node[anchor=east] at (37.8,15.0){PZT下端子};
\draw[->] (37.4,15.0)--(39,14.2);

\end{tikzpicture}
\caption{Mach世代インクジェットヘッドの断面模式図（2カラム対応）。上側は入力端子としてCOFとPCBをはんだ接合し、下側は出力端子としてCOFとPZTをACF接合する。ICの背面にはSUS放熱板を配置し、PZT下端はSUS島を介してキャビティ上の振動膜に接合される。}
\label{fig:bonding_twocolumn}
\end{figure}
