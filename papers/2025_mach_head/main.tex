...
%=================== Mach ヘッド アクチュエータ部 3図（上面×2 + 断面） ===================
% 必要: \usepackage{tikz}\usetikzlibrary{arrows.meta,positioning,fit}
\begin{figure}[t]
\centering
\begin{tikzpicture}[x=1mm,y=1mm,>=Latex,font=\scriptsize]
\tikzset{
  panel/.style={draw,rounded corners=1,thick,inner sep=2,minimum width=70,minimum height=40},
  film/.style={draw,rounded corners=1,fill=gray!10},
  pad/.style={draw=gray!40,fill=gray!40},
  bar/.style={draw=gray!25,fill=gray!25},
  thinlayer/.style={draw,fill=gray!60},
  label/.style={anchor=west}
}

% ---------------- (a) Top view BEFORE (Sn-Bi/Sn) ----------------
\node[panel] (A) at (0,28) {};
\node[anchor=north west] at ([xshift=-33]A.north) {(a) 上面：実装前（Sn–Bi/Sn）};

% COF pads row
\draw[film] ([xshift=-28,yshift=-10]A.north) rectangle ++(56,-14);
\node[label] at ([xshift=30,yshift=-16]A.north) {COF 端子列};
\foreach \x in {-26,-23,-20,-17,-14,-11,-8,-5,-2,1,4,7,10,13,16,19,22,25}{
  \draw[pad] ([xshift=\x,yshift=-10]A.north) rectangle ++(1.8,-14);
}
% PZT comb row (separate)
\foreach \x in {-26,-23,-20,-17,-14,-11,-8,-5,-2,1,4,7,10,13,16,19,22,25}{
  \draw[bar] ([xshift=\x,yshift=-34]A.north) rectangle ++(1.8,-8);
}
\node at ([yshift=-27]A.north) {PZT 櫛歯列（下面）};

% bond arrow
\draw[->,thick] ([yshift=-24]A.north) -- ++(0,-6) node[midway,right]{Sn–Bi/Sn 接合};

% GND/COM hint
\node[anchor=west] at ([xshift=-28,yshift=-8]A.north) {GND};
\node at ([xshift=0,yshift=-8]A.north) {COM 群};
\node[anchor=east] at ([xshift=28,yshift=-8]A.north) {GND};

% ---------------- (b) Top view AFTER (ACF/Sn) ----------------
\node[panel] (B) at (86,28) {};
\node[anchor=north west] at ([xshift=54]B.north) {(b) 上面：実装後（ACF/Sn）};

% COF pads now aligned over PZT; draw single stacked look
\draw[film] ([xshift=58,yshift=-10]B.north) rectangle ++(56,-14);
\node[label] at ([xshift=118,yshift=-16]B.north) {COF 端子列};
% stacked pads (same x)
\foreach \x in {60,63,66,69,72,75,78,81,84,87,90,93,96,99,102,105,108,111}{
  \draw[pad] ([xshift=\x,yshift=-10]B.north) rectangle ++(1.8,-14);
  \draw[bar] ([xshift=\x,yshift=-34]B.north) rectangle ++(1.8,-8);
}
% ACF bonding arrow
\draw[->,thick] ([yshift=-24]B.north) -- ++(0,-6) node[midway,right]{ACF/Sn 接合};

% ---------------- (c) Cross-section with PCB & cavity ----------------
\node[panel,minimum width=142,minimum height=48] (C) at (43,-30) {};
\node[anchor=north west] at ([xshift=-64]C.north) {(c) 断面：COF–ACF–PZT–SUS島（上にPCB）};

% PCB + SAC
\draw[film,fill=gray!15] ([xshift=-50,yshift=-8]C.north) rectangle ++(44,-5);
\node[label] at ([xshift=-6,yshift=-10.5]C.north) {PCB};
\draw[thinlayer] ([xshift=-50,yshift=-13]C.north) rectangle ++(44,-2); % SAC solder
\node[anchor=west] at ([xshift=-6,yshift=-14.5]C.north) {SAC はんだ};

% COF film
\draw[film] ([xshift=-50,yshift=-25]C.north) rectangle ++(44,-6);
\node[label] at ([xshift=-6,yshift=-28]C.north) {COF};

% ACF thin layer
\draw[thinlayer] ([xshift=-50,yshift=-31]C.north) rectangle ++(44,-2.3);
\node[anchor=west] at ([xshift=-6,yshift=-32.3]C.north) {ACF（異方導電）};

% PZT bulk
\draw[film,fill=gray!12] ([xshift=-50,yshift=-49]C.north) rectangle ++(44,-14);
\node[label] at ([xshift=-6,yshift=-56]C.north) {PZT（櫛歯）};

% SUS island + cavity
\draw[film,fill=gray!55] ([xshift=-40,yshift=-63]C.north) rectangle ++(24,-3) node[midway]{SUS 島};
\draw[film,fill=gray!08] ([xshift=-52,yshift=-66]C.north) rectangle ++(48,-6);
\node[anchor=west] at ([xshift=-2,yshift=-69]C.north) {キャビティ構造体};

% Guide arrows
\draw[->] ([xshift=-52,yshift=-31.5]C.north) -- ++(-6,0) node[left]{COF–PZT 接合};
\draw[->] ([xshift=-52,yshift=-13.8]C.north) -- ++(-6,0) node[left]{COF–PCB 接合};

\end{tikzpicture}
\caption{Mach ヘッド アクチュエータ部の3図。（a）実装前の上面（COF端子列とPZT櫛歯が分離、Sn–Bi/Snで接合）。（b）移行後の上面（ACF/Snで接合）。（c）断面（上からPCB–SAC–COF–ACF–PZT–SUS島–キャビティ）。GNDは両端、COMは中央群。}
\label{fig:mach_actuator_3views}
\end{figure}
%================================================================================
% ======================================================================
%--------------------------------------------------------
