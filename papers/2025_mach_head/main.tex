...
% --- 下層（Si/膜/キャビティ/SUS島＋絶縁接着） ---
\filldraw[fill=gray!10,draw=black] (0,6) rectangle (84,11);         % Si
\filldraw[fill=gray!20,draw=black] (0,11) rectangle (84,12.4);       % 振動膜
\draw[dashed] (6,6.3)--(78,6.3) node[anchor=west] at (84,6.3){キャビティ};
\filldraw[fill=gray!60,draw=black] (36,12.4) rectangle (48,14.0);    % SUS島（機械支持）
\filldraw[fill=white,draw=black]   (36,14.0) rectangle (48,14.6);    % 絶縁接着層
\node[anchor=west] at (48.5,13.7){SUS 島（機械支持）};
\node[anchor=west] at (48.5,14.4){絶縁接着層};

% --- PZT（中央・内部電極は水平層） ---
\filldraw[fill=gray!5,draw=black] (38.5,14.6) rectangle (45.5,46);   % 本体
\foreach \y in {16,19,22,25,28,31,34,37,40,43}{\draw[gray!60] (38.5,\y)--(45.5,\y);} % 交互層
\node[anchor=east] at (38.3,30){バルク PZT（$d_{31}$）};

% PZT両側GND端子（側面メタライズ）＋中央COM引出し
\filldraw[fill=black!40,draw=black] (38.5,26) rectangle (39.3,34);   % 左GND
\filldraw[fill=black!40,draw=black] (44.7,26) rectangle (45.5,34);   % 右GND
\filldraw[fill=black!15,draw=black] (41.3,32) rectangle (42.7,46);   % COM引出し（上面へ）
\node[anchor=east] at (38.3,35){GND};
\node[anchor=west] at (45.7,35){GND};
\node at (42.0,44.8){COM};

% --- COF 縦配線（右）＋ IC ＋ SUS放熱板 ---
\filldraw[fill=gray!15,draw=black] (58,14.6) rectangle (62,56); \node[anchor=west] at (62.5,36){COF};
\filldraw[fill=gray!30,draw=black] (57.2,40) rectangle (62.8,48); \node[anchor=west] at (63,44){IC};
\filldraw[fill=gray!55,draw=black] (62.9,18) rectangle (66.0,56); \node[anchor=west,align=left] at (66.3,39){SUS\\放熱板};

% COFバス：両端GND／中央COM（下端にパッド）
\filldraw[fill=black!40,draw=black] (58,52) rectangle (59,18);  % GND-L
\filldraw[fill=black!15,draw=black] (59.4,52) rectangle (60.6,18); % COM
\filldraw[fill=black!40,draw=black] (61,52) rectangle (62,18);  % GND-R
\filldraw[fill=black!40,draw=black] (58,14.8) rectangle (59,16.2);  % pad GND-L
\filldraw[fill=black!15,draw=black] (59.4,14.8) rectangle (60.6,16.2); % pad COM
\filldraw[fill=black!40,draw=black] (61,14.8) rectangle (62,16.2);  % pad GND-R

% --- ACF界面（COF↔PZT のみ導通） ---
\filldraw[pattern=north east lines, pattern color=gray!60, draw=black] (45.5,30) rectangle (58,34);
\node[anchor=west,align=left] at (68,32){ACF 密着接合\\（COM 中央／GND 両端）};
\draw[->] (68,32) -- (58,32);

% --- 入力：PCBとはんだ ---
\filldraw[fill=gray!25,draw=black] (56,56) rectangle (84,59); \node[anchor=west] at (84,57.5){PCB（入力）};
\filldraw[fill=gray!50,draw=black] (60.2,55.2) rectangle (63.0,56);
\node[anchor=west] at (64.2,55.6){はんだ接合（COF→PCB）};
\draw[->] (64.0,55.6) -- (63.0,55.6);

% --- 明示テキスト（SUS島は電気無関係） ---
\node[anchor=west,align=left] at (4,18){\footnotesize ※ SUS島は\textbf{機械支持のみ}。\\\footnotesize 電気的GNDには\textbf{接続しない}。\\\footnotesize GND/COMはCOF経由でPZT内部電極へ。};

\end{tikzpicture}
\caption{接合構造（2カラム）：GND/COMは\textbf{COF→ACF→PZT内部電極}のみでループを形成。SUS島は\textbf{機械支持・電気絶縁}であり、電気回路には関与しない。}
\label{fig:bonding_no_sus_gnd}
\end{figure}
