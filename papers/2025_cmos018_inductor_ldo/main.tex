...
% siunitx settings (detect-all deprecated -> new options)
\sisetup{
  mode                 = match,
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \draw[->] (ldo) -- (vout);
  \end{tikzpicture}
  \caption{Hybrid Buck--LDO regulator architecture using the on-chip laminated inductor and PGS. (overlap resolved)}
  \label{fig:block}
\end{figure}
