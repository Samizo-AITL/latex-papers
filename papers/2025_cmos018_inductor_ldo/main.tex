...
% --- Fig.2: Hybrid Buck–LDO block diagram (with FB & FF, no overlaps) ---
\begin{figure}[t]
  \centering
  \begin{tikzpicture}[font=\footnotesize, node distance=7mm]
    ...
    \tikzset{
      ...
      tap/.style={circle, fill, inner sep=0.8pt}, % feedforward tap
      line/.style={-Stealth}
    }
    ...
    \node[sum, right=14mm of ldo] (sum) {};              % summer before Vout
    \node[right=10mm of sum] (vout) {$V_\text{out}$};    % output label
    \node[above=7mm of buck] (vin) {$V_\text{in}$};

    % Inductor/PGS block (below path)
    \node[blk, below=10mm of ldo, minimum width=40mm] (filter) {On-chip laminated inductor + PGS};

    % Feedback & Feedforward labels (placed below to avoid overlaps)
    \node[below=7mm of buck] (fb) {Feedback};
    \node[below=7mm of ldo] (ff) {Feedforward};

    % Main forward path
    \draw[line] (vin) -- (buck);
    \draw[line] (buck) -- node[above, midway] {$20\,\text{MHz}$} (ldo);
    \draw[line] (ldo) -- (sum);
    \draw[line] (sum) -- ++(6mm,0) node[tap] (tff) {} -- (vout); % tap node for FF

    % Inductor path
    \draw[line] (vout) |- (filter.east);
    \draw[line] (filter.west) -| (buck.south);

    % Feedback (from Vout back to Buck)
    \draw[line] (vout) |- (fb.east);
    \draw[line] (fb) -- (buck.south);

    % Feedforward (from tap near Vout to LDO)
    \draw[line] (tff) |- (ff.east);
    \draw[line] (ff) -- (ldo.south) node[midway,right] {load step};
  \end{tikzpicture}
  \caption{Hybrid Buck--LDO block diagram with explicit \emph{feedback} (V\_out→Buck) and \emph{feedforward} (load-step tap→LDO) using the on-chip laminated inductor with PGS.}
  \label{fig:block}
\end{figure}
