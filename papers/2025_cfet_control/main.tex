...
\begin{tikzpicture}[
  ...
  bidir/.style={<->,thick},               % 双方向矢印
  dashedarrow/.style={-Latex,thick,dashed},
  cut/.style={preaction={draw,white,line width=3pt}} % 下に白い下地
]
...
% u：プラント外周を回す（左下→左辺へ）。値は十分大きくして突き抜け防止。
\coordinate (uhub) at ($(plant.south west)+(-1.6,-0.8)$);
\draw[arrow,cut] (act.west) -- (uhub) |- node[pos=0.25,left,fill=white,inner sep=1pt]{$u$} (plant.west);

% ---- 下：LLM -------------------------------------------------------
\node[box,below=1.30cm of plant] (llm)
  {LLM supervisor\\ retune $(K_p,K_i,K_d),\,T_{th}$};

% 温度フィードバック（下段専用レーンで箱から離して通す）
\draw[dashedarrow,cut] ($(plant.south)+(0,-0.05)$) |- node[pos=0.20,left,fill=white,inner sep=1pt]{$T_{top},\,T_{bot}$} (llm.north);

% ε_d タップ（右外周を経由して文字を避ける）
\draw[dashedarrow,cut] (pid.east) -- ++(1.0,0) |- node[pos=0.35,right,fill=white,inner sep=1pt]{$\varepsilon_d$} (llm.east);

% retune 出力（PID/FSM へ戻す。ラベルは白背景）
\draw[dashedarrow,cut] (llm.east) -- ++(1.1,0) |- node[pos=0.25,right,fill=white,inner sep=1pt]{gain/threshold updates} (pid.north);
\draw[dashedarrow,cut] (llm.east) -- ++(0.8,0) |- (fsm.west);

\end{tikzpicture}%
}
\caption{CFET control block diagram (2-column). Signals are routed in dedicated lanes; labels have white backgrounds; arrows never penetrate blocks; the Top/Bottom coupling is shown by a single bidirectional arrow $k_c$.}
