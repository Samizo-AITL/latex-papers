...
\begin{tikzpicture}[
  font=\footnotesize,
  every node/.style={align=center}, % ← これがポイント
...
\node at ($(plant.north)+(0,0.3)$) {\textbf{CFET stacked tiers (plant)}};

\node[smallbox,anchor=north west,minimum width=6.0cm] (top)
  at ($(plant.north west)+(0.25,-0.55)$)
  {Top tier (p-FET)\quad $P_{\text{top}}\!\to\!T_{\text{top}}$};

\node[smallbox,anchor=north west,minimum width=6.0cm] (bot)
  at ($(top.south west)+(0,-0.4)$)
...
% LLM supervisor
\node[box,minimum width=4.0cm,minimum height=1.3cm,
      below left=0.0cm and 1.5cm of fsm] (llm)
  {\textbf{LLM supervisor}\\ retune $(K_p,K_i,K_d)$, $T_{\text{th}}$};

\draw[dashedarrow] (plant.south) |- node[below,pos=0.25]
  {$T_{\text{top}},T_{\text{bot}}$} (llm.west);
\draw[dashedarrow] (pid.east) |- node[right,pos=0.2]
  {$\varepsilon_d$} (llm.north);
\draw[dashedarrow] (llm.east) -| node[above,pos=0.25]
