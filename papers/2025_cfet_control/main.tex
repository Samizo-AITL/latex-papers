...
\begin{tikzpicture}[
  font=\footnotesize,
  box/.style={draw,rounded corners,fill=gray!5,inner sep=6pt,align=center},
  smallbox/.style={draw,rounded corners,fill=gray!5,inner sep=4pt,align=center},
...
\node at ($(plant.north)+(0,0.25)$) {\textbf{CFET stacked tiers (plant)}};

% tiers
\node[smallbox,anchor=north west,minimum width=3.8cm] (top)
      at ($(plant.north west)+(0.2,-0.45)$)
  {Top tier (e.g., p-FET)\\ $P_{\text{top}}\!\to\!T_{\text{top}}$};

\node[smallbox,anchor=north west,minimum width=3.8cm] (bot)
      at ($(top.south west)+(0,-0.35)$)
  {Bottom tier (e.g., n-FET)\\ $T_{\text{bot}}$};
...
% LLM supervisor
\node[box,minimum width=3.1cm,minimum height=1.1cm,
      below left=0.0cm and 1.1cm of fsm] (llm)
