...
% ===== Fig.1 : 2カラム全幅・トップ配置・重なり回避・高さ控えめ =====
\begin{figure*}[!t]
\centering
% 高さを少し抑えて他図との干渉を減らす（0.92倍）
\scalebox{0.92}{%
\begin{tikzpicture}[
  font=\footnotesize,
  ...
  % 交差が避けられない場合の“白縁取り”（下地を白で描いてから本線）
...
\node[box,minimum width=6.5cm,below=0.60cm of top]
  (bot) {Bottom tier (e.g., n-FET)\\ $R_{\text{wire}}{+}R_{\text{via}},\; C_{\text{load}}{+}C_{\text{inter}}\quad T_{bot}$};

% 縦熱結合（平行2本・被り無し）
\draw[arrow,cut] ($(top.south west)!0.68!(top.south east)$)
 -- node[right,fill=white,inner sep=1pt]{$k_c$}
 ($(bot.north west)!0.68!(bot.north east)$);
\draw[arrow,cut] ($(top.south west)!0.90!(top.south east)$)
...
% u を左下ルートでプラントへ（交差回避）
\draw[arrow,cut] (act.west) -- ++(-1.25,0) |- node[left,fill=white,inner sep=1pt]{$u$} (plant.west);

% ---- 下：LLM -------------------------------------------------------
\node[box,below=1.3cm of plant] (llm)
  {LLM supervisor\\ retune $(K_p,K_i,K_d),\,T_{th}$};

% 温度フィードバック（下段専用レーン）
\draw[dashedarrow,cut] (plant.south) -- (llm.north)
  node[midway,left,fill=white,inner sep=1pt]{$T_{top},\,T_{bot}$};

% ε_d タップ（右外周をUターンして LLM へ）
\draw[dashedarrow,cut] (pid.east) -- ++(1.0,0) |- (llm.east)
  node[pos=0.25,right,fill=white,inner sep=1pt]{$\varepsilon_d$};

% retune 出力（PID/FSM へ戻し）
\draw[dashedarrow,cut] (llm.east) -- ++(1.2,0) |- (pid.north)
  node[pos=0.35,right,fill=white,inner sep=1pt]{gain/threshold updates};
