...
\begin{figure*}[t] % 片カラムなら figure に変更
\centering
\begin{tikzpicture}[
  font=\footnotesize,
  ...
  plantbox/.style={draw,rounded corners,fill=gray!6,inner sep=6pt,
                   minimum width=7.2cm,minimum height=3.2cm},
  arrow/.style={-Latex,thick},
  dashedarrow/.style={-Latex,thick,dashed},
  lbl/.style={fill=white,inner sep=1pt,text opacity=1,fill opacity=0.95}
]
...
\node[box,minimum width=6.6cm,anchor=north] (top)
  at ($(plant.north)+(0,-7mm)$)
  {Top tier (e.g., p-FET)\quad $P_{\text{top}}\!\to\!T_{\text{top}}$};
\node[box,minimum width=6.6cm,anchor=north] (bot)
  at ($(top.south)+(0,-4mm)$)
...
% vertical thermal coupling k_c（右側で2本・重なり無し）
\draw[arrow] ($(top.south)!0.42!(top.south east)$)
  -- node[lbl,right] {$k_c$}
  ($(bot.north)!0.42!(bot.north east)$);
\draw[arrow] ($(top.south)!0.74!(top.south east)$)
  -- ($(bot.north)!0.74!(bot.north east)$);

% ε_d：ラベルは矢印外側へ
\draw[arrow] (plant.east) -- ++(11mm,0) coordinate (tapd);
\node[lbl,above] at ($(plant.east)!0.55!(tapd)$) {$\varepsilon_d$};

% ---------------- PID / FSM / ACT ----------------
\node[box,minimum width=3.6cm] (pid)
  at ($(tapd)+(24mm,0)$)
  {\textbf{PID} controller\\[-1pt] $(K_p,K_i,K_d)$};
\draw[arrow] (tapd) -- (pid.west);

\node[box,minimum width=3.6cm,below=11mm of pid] (fsm)
  {\textbf{FSM} guard\\[-1pt] HOT mode if $T_{\text{top}}>85^\circ\mathrm{C}$};
\draw[arrow] (pid.south) -- ++(0,-3mm) -- (fsm.north);

\node[box,minimum width=3.0cm,below=11mm of fsm] (act)
  {Actuation $u$\\[-1pt] (DVFS)};
\draw[arrow] (fsm.south) -- (act.north);

% u をプラントへ：右→下→左→上の専用レーンで交差回避
\path (act.west) -- ++(-15mm,0) coordinate (uA)
                -- ++(0,-8mm)   coordinate (uB)
                -- ++(-38mm,0)  coordinate (uC);
\draw[arrow] (act.west) -- (uA) -- (uB) -- (uC) |- (plant.west);
\node[lbl,left] at ($(uC)!0.35!(plant.west)$) {$u$};

% ---------------- LLM supervisor（プラントの真下で安全距離） -----
\node[box,align=center] (llm)
  at ($(plant.south)-(0,15mm)$)
  {\textbf{LLM} supervisor\\[-1pt]
   retune $(K_p,K_i,K_d),\,T_{\text{th}}$};

% T_top/T_bot 計測線：ラベルは2行にして接触回避
\draw[dashedarrow] (plant.south) |- ($(llm.west)+(0,0)$);
\node[lbl,below] at ($(plant.south)!0.35!(llm.west)$)
  {\shortstack{$T_{\text{top}}$\\$T_{\text{bot}}$}};

% ε_d タップは PID から LLM へ：右外側に回してから下降→左折
\path (pid.east) -- ++(10mm,0) coordinate (pout)
                 -- ++(0,-18mm) coordinate (pturn)
                 -- ++(-10mm,0) coordinate (pin);
\draw[dashedarrow] (pid.east) -- (pout) -- (pturn) -- (pin) -- (llm.north);
\node[lbl,right] at ($(pout)!0.05!(pturn)$) {$\varepsilon_d$};

% LLM からの retune：長文ラベルは右端で表示して衝突回避
\path (llm.east) -- ++(12mm,0) coordinate (r1)
                 -- ++(0,23mm) coordinate (r2);
\draw[dashedarrow] (llm.east) -- (r1) -- (r2) -| (pid.north);
\draw[dashedarrow] (llm.east) -- (r1) |- (fsm.west);
\node[lbl,above] at ($(r1)!0.65!(r2)$) {gain/threshold updates};

\end{tikzpicture}

\caption{Proposed architecture arranged to avoid any line overlap.
All labels use white backgrounds; signal routes are kept in dedicated
lanes outside boxes.}
