...
% --- Plant -----------------------
\node[draw,rounded corners,minimum width=6cm,minimum height=2.6cm] (plant) {};
\node[lbl,above] at (plant.north) {CFET stacked tiers (plant)};

\node[box,minimum width=5.4cm] (top) at ($(plant.north)+(0,-0.6)$)
  {Top tier (e.g., p-FET)\\ $P_{top}\!\to\!T_{top}$};
\node[box,minimum width=5.4cm,below=0.5cm of top]
  (bot) {Bottom tier (e.g., n-FET)\\ $T_{bot}$};

% thermal coupling
\draw[arrow] ($(top.south east)!0.7!(top.south west)$)
 -- node[right]{$k_c$}
 ($(bot.north east)!0.7!(bot.north west)$);

% delay deviation
\coordinate (outd) at ($(plant.east)+(0,0)$);
\draw[arrow] (plant.east) -- ++(1.2,0) coordinate(eps);
\node[lbl,above] at ($(plant.east)!0.5!(eps)$) {$\varepsilon_d$};

% --- PID / FSM / Actuation --------
\node[box,right=2.5cm of plant] (pid)
  {PID controller\\ $(K_p,K_i,K_d)$};
\draw[arrow] (eps) -- (pid.west);

\node[box,below=0.8cm of pid] (fsm)
  {FSM guard\\ HOT if $T_{top}>85^\circ$C};
\draw[arrow] (pid.south) -- (fsm.north);

\node[box,below=0.8cm of fsm] (act)
  {Actuation $u$\\ (DVFS)};
\draw[arrow] (fsm.south) -- (act.north);

% u back to plant
\draw[arrow] (act.west) -- ++(-1.0,0) |- node[lbl,left]{$u$} (plant.west);

% --- LLM --------------------------
\node[box,below=1.2cm of plant] (llm)
...
% eps_d feedback
\draw[dashedarrow] (pid.east) -- ++(0.8,0) |- (llm.east)
  node[pos=0.25,lbl,right]{$\varepsilon_d$};

% retuning outputs
\draw[dashedarrow] (llm.east) -- ++(1.0,0) |- (pid.north)
  node[pos=0.3,lbl,right]{gain updates};
\draw[dashedarrow] (llm.east) -- ++(0.5,0) |- (fsm.west);

\end{tikzpicture}
\caption{CFET control block diagram with arrows routed to avoid overlap.}
\end{figure}
