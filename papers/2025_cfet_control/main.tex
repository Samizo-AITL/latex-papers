...
% --- Plant: CFET stacked tiers ------------------------------------
\node[box,minimum width=4.2cm,minimum height=2.8cm] (plant) {};
\node at ($(plant.north)+(0,0.25)$) {\bf CFET stacked tiers (plant)};

% top tier
\node[smallbox,anchor=north west,minimum width=3.8cm] (top) at ($(plant.north west)+(0.2,-0.45)$)
  {Top tier (e.g., p-FET)\quad $P_{\text{top}}\!\to\!T_{\text{top}}$};
% bottom tier
\node[smallbox,anchor=north west,minimum width=3.8cm] (bot) at ($(top.south west)+(0,-0.35)$)
...
% vertical thermal coupling kc
\draw[arrow] ([xshift=0.4cm]top.south) -- node[right] {$k_c$} ([xshift=0.4cm]bot.north);
\draw[arrow] ([xshift=1.0cm]top.south) -- ([xshift=1.0cm]bot.north);

% output: delay deviation eps_d
\node[right=1.5cm of plant.east] (epslabel) {};
\draw[arrow] (plant.east) -- node[above,pos=0.5] {delay deviation $\varepsilon_d$} (epslabel.center);

% --- PID controller ------------------------------------------------
\node[box,minimum width=2.5cm,minimum height=1.0cm, right=2.2cm of plant] (pid)
  {{\bf PID} controller\\ $(K_p,K_i,K_d)$};
\draw[arrow] (epslabel.center) -- (pid.west);

% --- FSM guard -----------------------------------------------------
\node[box,minimum width=2.6cm,minimum height=1.0cm, below=0.45cm of pid] (fsm)
  {{\bf FSM} guard\\ HOT mode if $T_{\text{top}}>85^\circ$C};
\draw[arrow] (pid.south) -- node[right] {} (fsm.north);

% --- Actuation u ---------------------------------------------------
\node[box,minimum width=1.8cm,minimum height=0.9cm, below=0.45cm of fsm] (act)
  {Actuation $u$\\ (DVFS)};
\draw[arrow] (fsm.south) -- (act.north);

% feed back into plant (u -> plant)
\draw[arrow] (act.west) -- ++(-1.05,0) |- node[pos=0.25,left] {$u$} (plant.west);

% --- LLM supervisor (retuning) ------------------------------------
\node[box,minimum width=3.1cm,minimum height=1.1cm, below left=0.0cm and 1.1cm of fsm] (llm)
  {{\bf LLM} supervisor\\ retune $(K_p,K_i,K_d)$,\;$T_{\text{th}}$};
% taps from signals to LLM
...
% legend (optional)
\node[note,below=0.2cm of act] (leg1) {\tikz{\draw[arrow] (0,0)--(0.6,0);} control signal};
\node[note,below=0.2cm of leg1] (leg2) {\tikz{\draw[dashedarrow] (0,0)--(0.6,0);} supervision/retuning};
