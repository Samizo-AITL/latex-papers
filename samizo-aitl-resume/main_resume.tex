...
\PassOptionsToClass{engine=luatex}{bxjsarticle} % ← 正しい指定
...
%==== クラス指定 ====
\documentclass[ja=standard,11pt]{bxjsarticle}  % engine=luatex は PassOptionsToClass で渡したので不要
\usepackage{geometry} % ← 余白18mmを有効化（本文で読み込む）

%==== 日本語／欧文フォント設定 ====
\usepackage{luatexja}
\usepackage{luatexja-fontspec} % fontspec を内包
...
% （★）siunitx は未使用なので外します。使う場合は v3 互換で：
% \usepackage{siunitx}
